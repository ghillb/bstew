...
class NetLogoDataLoader:
    ...
    def _load_population_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_path in pop_files:
                ...
                pop_data.update(
                    {
                        "total_population_time_series": df.get(
                            "total_population", pd.Series([])
                        ).tolist()
                        if "total_population" in df
                        else [],
                        "egg_count_time_series": df.get(
                            "egg_count", pd.Series([])
                        ).tolist()
                        if "egg_count" in df
                        else [],
                        "larva_count_time_series": df.get(
                            "larva_count", pd.Series([])
                        ).tolist()
                        if "larva_count" in df
                        else [],
                        "pupa_count_time_series": df.get(
                            "pupa_count", pd.Series([])
                        ).tolist()
                        if "pupa_count" in df
                        else [],
                        "adult_count_time_series": df.get(
                            "adult_count", pd.Series([])
                        ).tolist()
                        if "adult_count" in df
                        else [],
                        "queen_count": df.get("queen_count", pd.Series([])).tolist()
                        if "queen_count" in df
                        else [],
                        "drone_count": df.get("drone_count", pd.Series([])).tolist()
                        if "drone_count" in df
                        else [],
                        "worker_count": df.get("worker_count", pd.Series([])).tolist()
                        if "worker_count" in df
                        else [],
                    }
                )
        ...
        try:
            ...
            for file_path in activity_files:
                ...
                activity_data.update(
                    {
                        "foraging_activity": df.get(
                            "foraging_bees", pd.Series([])
                        ).tolist()
                        if "foraging_bees" in df
                        else [],
                        "nursing_activity": df.get(
                            "nursing_bees", pd.Series([])
                        ).tolist()
                        if "nursing_bees" in df
                        else [],
                        "resting_activity": df.get(
                            "resting_bees", pd.Series([])
                        ).tolist()
                        if "resting_bees" in df
                        else [],
                        "building_activity": df.get(
                            "building_bees", pd.Series([])
                        ).tolist()
                        if "building_bees" in df
                        else [],
                        "activity_transitions": df.get(
                            "activity_transitions", pd.Series([])
                        ).tolist()
                        if "activity_transitions" in df
                        else [],
    ...
    def _load_foraging_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_path in foraging_files:
                ...
                # Extract data columns with proper type handling
                def safe_tolist(column_data: Any) -> List[Any]:
                    """Safely convert column data to list"""
                    if hasattr(column_data, "tolist"):
                        return list(column_data.tolist())
                ...
                foraging_data.update(
                    {
                        "total_foraging_trips": safe_tolist(df.get("total_trips", [])),
                        "successful_trips": safe_tolist(df.get("successful_trips", [])),
                        "foraging_efficiency": safe_tolist(
                            df.get("foraging_efficiency", [])
                        ),
                        "trip_durations": safe_tolist(df.get("trip_duration", [])),
                        "energy_collected": safe_tolist(df.get("energy_collected", [])),
                        "patch_visitation": safe_tolist(df.get("patches_visited", [])),
                        "dance_frequency": safe_tolist(df.get("dances_performed", [])),
                    }
                )
    ...
    def _load_mortality_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_path in mortality_files:
                ...
                def safe_to_dict(column_data: Any) -> Dict[str, Any]:
                    """Safely convert column data to dict"""
                    if hasattr(column_data, "to_dict"):
                        return dict(column_data.to_dict())
                ...
                mortality_data.update(
                    {
                        "daily_deaths": safe_tolist(df.get("daily_deaths", [])),
                        "mortality_rate": safe_tolist(df.get("mortality_rate", [])),
                        "death_causes": safe_to_dict(df.get("death_causes", {}))
                        if "death_causes" in df
                        else {},
                        "lifespan_distribution": safe_tolist(df.get("lifespan", [])),
                        "age_at_death": safe_tolist(df.get("age_at_death", [])),
                    }
                )
    ...
    def _load_energy_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_path in energy_files:
                ...
                energy_data.update(
                    {
                        "colony_energy": safe_tolist(df.get("colony_energy", [])),
                        "daily_energy_intake": safe_tolist(df.get("daily_intake", [])),
                        "daily_energy_consumption": safe_tolist(
                            df.get("daily_consumption", [])
                        ),
                        "energy_balance": safe_tolist(df.get("energy_balance", [])),
                        "individual_energy_levels": safe_tolist(
                            df.get("individual_energy", [])
                        ),
                    }
                )
    ...
    def _load_spatial_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_path in spatial_files:
                ...
                spatial_data.update(
                    {
                        "foraging_distances": safe_tolist(
                            df.get("foraging_distance", [])
                        ),
                        "patch_preferences": safe_to_dict(
                            df.get("patch_preferences", {})
                        )
                        if "patch_preferences" in df
                        else {},
                        "territory_size": safe_tolist(df.get("territory_size", [])),
                        "spatial_clustering": safe_tolist(
                            df.get("clustering_coefficient", [])
                        ),
                    }
                )
    ...
    def _extract_daily_cycles(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        if "hour" in df.columns and "activity_level" in df.columns:
            day_column = df.get("day", [0])
            unique_days = day_column.unique() if hasattr(day_column, "unique") else [0]
            for day in unique_days:
                day_data = df[df["day"] == day] if "day" in df else df
                cycle = {
                    "day": day,
                    "hourly_activity": day_data.groupby("hour")["activity_level"]
                    .mean()
                    .to_dict(),
                }
                cycles.append(cycle)
        ...
        try:
            ...
            for colony_id, metrics in self.data_collector.colony_metrics.items():
                if hasattr(metrics, "population_size_day_list"):
                    population_data[f"colony_{colony_id}_population"] = (
                        metrics.population_size_day_list
                    )
                    population_data[f"colony_{colony_id}_eggs"] = getattr(
                        metrics, "egg_count_day_list", []
                    )
                    population_data[f"colony_{colony_id}_larvae"] = getattr(
                        metrics, "larva_count_day_list", []
                    )
                    population_data[f"colony_{colony_id}_pupae"] = getattr(
                        metrics, "pupa_count_day_list", []
                    )
                    population_data[f"colony_{colony_id}_adults"] = getattr(
                        metrics, "adult_count_day_list", []
                    )
            ...
            if population_data:
                ...
                if all_colonies_pop:
                    ...
                    for i in range(max_len):
                        day_total = sum(
                            series[i] if i < len(series) else 0
                            for series in all_colonies_pop
                        )
            ...
            foraging_data.update(
                {
                    ...
                    "foraging_efficiency": [
                        s / t if t > 0 else 0
                        for s, t in zip(successful_trips, total_trips)
                    ]
                    if total_trips
                    else [],
                }
...
class BehavioralValidator:
    ...
    def validate_population_dynamics(
        self, bstew_data: Dict[str, Any], netlogo_data: Dict[str, Any]
    ) -> ValidationResult:
        ...
        if (
            "total_population_time_series" in bstew_data
            and "total_population_time_series" in netlogo_data
        ):
    ...
    def validate_activity_patterns(
        ...
        for activity in activities:
            ...
            if bstew_key in bstew_data and netlogo_key in netlogo_data:
                ...
                metric = self._create_validation_metric(
                    f"{activity}_level", bstew_val, float(netlogo_val), tolerance
                )
                metrics.append(metric)

        return self._create_validation_result("activity_patterns", metrics)

    def validate_foraging_behavior(
        self, bstew_data: Dict[str, Any], netlogo_data: Dict[str, Any]
    ) -> ValidationResult:
    ...
    def _create_validation_metric(
        self, name: str, bstew_val: float, netlogo_val: float, tolerance: float
    ) -> ValidationMetric:
        """Create a validation metric object"""

        difference = bstew_val - netlogo_val
        relative_diff = abs(difference) / max(
            abs(netlogo_val), 1e-6
    ...
    def _create_validation_result(
        self, category: str, metrics: List[ValidationMetric]
    ) -> ValidationResult:
...
class NetLogoValidationSuite:
    ...
    def __init__(
        self,
        netlogo_data_path: str,
        output_path: str = "artifacts/validation/validation_results",
    ):
    ...
    def run_complete_validation(
        ...
        if netlogo_data.get("population_data") and bstew_data.get("population_data"):
            validation_results["population_dynamics"] = (
                self.validator.validate_population_dynamics(
                    bstew_data["population_data"], netlogo_data["population_data"]
                )
            )

        # Activity patterns validation
        if netlogo_data.get("activity_patterns") and bstew_data.get(
            "activity_patterns"
        ):
            validation_results["activity_patterns"] = (
                self.validator.validate_activity_patterns(
        ...
        if netlogo_data.get("foraging_data") and bstew_data.get("foraging_data"):
            validation_results["foraging_behavior"] = (
                self.validator.validate_foraging_behavior(
    ...
    def _create_validation_plots(self, results: Dict[str, ValidationResult]) -> None:
        ...
        try:
            ...
            axes[0, 0].bar(
                ...
                pass_rates,
                color=[
                    "green" if pr >= 0.8 else "orange" if pr >= 0.6 else "red"
            ...
            for result in results.values():
                all_diffs.extend(
                    [m.relative_difference for m in result.individual_results]
                )
            ...
            labels = [
                ...
                f"Failed ({total_metrics - total_passed})",
            ]
            ...
            plt.tight_layout()
            plt.savefig(
                self.output_path / "validation_plots.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
