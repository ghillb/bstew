...
class VisualizationEngine(BaseModel):
    ...
    def register_data_stream(self, stream_id: str, data_source: str, 
    ...
    def create_colony_overview_chart(self, colony_data: Dict[str, Any]) -> Optional[Any]:
        ...
        fig.update_layout(
            title_text="Colony Overview Dashboard",
            showlegend=True,
            height=800
        )
    ...
    def create_spatial_visualization(self, spatial_data: Dict[str, Any]) -> Optional[Any]:
    ...
    def create_communication_network(self, communication_data: Dict[str, Any]) -> Optional[Any]:
    ...
    def create_time_series_chart(self, time_series_data: Dict[str, Any], 
                                chart_type: VisualizationType) -> Optional[Any]:
        ...
        fig.update_layout(
            title=f"{chart_type.replace('_', ' ').title()} - Time Series",
            xaxis_title="Time",
            yaxis_title="Value",
            showlegend=True
        )
    ...
    def generate_dashboard(self, data_snapshot: Dict[str, Any]) -> Optional[str]:
        ...
        if 'colony_data' in data_snapshot:
            colony_chart = self.create_colony_overview_chart(data_snapshot['colony_data'])
            if colony_chart:
                charts.append(('Colony Overview', colony_chart))
        
        # Spatial visualization
        if 'spatial_data' in data_snapshot:
            spatial_chart = self.create_spatial_visualization(data_snapshot['spatial_data'])
            if spatial_chart:
                charts.append(('Spatial Distribution', spatial_chart))
        
        # Communication network
        if 'communication_data' in data_snapshot:
            comm_chart = self.create_communication_network(data_snapshot['communication_data'])
            if comm_chart:
                charts.append(('Communication Network', comm_chart))
        
        # Time series charts
        for chart_type in [VisualizationType.POPULATION_TRENDS, 
                          VisualizationType.FORAGING_ACTIVITY,
                          VisualizationType.HEALTH_MONITORING]:
            if chart_type in data_snapshot:
                ts_chart = self.create_time_series_chart(data_snapshot[chart_type], chart_type)
                if ts_chart:
                    charts.append((chart_type.replace('_', ' ').title(), ts_chart))
    ...
    def _create_live_visualization(self, stream_id: str, data: List[VisualizationData]) -> None:
        ...
        if time_series_data:
            chart = self.create_time_series_chart(time_series_data, stream_id)
            
            if chart:
                # Save chart
                output_file = f"{self.output_directory}/live_{stream_id}_{int(time.time())}.html"
...
class LiveVisualizationManager(BaseModel):
    ...
    def register_model_data_collector(self, model, stream_prefix: str = "model") -> None:
        """Register data collector for simulation model"""
        
        # Colony data stream
        def collect_colony_data():
            ...
            for colony in getattr(model, 'get_colonies', lambda: [])():
                ...
                data = VisualizationData(
                    ...
                    data={
                        'colony_id': colony_id,
                        'population': getattr(colony, 'get_adult_population', lambda: 0)(),
                        'health_score': getattr(colony, 'get_health_score', lambda: 0.5)(),
                        'honey_stores': getattr(colony, 'get_honey_stores', lambda: 0.0)(),
                        'forager_count': getattr(colony, 'get_forager_count', lambda: 0)()
                    }
                )
        ...
        def collect_communication_data():
            if hasattr(model, 'dance_communication_integrator') and model.dance_communication_integrator:
                for colony in getattr(model, 'get_colonies', lambda: [])():
                    colony_id = getattr(colony, 'unique_id', 0)
                    comm_metrics = model.dance_communication_integrator.get_colony_communication_metrics(colony_id)
                    ...
                    self.engine.add_data(f"{stream_prefix}_communication_{colony_id}", data)
        ...
        # Register streams
        self.engine.register_data_stream(f"{stream_prefix}_colonies", "model", self.collection_interval)
        self.engine.register_data_stream(f"{stream_prefix}_spatial", "model", self.collection_interval)
        self.engine.register_data_stream(f"{stream_prefix}_communication", "model", self.collection_interval)
        
        self.logger.info(f"Registered model data collectors with prefix: {stream_prefix}")
...
def create_live_visualization_system(output_dir: str = "visualization_output") -> LiveVisualizationManager:
