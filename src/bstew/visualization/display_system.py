...
class DisplayStateManager:
    ...
    def __init__(
        self, config_file: str = "artifacts/visualization/display_config.json"
    ):
    ...
    def create_custom_configuration(
        ...
        name: str,
        description: str,
        toggle_states: Optional[Dict[DisplayToggle, bool]] = None,
    ) -> None:
        ...
        config = DisplayConfiguration(
            ...
            visibility_filters=self.visibility_filters.copy()
            if self.visibility_filters
            else None,
            display_parameters=self.display_parameters.copy()
    ...
    def save_configuration(self) -> None:
        ...
        try:
            config_data = {
                "current_toggles": {
                    toggle.value: state
                    for toggle, state in self.current_toggles.items()
                },
                ...
                "predefined_configs": {
                    name: {
                        ...
                        "toggle_states": {
                            toggle.value: state
                            for toggle, state in config.toggle_states.items()
                        },
        ...
        try:
            ...
            if "current_color_scheme" in config_data:
                self.current_color_scheme = ColorScheme(
                    config_data["current_color_scheme"]
                )
            ...
            if "predefined_configs" in config_data:
                for name, config_dict in config_data["predefined_configs"].items():
                    if (
                        name not in self.predefined_configs
                    ):  # Don't override built-in configs
                        toggle_states = {
                            DisplayToggle(toggle): state
...
class ConditionalVisualization:
        ...
        if current_scheme in self.color_schemes:
            ...
            if hasattr(entity, attribute):
                ...
                if isinstance(value, str):
                    default_color = color_map.get("default", "#808080")
                    return str(color_map.get(value, default_color))
                elif isinstance(value, (int, float)):
                    # Map numeric values to categories
                    if attribute in ["health", "habitat_quality", "connectivity_index"]:
                        if value > 0.8:
                            return str(color_map.get("high", "#2ECC71"))
                        elif value > 0.5:
                            return str(color_map.get("medium", "#F39C12"))
                        elif value > 0.2:
                            return str(color_map.get("low", "#E74C3C"))
                        else:
                            return str(color_map.get("none", "#95A5A6"))

            return str(color_map.get("default", "#808080"))

        # Use custom colors if available
        if self.display_manager.custom_colors:
            return str(self.display_manager.custom_colors.get(attribute, "#808080"))
    ...
    def should_display_entity(self, entity: Any, entity_type: str) -> bool:
        ...
        if entity_type == "cohort" and not self.display_manager.get_toggle_state(
            DisplayToggle.SHOW_COHORTS
        ):
            return False

        if entity_type == "dead_colony" and not self.display_manager.get_toggle_state(
            DisplayToggle.SHOW_DEAD_COLS
        ):
            return False

        if entity_type == "path" and not self.display_manager.get_toggle_state(
            DisplayToggle.SHOW_PATHS
        ):
            return False

        if entity_type == "resource" and not self.display_manager.get_toggle_state(
            DisplayToggle.SHOW_RESOURCES
        ):
            return False

        if entity_type == "territory" and not self.display_manager.get_toggle_state(
            DisplayToggle.SHOW_TERRITORIES
        ):
            return False
        ...
        if self.display_manager.visibility_filters:
            for (
                filter_name,
                filter_value,
            ) in self.display_manager.visibility_filters.items():
    ...
    def get_display_properties(self, entity: Any, entity_type: str) -> Dict[str, Any]:
        ...
        if self.display_manager.get_toggle_state(DisplayToggle.SHOW_SPECIES):
            ...
            properties["color"] = self.get_color_for_entity(
                entity, "connectivity_index"
            )
    ...
    def apply_conditional_styling(
        self, ax: plt.Axes, entities: List[Any], entity_type: str
    ) -> None:
        ...
        # Plot points - use average alpha since scatter doesn't support per-point alpha
        avg_alpha = sum(alphas) / len(alphas) if alphas else 0.7
        _scatter = ax.scatter(
            x_coords,
            y_coords,
            c=colors,
    ...
    def export_display_configuration(self, output_path: str) -> None:
        ...
        config_info = {
            ...
            "color_schemes": {
                scheme.value: colors for scheme, colors in self.color_schemes.items()
            },
        }
...
class DisplayControlInterface:
    ...
    def create_visualization(
        self, entities: List[Any], entity_type: str, output_path: Optional[str] = None
    ) -> plt.Figure:
