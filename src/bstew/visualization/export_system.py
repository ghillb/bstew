...
class VisualizationExporter:
    ...
    def export_static_visualization(self,
                                  agents: List[Any],
                                  patches: Optional[List[Any]] = None,
                                  output_path: str = "artifacts/exports/visualization",
                                  config: Optional[ExportConfiguration] = None) -> Dict[str, str]:
        ...
        fig = self.viz_engine.create_static_visualization(
            agents=agents,
            patches=patches,
            title="BSTEW Agent Visualization"
        )
        ...
        self.logger.info(f"Static visualization exported to {len(exported_files)} files")
        return exported_files
        
    def export_animation(self,
                        frame_data: List[VisualizationFrame],
                        output_path: str = "artifacts/exports/animation",
                        config: Optional[ExportConfiguration] = None) -> Dict[str, str]:
        ...
        metadata_path = output_path.with_suffix('_frames.json')
        with open(metadata_path, 'w') as f:
    ...
    def export_interactive_plots(self,
                                data_sources: Dict[str, Any],
                                output_path: str = "artifacts/exports/interactive",
                                config: Optional[ExportConfiguration] = None) -> Dict[str, str]:
            ...
            if isinstance(data, (list, pd.DataFrame)):
                ...
                if len(numeric_cols) >= 2:
                    ...
                    scatter_fig = self.data_engine.create_interactive_plot(
                        source_name, "scatter", numeric_cols[0], numeric_cols[1],
                    ...
                    # Correlation matrix
                    if len(numeric_cols) > 2:
                        corr_fig = self.data_engine.create_correlation_matrix(source_name)
                        corr_path = output_path.parent / f"{source_name}_correlation.html"
    ...
    def generate_comprehensive_report(self,
                                    ...
                                    config: Optional[ExportConfiguration] = None) -> str:
        """Generate comprehensive analysis report with visualizations"""
        
        if config is None:
            config = ExportConfiguration(
                format=ExportFormat.PDF,
                quality=ExportQuality.PUBLICATION
            )
        ...
        if analysis_data:
            ...
            for analysis_name, data in analysis_data.items():
                story.append(Paragraph(analysis_name.replace('_', ' ').title(), styles['Heading3']))
    ...
    def export_publication_figures(self,
                                 agents: List[Any],
                                 patches: Optional[List[Any]] = None,
                                 output_directory: str = "artifacts/exports/publication",
                                 formats: List[ExportFormat] = None) -> Dict[str, List[str]]:
        ...
        fig2.suptitle("Species Composition in BSTEW Simulation", fontsize=16, fontweight='bold')
        ...
        fig3.suptitle("Activity Distribution Across Agent Population", fontsize=16, fontweight='bold')
        ...
        self.logger.info(f"Publication figures exported: {total_files} files across {len(formats)} formats")
    ...
    def create_export_package(self,
                             ...
                             analysis_data: Optional[Dict[str, Any]] = None,
                             output_path: str = "artifacts/exports/complete_package.zip") -> str:
        ...
        try:
            ...
            if frame_data:
                ...
                self.export_animation(
                    frame_data,
                    str(anim_dir / "simulation_animation")
                )
            ...
            self.export_interactive_plots(
                data_sources,
                str(interactive_dir / "dashboard")
            )
            
            # Generate comprehensive report
            _report_path = self.generate_comprehensive_report(
                agents, patches, analysis_data,
                str(temp_dir / "comprehensive_report")
            )
        ...
        self.logger.info(f"Complete export package created: {output_path}")
        return str(output_path)
        
    def _apply_export_styling(self, fig: plt.Figure, config: ExportConfiguration) -> None:
    ...
    def _create_species_distribution_chart(self, agents: List[Any]) -> plt.Figure:
        ...
        wedges, texts, autotexts = ax.pie(counts, labels=species, autopct='%1.1f%%', startangle=90)
