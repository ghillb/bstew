...
from .display_system import DisplayStateManager, ConditionalVisualization, DisplayToggle
...
class AgentVisualizationEngine:
    ...
    def create_static_visualization(
        ...
        fig.patch.set_facecolor(str(self.config["background_color"]))
        ...
        if self.display_manager.get_toggle_state(DisplayToggle.SHOW_STATISTICS):
    ...
    def _draw_patches(self, ax: plt.Axes, patches: List[SpatialPatch]) -> None:
        ...
        if not self.display_manager.get_toggle_state(DisplayToggle.SHOW_RESOURCES):
            return
    ...
    def _draw_connections(
        self, ax: plt.Axes, connections: List[Dict[str, Any]]
    ) -> None:
        """Draw connections between patches or agents"""

        if not self.display_manager.get_toggle_state(DisplayToggle.SHOW_CONNECTIVITY):
            return
    ...
    def _draw_agents(self, ax: plt.Axes, agents: List[Any]) -> None:
        ...
        agent_groups: Dict[Tuple[str, str], Dict[str, List[Any]]] = {}
        ...
        for agent in agents:
            ...
            color = self.color_manager.get_agent_color(
                agent, str(self.config.get("color_mode", "species"))
            )
            base_size = self.config.get("agent_base_size", 5.0)
            # Convert base_size safely
            if isinstance(base_size, (int, float)):
                size_value = float(base_size)
            else:
                size_value = 5.0
            size = self.color_manager.get_agent_size(agent, size_value)
        ...
        for (species, role), group_data in agent_groups.items():
            ...
            ax.scatter(
                ...
                c=group_data["colors"],
                s=group_data["sizes"],
                alpha=group_data["alphas"][0] if group_data["alphas"] else 0.7,
                edgecolors="black",
                linewidth=0.5,
    ...
    def _add_legend(self, ax: plt.Axes, agents: List[Any]) -> None:
        ...
        for species, role in sorted(species_roles):
            color = self.color_manager.species_colors.get(species, {}).get(
                role, "#808080"
            )
    ...
    def _add_statistics(self, ax: plt.Axes, agents: List[Any]) -> None:
        ...
        status_counts: Dict[BeeStatus, int] = {}
    ...
    def create_animated_visualization(
        self, frame_data: List[VisualizationFrame], output_path: Optional[str] = None
        ...
        def animate(frame_idx: int) -> List[Any]:
            ...
            if frame_idx < len(frame_data):
                ...
                ax.text(
                    0.02,
                    0.02,
                    f"Time: {frame.timestamp.strftime('%H:%M:%S')}",
                    transform=ax.transAxes,
                    fontsize=10,
                    bbox=dict(boxstyle="round", facecolor="white", alpha=0.8),
                )
    ...
    def create_comparative_visualization(
        ...
        title: str = "Comparative Agent Visualization",
    ) -> plt.Figure:
        ...
        fig, axes = plt.subplots(
            ...
            cols,
            figsize=(
                float(self.config.get("figure_size", (12, 10))[0]) * cols,  # type: ignore[index]
                float(self.config.get("figure_size", (12, 10))[1]) * rows,  # type: ignore[index]
            ),
        )
    ...
    def set_visualization_config(self, **kwargs: Any) -> None:
    ...
    def get_visualization_summary(self, agents: List[Any]) -> Dict[str, Any]:
        ...
        for agent in agents:
            ...
            summary["species_distribution"][species] = (
                summary["species_distribution"].get(species, 0) + 1
            )
            summary["role_distribution"][role] = (
                summary["role_distribution"].get(role, 0) + 1
            )
            summary["status_distribution"][str(status)] = (
                summary["status_distribution"].get(str(status), 0) + 1
            )
