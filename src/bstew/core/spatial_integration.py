...
class SpatialBeeState:
    ...
    spatial_memory: Dict[int, float] = field(
        default_factory=dict
    )  # patch_id -> quality memory

    def update_position(
        self, new_position: SpatialPoint, patch_id: Optional[int] = None
    ) -> None:
    ...
    def remember_patch_quality(self, patch_id: int, quality: float) -> None:
        ...
        if patch_id in self.spatial_memory:
            self.spatial_memory[patch_id] = (
                0.7 * self.spatial_memory[patch_id] + 0.3 * quality
            )
        else:
...
class SpatialEnvironmentManager(BaseModel):
    ...
    def initialize_landscape(self, landscape_config: Dict[str, Any]) -> None:
        ...
        self.logger.info(
            f"Initialized landscape with {len(self.spatial_index._patches)} patches"
        )
    ...
    def _load_patches_from_gis(
        self, landscape_config: Dict[str, Any]
    ) -> List[SpatialPatch]:
        ...
        try:
            ...
            gis_patches = self.gis_manager.load_spatial_data_from_config(
                landscape_config
            )
            ...
            next_id = (
                max(
                    [p.patch_id for p in self.spatial_index._patches.values()],
                    default=-1,
                )
                + 1
            )
    ...
    def _update_patch_dynamics(self, patch_id: int, patch: SpatialPatch) -> None:
        ...
        if usage > 10:  # High usage threshold
            ...
            self.patch_qualities[patch_id] = max(
                0.1, self.patch_qualities[patch_id] - degradation
            )
        ...
        for patch_id in self.resource_levels:
            ...
            if patch:
                ...
                if current_level < max_level:
                    regeneration = self.resource_regeneration_rate * (
                        max_level - current_level
                    )
                    self.resource_levels[patch_id] = min(
                        max_level, current_level + regeneration
                    )
    ...
    def find_optimal_foraging_patches(
        self,
        bee_position: SpatialPoint,
        ...
        nearby_patches = self.spatial_index.find_patches_in_radius(
            bee_position, max_distance
        )
        ...
        for patch_id in nearby_patches:
            if patch_id in self.spatial_index._patches:
                ...
                if current_quality >= min_quality and resource_level > 0.1:
                    ...
                    score = (current_quality * resource_level) / max(
                        1.0, distance / 10.0
                    )
    ...
    def export_landscape_to_gis(
        ...
        if not self.gis_manager:
            raise RuntimeError(
                "GIS manager not initialized - cannot export landscape data"
            )
        ...
        try:
            ...
            format_mapping = {"geojson": DataFormat.GEOJSON, "json": DataFormat.GEOJSON}
    ...
    def get_gis_analysis_results(self) -> Dict[str, Any]:
        ...
        if patches:
            ...
            qualities = [
                self.get_patch_current_quality(patch.patch_id) for patch in patches
            ]
            ...
            results["spatial_statistics"] = {
                ...
                "quality_variance": sum(
                    (q - sum(qualities) / len(qualities)) ** 2 for q in qualities
                )
                / len(qualities),
            }
        ...
        if self.gis_manager:
            results["coordinate_system"] = {
                "source_crs": str(self.gis_manager.coordinate_transformer.source_crs),
                "target_crs": str(self.gis_manager.coordinate_transformer.target_crs),
            }
...
class SpatialBeeManager(BaseModel):
    ...
    def register_bee(
        ...
        initial_patch: Optional[int] = None,
    ) -> None:
    ...
    def update_bee_spatial_behavior(
        self, bee_id: int, bee_status: BeeStatus, timestep: int
    ) -> Dict[str, Any]:
    ...
    def _handle_foraging_behavior(
        self, bee_state: SpatialBeeState, timestep: int
    ) -> Dict[str, Any]:
        ...
        if not bee_state.target_patch or not bee_state.planned_path:
            optimal_patches = self.environment_manager.find_optimal_foraging_patches(
                bee_state.current_position, self.max_foraging_distance
            )
            ...
            if optimal_patches:
                ...
                if bee_state.current_patch:
                    path = self.environment_manager.path_finder.find_shortest_path(
                        bee_state.current_patch, target_patch
                    )
                    if path:
        ...
        if bee_state.current_patch == bee_state.target_patch:
            ...
            if bee_state.target_patch:
                quality = self.environment_manager.get_patch_current_quality(
                    bee_state.target_patch
                )
                resources_consumed = self.environment_manager.consume_resources(
                    bee_state.target_patch, 0.1
                )
    ...
    def _handle_scouting_behavior(
        ...
        if best_patch and best_patch != bee_state.current_patch:
            ...
            if bee_state.current_patch:
                path = self.environment_manager.path_finder.find_shortest_path(
                    bee_state.current_patch, best_patch
                )
                if path:
    ...
    def _handle_following_behavior(
        self, bee_state: SpatialBeeState, timestep: int
    ) -> Dict[str, Any]:
    ...
    def _handle_returning_behavior(
        self, bee_state: SpatialBeeState, timestep: int
    ) -> Dict[str, Any]:
    ...
    def get_bee_spatial_metrics(self, bee_id: int) -> Dict[str, Any]:
        ...
        if len(bee_state.movement_history) > 1:
            for i in range(1, len(bee_state.movement_history)):
                total_distance += bee_state.movement_history[i - 1].distance_to(
                    bee_state.movement_history[i]
                )

        # Spatial memory statistics
        known_patches = len(bee_state.spatial_memory)
        avg_remembered_quality = (
            np.mean(list(bee_state.spatial_memory.values()))
            if bee_state.spatial_memory
            else 0
        )
...
def create_spatial_integration_system(
    landscape_config: Dict[str, Any],
) -> Tuple[SpatialEnvironmentManager, SpatialBeeManager]:
    """Factory function to create integrated spatial system"""

    # Create core spatial analysis components
    spatial_index, path_finder, connectivity_analyzer, landscape_analyzer = (
        create_spatial_analysis_system()
    )
