...
try:
    ...
    logging.warning(
        "geopandas/shapely not available - vector processing will be limited"
    )
...
class CoordinateTransformer(BaseModel):
    ...
    def _initialize_transformers(self) -> None:
        ...
        if not PYPROJ_AVAILABLE:
            self.logger.warning(
                "pyproj not available - using simplified transformations"
            )
            return
        ...
        try:
            ...
            self.logger.info(
                f"Initialized coordinate transformer: {self.source_crs} -> {self.target_crs}"
            )
...
class RasterProcessor(BaseModel):
        ...
        try:
            with rasterio.open(file_path) as dataset:
                ...
                for i in range(dataset.count):
                    band_data = data[i]
                    valid_data = (
                        band_data[band_data != dataset.nodata]
                        if dataset.nodata
                        else band_data
                    )
                ...
                self.logger.info(
                    f"Loaded raster: {metadata.width}x{metadata.height}, {metadata.bands} bands"
                )
    ...
    def extract_land_cover_patches(
        self,
        raster_data: np.ndarray,
        metadata: RasterMetadata,
        land_cover_mapping: Dict[int, LandCoverType],
    ) -> List[SpatialPatch]:
        ...
        for value in unique_values:
            ...
            if not RASTERIO_AVAILABLE:
                ...
                patch_area = (
                    float(np.sum(mask)) * (self.default_resolution**2) / 10000
                )  # hectares
                ...
                patch = SpatialPatch(
                    ...
                    resource_density=self._land_cover_to_resource_density(
                        land_cover_type
                    ),
                )
            ...
            try:
                ...
                if metadata.transform.bounds is None:
                    continue

                bounds = metadata.transform.bounds
                ...
                for shape, shape_value in shapes_gen:
                    if shape_value == 1:  # Valid patch
                        ...
                        if shape["type"] == "Polygon":
                            ...
                            tx, ty, tz = self.coordinate_transformer.transform_point(
                            ...
                            patch_area = (
                                self._calculate_polygon_area(coordinates) / 10000
                            )  # hectares
                            ...
                            for coord in coordinates[
                                ...
                                vtx, vty, vtz = (
                                    self.coordinate_transformer.transform_point(
                                        coord[0], coord[1], 0.0
                                    )
                                )
                            ...
                            patch = SpatialPatch(
                                ...
                                vertices=vertices,
                                area=patch_area,
                                perimeter=self._calculate_polygon_perimeter(
                                    coordinates
                                ),
                                quality=self._land_cover_to_quality(land_cover_type),
                                resource_density=self._land_cover_to_resource_density(
                ...
                self.logger.warning(
                    f"Failed to extract shapes for land cover {value}: {e}"
                )
                continue
    ...
    def _calculate_polygon_perimeter(
...
class VectorProcessor(BaseModel):
    ...
    def load_vector_data(
        self, file_path: str, data_format: DataFormat
    ) -> Tuple[List[Dict[str, Any]], VectorMetadata]:
            ...
            for feature in features:
                ...
                if geometry:
                    ...
                    processed_features.append(
                        {"geometry": transformed_geometry, "properties": properties}
                    )
    ...
    def _load_shapefile(
        ...
        try:
            ...
            metadata = VectorMetadata(
                geometry_type=str(gdf.geom_type.iloc[0])
                if not gdf.empty
                else "Unknown",
    ...
    def _load_geopackage(
        ...
        try:
            ...
            self.logger.info(
                f"Loaded GeoPackage layer '{layer_name}': {metadata.feature_count} features"
            )
            return features, metadata
    ...
    def _extract_coordinates_from_geometry(
    ...
    def convert_to_spatial_patches(
        self,
        ...
        for i, feature in enumerate(features):
                ...
                if coordinates and len(coordinates) > 0:
                    ...
                    area = (
                        self._calculate_polygon_area(
                            [(coord[0], coord[1]) for coord in exterior_ring]
                        )
                        / 10000
                    )  # hectares
                    perimeter = self._calculate_polygon_perimeter(
            ...
            if resource_field and resource_field in properties:
                try:
                    ...
                    resource_density = max(
                        0.0, min(1.0, resource_density)
                    )  # Clamp to [0,1]
                except (ValueError, TypeError):
                    pass
...
class GISIntegrationManager(BaseModel):
    ...
    def __init__(self, **data: Any) -> None:
        ...
        if "raster_processor" not in data:
            data["raster_processor"] = RasterProcessor(
                coordinate_transformer=data["coordinate_transformer"]
            )

        if "vector_processor" not in data:
            data["vector_processor"] = VectorProcessor(
                coordinate_transformer=data["coordinate_transformer"]
            )

        super().__init__(**data)

    def load_spatial_data_from_config(
    ...
    def _process_vector_source(self, config: Dict[str, Any]) -> List[SpatialPatch]:
        ...
        # Load vector data
        features, metadata = self.vector_processor.load_vector_data(
    ...
    def export_spatial_data(
        ...
        output_path: str,
        output_format: DataFormat = DataFormat.GEOJSON,
    ) -> str:
    ...
    def _export_to_geojson(self, spatial_index: SpatialIndex, output_path: str) -> str:
        ...
        geojson = {
            ...
            "crs": {
                "type": "name",
                "properties": {"name": self.coordinate_transformer.source_crs},
            },
            "features": features,
        }
...
def create_gis_integration_system(
    source_crs: str = CoordinateSystem.WGS84.value,
    target_crs: str = CoordinateSystem.LOCAL.value,
) -> GISIntegrationManager:
