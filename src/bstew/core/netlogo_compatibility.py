...
class NetLogoCSVReader(BaseModel):
    ...
    def _initialize_default_schemas(self) -> None:
        ...
        # Agent (bee) schema
        self.agent_schema = {
            "who": ColumnSchema("who", NetLogoDataType.NUMBER, True, description="Agent ID"),
            "breed": ColumnSchema("breed", NetLogoDataType.STRING, True, description="Agent breed"),
            "xcor": ColumnSchema("xcor", NetLogoDataType.NUMBER, True, description="X coordinate"),
            "ycor": ColumnSchema("ycor", NetLogoDataType.NUMBER, True, description="Y coordinate"),
            "heading": ColumnSchema("heading", NetLogoDataType.NUMBER, True, valid_range=(0, 360), description="Agent heading"),
            "color": ColumnSchema("color", NetLogoDataType.NUMBER, False, default_value=15, description="Agent color"),
            "size": ColumnSchema("size", NetLogoDataType.NUMBER, False, default_value=1.0, valid_range=(0, 10), description="Agent size"),
            "energy": ColumnSchema("energy", NetLogoDataType.NUMBER, False, default_value=100.0, valid_range=(0, 1000), description="Agent energy"),
            "age": ColumnSchema("age", NetLogoDataType.NUMBER, False, default_value=0, valid_range=(0, 365), description="Agent age"),
            "status": ColumnSchema("status", NetLogoDataType.STRING, False, 
                                 valid_values=["foraging", "resting", "nursing", "guarding", "building", "dead"],
                                 description="Agent status")
        }
        
        # Patch schema
        self.patch_schema = {
            "pxcor": ColumnSchema("pxcor", NetLogoDataType.NUMBER, True, description="Patch X coordinate"),
            "pycor": ColumnSchema("pycor", NetLogoDataType.NUMBER, True, description="Patch Y coordinate"),
            "pcolor": ColumnSchema("pcolor", NetLogoDataType.NUMBER, False, default_value=0, description="Patch color"),
            "flower-density": ColumnSchema("flower-density", NetLogoDataType.NUMBER, False, default_value=0.0, 
                                         valid_range=(0, 100), description="Flower density"),
            "nectar-amount": ColumnSchema("nectar-amount", NetLogoDataType.NUMBER, False, default_value=0.0,
                                        valid_range=(0, 1000), description="Nectar amount"),
            "pollen-amount": ColumnSchema("pollen-amount", NetLogoDataType.NUMBER, False, default_value=0.0,
                                        valid_range=(0, 1000), description="Pollen amount"),
            "resource-quality": ColumnSchema("resource-quality", NetLogoDataType.NUMBER, False, default_value=0.5,
                                           valid_range=(0, 1), description="Resource quality")
        }
        
        # Global variables schema
        self.global_schema = {
            "ticks": ColumnSchema("ticks", NetLogoDataType.NUMBER, True, description="Simulation time"),
            "total-bees": ColumnSchema("total-bees", NetLogoDataType.NUMBER, False, description="Total bee count"),
            "total-energy": ColumnSchema("total-energy", NetLogoDataType.NUMBER, False, description="Total system energy"),
            "temperature": ColumnSchema("temperature", NetLogoDataType.NUMBER, False, valid_range=(-20, 50), description="Temperature"),
            "season": ColumnSchema("season", NetLogoDataType.STRING, False, 
                                 valid_values=["spring", "summer", "autumn", "winter"], description="Current season")
        }
    
    def read_csv(self, file_path: str, csv_type: str = "agents") -> Tuple[pd.DataFrame, ValidationResult]:
        ...
        try:
            ...
            if validation_result.is_valid or self.validation_level == CSVValidationLevel.LENIENT:
                # Apply data conversions and fixes
                df = self._apply_data_conversions(df, csv_type, validation_result)
                
                self.logger.info(f"Successfully read CSV: {validation_result.row_count} rows, {validation_result.column_count} columns")
                
                return df, validation_result
            else:
                self.logger.error(f"CSV validation failed: {len(validation_result.errors)} errors")
    ...
    def _validate_required_columns(self, df: pd.DataFrame, schema: Dict[str, ColumnSchema], 
    ...
    def _validate_data_types(self, df: pd.DataFrame, schema: Dict[str, ColumnSchema],
                           result: ValidationResult) -> None:
        ...
        for col_name, col_schema in schema.items():
            ...
            if expected_type == NetLogoDataType.NUMBER:
                non_numeric = column.apply(lambda x: not self._is_numeric(x) if pd.notna(x) else False)
                if non_numeric.any():
                    invalid_count = non_numeric.sum()
                    result.errors.append(f"Column '{col_name}' contains {invalid_count} non-numeric values")
            
            elif expected_type == NetLogoDataType.BOOLEAN:
                non_boolean = column.apply(lambda x: not self._is_boolean(x) if pd.notna(x) else False)
                if non_boolean.any():
                    invalid_count = non_boolean.sum()
                    result.errors.append(f"Column '{col_name}' contains {invalid_count} non-boolean values")
            
            elif expected_type == NetLogoDataType.STRING:
                # Strings are generally flexible, just check for extreme values
                if column.dtype == 'object':
                    very_long_strings = column.apply(lambda x: len(str(x)) > 1000 if pd.notna(x) else False)
                    if very_long_strings.any():
                        result.warnings.append(f"Column '{col_name}' contains very long string values")
    
    def _validate_value_constraints(self, df: pd.DataFrame, schema: Dict[str, ColumnSchema],
                                  result: ValidationResult) -> None:
        """Validate value ranges and constraints"""
        ...
        for col_name, col_schema in schema.items():
            ...
            # Check valid range
            if col_schema.valid_range and col_schema.data_type == NetLogoDataType.NUMBER:
                ...
                if out_of_range.any():
                    ...
                    result.warnings.append(f"Column '{col_name}' has {count} values outside range [{min_val}, {max_val}]")
            
            # Check valid values
            if col_schema.valid_values:
                invalid_values = ~column.isin(col_schema.valid_values + [col_schema.default_value])
                invalid_values = invalid_values & column.notna()
                
                if invalid_values.any():
                    count = invalid_values.sum()
                    unique_invalid = column[invalid_values].unique()[:5]  # Show first 5 invalid values
                    result.warnings.append(f"Column '{col_name}' has {count} invalid values. Examples: {list(unique_invalid)}")
    
    def _validate_netlogo_conventions(self, df: pd.DataFrame, result: ValidationResult) -> None:
        ...
        for col in df.columns:
            ...
            if not re.match(r'^[a-z][a-z0-9\-]*$', col):
                result.warnings.append(f"Column '{col}' doesn't follow NetLogo naming convention (lowercase with hyphens)")
            
            # Check for reserved NetLogo words
            reserved_words = ['breed', 'color', 'heading', 'hidden?', 'label', 'label-color', 
        ...
        if 'who' in df.columns:
            ...
            if who_column.duplicated().any():
                ...
                result.errors.append(f"'who' column contains {duplicate_count} duplicate agent IDs")
        ...
        if 'xcor' in df.columns and 'ycor' in df.columns:
            ...
            if xcor.isna().any() or ycor.isna().any():
                result.errors.append("Coordinate columns (xcor, ycor) contain non-numeric values")
    
    def _validate_data_consistency(self, df: pd.DataFrame, csv_type: str, result: ValidationResult) -> None:
        ...
        if csv_type.lower() in ['agents', 'turtles', 'bees']:
            ...
            if 'energy' in df.columns:
                ...
                if negative_energy.any():
                    ...
                    result.warnings.append(f"{count} agents have negative energy values")
            ...
            if 'age' in df.columns:
                ...
                if unrealistic_age.any():
                    ...
                    result.warnings.append(f"{count} agents have unrealistic age values (>365)")
            
            # Status consistency
            if 'status' in df.columns and 'energy' in df.columns:
                dead_agents = df['status'] == 'dead'
                dead_with_energy = dead_agents & (pd.to_numeric(df['energy'], errors='coerce') > 0)
    ...
    def _apply_data_conversions(self, df: pd.DataFrame, csv_type: str, 
        ...
        for col_name, col_schema in schema.items():
            if col_name not in df_converted.columns:
                ...
                if col_schema.default_value is not None:
                    ...
                    self.logger.info(f"Added missing column '{col_name}' with default value: {col_schema.default_value}")
                continue
            ...
            if col_schema.data_type == NetLogoDataType.NUMBER:
                ...
                df_converted[col_name] = df_converted[col_name].apply(self._convert_to_boolean)
            elif col_schema.data_type == NetLogoDataType.STRING:
                df_converted[col_name] = df_converted[col_name].astype(str)
        
        return df_converted
    
    def _generate_suggested_fixes(self, result: ValidationResult, schema: Dict[str, ColumnSchema]) -> List[str]:
        ...
        for error in result.errors:
            if "Required column missing" in error:
                ...
                if col_name in schema:
                    ...
                    if col_schema.default_value is not None:
                        fixes.append(f"Add column '{col_name}' with default value: {col_schema.default_value}")
                    else:
                        fixes.append(f"Add required column '{col_name}' ({col_schema.description})")
                ...
                if "'" in error:
                    try:
                        col_name = error.split("'")[1]
                        fixes.append(f"Convert non-numeric values in column '{col_name}' to numbers or remove invalid rows")
                    except IndexError:
                        fixes.append("Convert non-numeric values to numbers or remove invalid rows")
                else:
                    fixes.append("Convert non-numeric values to numbers or remove invalid rows")
            
            elif "duplicate agent IDs" in error:
                fixes.append("Remove duplicate agent IDs from 'who' column or reassign unique IDs")
            
            elif "non-numeric values" in error and "coordinate" in error:
                fixes.append("Ensure xcor and ycor columns contain only numeric coordinate values")
...
class NetLogoCSVWriter(BaseModel):
    ...
    def write_csv(self, df: pd.DataFrame, file_path: str, csv_type: str = "agents") -> ValidationResult:
        ...
        try:
            ...
            if validation_result.is_valid or reader.validation_level == CSVValidationLevel.LENIENT:
                # Write to CSV
                df_formatted.to_csv(
                    file_path,
                    index=False,
                ...
                self.logger.info(f"Successfully wrote CSV: {len(df_formatted)} rows, {len(df_formatted.columns)} columns")
                
                return validation_result
            else:
                self.logger.error(f"Cannot write invalid CSV data: {len(validation_result.errors)} errors")
                return validation_result
    ...
    def _format_for_netlogo(self, df: pd.DataFrame, csv_type: str) -> pd.DataFrame:
        ...
        for col in bool_columns:
            if self.boolean_format == "1/0":
                ...
                df_formatted[col] = df_formatted[col].map({True: "true", False: "false"})
    ...
    def _reorder_columns_for_netlogo(self, df: pd.DataFrame, csv_type: str) -> pd.DataFrame:
        """Reorder columns to match NetLogo conventions"""
        
        if csv_type.lower() in ['agents', 'turtles', 'bees']:
            # Standard NetLogo agent column order
            standard_order = ['who', 'breed', 'color', 'heading', 'xcor', 'ycor', 'shape', 'label', 'label-color', 
                            'size', 'pen-size', 'pen-mode', 'hidden?']
            
            # Add custom columns
            custom_columns = [col for col in df.columns if col not in standard_order]
            desired_order = [col for col in standard_order if col in df.columns] + sorted(custom_columns)
...
class NetLogoDataConverter(BaseModel):
    ...
    def convert_bstew_to_netlogo(self, data: Dict[str, Any], data_type: str = "agent") -> Dict[str, Any]:
    ...
    def convert_netlogo_to_bstew(self, data: Dict[str, Any], data_type: str = "agent") -> Dict[str, Any]:
    ...
    def _convert_agent_to_netlogo(self, agent_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(pos, (list, tuple)) and len(pos) >= 2:
            ...
            netlogo_data['xcor'] = (x * self.coordinate_scale) + self.coordinate_offset[0]
            netlogo_data['ycor'] = (y * self.coordinate_scale) + self.coordinate_offset[1]
        else:
            netlogo_data['xcor'] = 0
            netlogo_data['ycor'] = 0
    ...
    def _convert_global_to_netlogo(self, global_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Time
        netlogo_data['ticks'] = global_data.get('current_step', global_data.get('current_day', 0))
        ...
        # System metrics
        netlogo_data['foraging-efficiency'] = global_data.get('foraging_efficiency', 1.0)
        netlogo_data['resource-scarcity'] = global_data.get('resource_scarcity', 0.0)
        
        return netlogo_data
    
    def _convert_agent_from_netlogo(self, netlogo_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _convert_patch_from_netlogo(self, netlogo_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _convert_global_from_netlogo(self, netlogo_data: Dict[str, Any]) -> Dict[str, Any]:
...
def create_netlogo_export_package(model_data: Dict[str, Any], output_directory: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if "globals" in model_data:
            netlogo_globals = converter.convert_bstew_to_netlogo(model_data["globals"], "global")
        ...
        metadata = {
            ...
            "files_included": [os.path.basename(f) for f in export_results["files_created"]],
            "validation_summary": {
        ...
        logger.info(f"Export package created: {len(export_results['files_created'])} files")
