...
class TripResults(TypedDict):
    """Type definition for foraging trip results"""

    flowers_visited: int
    successful_collections: int
    total_nectar_collected: float
    total_pollen_collected: float
...
class ResourceExtractionModel(BaseModel):
    ...
    optimal_depth_ratio: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Optimal proboscis:corolla depth ratio"
    )
    compatibility_threshold: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Minimum compatibility threshold"
    )

    # Extraction efficiency parameters
    base_extraction_rate: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Base extraction rate"
    )
    experience_factor: float = Field(
        default=0.2, ge=0.0, le=1.0, description="Experience improvement factor"
    )
    energy_efficiency_factor: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Energy level impact factor"
    )

    # Time-based parameters
    base_handling_time: float = Field(
        default=3.0, ge=0.0, description="Base handling time (seconds)"
    )
    complexity_multiplier: float = Field(
        default=1.5, ge=1.0, description="Complexity handling multiplier"
    )

    def calculate_proboscis_compatibility(
        self, bee_proboscis_length: float, corolla_depth: float, corolla_diameter: float
    ) -> float:
        ...
        diameter_score = min(
            1.0, corolla_diameter / 2.0
        )  # Assume min 2mm diameter needed
    ...
    def calculate_extraction_efficiency(
        ...
        # Energy level impact
        energy_factor = (
            bee_physiology.energy_level / 100.0
        ) * self.energy_efficiency_factor
    ...
    def calculate_handling_time(
        self, flower: FlowerCharacteristics, compatibility: float, efficiency: float
    ) -> float:
...
class CropVolumeManager(BaseModel):
    ...
    # Capacity parameters
    max_crop_capacity: float = Field(
        default=70.0, ge=0.0, description="Maximum crop capacity (μL)"
    )
    optimal_load_factor: float = Field(
        default=0.85, ge=0.0, le=1.0, description="Optimal load factor"
    )

    # Volume management
    nectar_concentration_factor: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Nectar concentration factor"
    )
    water_dilution_factor: float = Field(
        default=1.2, ge=1.0, description="Water dilution factor"
    )

    # Physiological constraints
    flight_efficiency_threshold: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Flight efficiency threshold"
    )
    energy_cost_multiplier: float = Field(
        default=1.5, ge=1.0, description="Energy cost multiplier when full"
    )

    def calculate_available_capacity(
        self, current_volume: float, resource_type: ResourceType
    ) -> float:
    ...
    def calculate_collection_limit(
        self,
    ...
    def calculate_flight_efficiency(self, current_volume: float) -> float:
        ...
        if load_factor <= self.optimal_load_factor:
            ...
            overload_factor = (load_factor - self.optimal_load_factor) / (
                1.0 - self.optimal_load_factor
            )
...
class ResourceDepletionModel(BaseModel):
    ...
    base_depletion_rate: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Base depletion rate per collection"
    )
    visitor_impact_factor: float = Field(
        default=0.05, ge=0.0, description="Additional depletion per visitor"
    )

    # Regeneration parameters
    base_regeneration_rate: float = Field(
        default=0.02, ge=0.0, description="Base regeneration rate per time step"
    )
    environmental_factor: float = Field(
        default=1.0, ge=0.0, description="Environmental regeneration factor"
    )

    # Resource-specific parameters
    nectar_regeneration_time: float = Field(
        default=30.0, ge=0.0, description="Nectar regeneration time (minutes)"
    )
    pollen_regeneration_time: float = Field(
        default=240.0, ge=0.0, description="Pollen regeneration time (minutes)"
    )

    def calculate_depletion_amount(
        self,
        resource_type: ResourceType,
    ...
    def calculate_regeneration_amount(
        self,
        resource_type: ResourceType,
        current_amount: float,
        max_amount: float,
...
class DetailedResourceCollector(BaseModel):
    ...
    # Component systems
    extraction_model: ResourceExtractionModel = Field(
        default_factory=ResourceExtractionModel
    )
    crop_manager: CropVolumeManager = Field(default_factory=CropVolumeManager)
    depletion_model: ResourceDepletionModel = Field(
        default_factory=ResourceDepletionModel
    )

    # Collection parameters
    max_collection_attempts: int = Field(
        default=10, ge=1, description="Maximum collection attempts per patch"
    )
    success_threshold: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Minimum efficiency for success"
    )

    # Performance tracking
    collection_history: List[Dict[str, Any]] = Field(
        default_factory=list, description="Collection history"
    )
    ...
    def attempt_resource_collection(
        ...
        experience_level: float,
        environmental_conditions: Dict[str, float],
    ) -> Tuple[CollectionResult, float, Dict[str, Any]]:
        ...
        if efficiency < self.success_threshold:
            return (
                ...
                0.0,
                {"compatibility": compatibility, "efficiency": efficiency},
            )
        ...
        if resource_type == ResourceType.NECTAR:
            ...
            collection_limit = min(
                available_amount,
                bee_physiology.pollen_basket_capacity
                - bee_physiology.current_pollen_load,
            )
        else:
        ...
        if collection_limit <= 0:
            return (
                CollectionResult.CAPACITY_FULL,
                0.0,
                {"compatibility": compatibility, "efficiency": efficiency},
            )
    ...
    def update_bee_physiology(
        ...
        if collection_result in [
            CollectionResult.SUCCESS,
            CollectionResult.PARTIAL_SUCCESS,
        ]:
            ...
            bee_physiology.energy_level = max(
                0, bee_physiology.energy_level - energy_cost
            )

        # Update collection efficiency based on success/failure
        if collection_result == CollectionResult.SUCCESS:
            bee_physiology.collection_efficiency = min(
                1.0, bee_physiology.collection_efficiency + 0.01
            )
        elif collection_result == CollectionResult.FAILURE:
            bee_physiology.collection_efficiency = max(
                0.1, bee_physiology.collection_efficiency - 0.005
            )

    def update_flower_resources(
        self,
        flower: FlowerCharacteristics,
        collection_amount: float,
        resource_type: ResourceType,
        ...
        if time_since_last_visit > 0:
            self._regenerate_flower_resources(
                flower, time_since_last_visit, environmental_conditions
            )

    def _regenerate_flower_resources(
        self,
        flower: FlowerCharacteristics,
        time_passed: float,
        environmental_conditions: Dict[str, float],
        ...
        flower.pollen_availability = min(
            max_pollen, flower.pollen_availability + pollen_regeneration
        )
    ...
    def get_collection_efficiency_stats(self) -> Dict[str, float]:
        ...
        compatibilities = [
            record["compatibility"] for record in self.collection_history
        ]
        collection_amounts = [
            record["collection_amount"] for record in self.collection_history
        ]
    ...
    def simulate_foraging_trip(
        ...
        for flower in flowers:
            ...
            if (
                bee_physiology.current_crop_volume
                >= self.crop_manager.max_crop_capacity * 0.9
                and bee_physiology.current_pollen_load
                >= bee_physiology.pollen_basket_capacity * 0.9
            ):
                break  # Trip complete
            ...
            if (
                ...
                if result in [
                    ...
                    self.update_bee_physiology(
                        bee_physiology,
                        result,
                        amount,
                        ResourceType.NECTAR,
                        details["handling_time"],
                    )
        ...
        if trip_results["flowers_visited"] > 0:
            trip_results["trip_efficiency"] = (
                trip_results["successful_collections"] / trip_results["flowers_visited"]
            )
