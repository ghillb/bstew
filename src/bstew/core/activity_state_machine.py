...
class StateTransitionRule(BaseModel):
    ...
    conditions: Dict[str, Any] = Field(
        default_factory=dict, description="Transition conditions"
    )
    min_duration: int = Field(
        default=0, ge=0, description="Minimum steps in current state"
    )
    max_duration: Optional[int] = Field(
        default=None, ge=0, description="Maximum steps in current state"
    )
...
class ActivityStateConfig(BaseModel):
    ...
    energy_consumption: float = Field(
        default=1.0, ge=0.0, description="Energy consumption per step"
    )
    duration_range: Tuple[int, int] = Field(
        default=(1, 10), description="Duration range in steps"
    )
    required_conditions: Dict[str, Any] = Field(
        default_factory=dict, description="Required conditions"
    )
    energy_cost_multiplier: float = Field(
        default=1.0, ge=0.0, description="Energy cost multiplier"
    )
    success_probability: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Activity success probability"
    )

    # Physiological parameters
    flight_speed: float = Field(default=5.0, ge=0.0, description="Flight speed in m/s")
    metabolic_rate: float = Field(
        default=1.0, ge=0.0, description="Metabolic rate multiplier"
    )
    stress_level: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Stress level (0-1)"
    )
    thermoregulation_cost: float = Field(
        default=0.0, ge=0.0, description="Thermoregulation energy cost"
    )
...
class PersonalTimeTracker(BaseModel):
    ...
    current_state: BeeStatus = Field(
        default=BeeStatus.ALIVE, description="Current activity state"
    )
    state_start_time: int = Field(
        default=0, ge=0, description="Time when current state began"
    )
    state_duration: int = Field(
        default=0, ge=0, description="Duration in current state"
    )
    daily_activities: Dict[str, int] = Field(
        default_factory=dict, description="Daily activity tracking"
    )
    ...
    last_activity_change: int = Field(
        default=0, ge=0, description="Last activity change time"
    )

    # Enhanced context-awareness fields
    state_transition_history: List[Dict[str, Any]] = Field(
        default_factory=list, description="Recent state transitions"
    )
    success_history: List[bool] = Field(
        default_factory=list, description="Recent success/failure history"
    )
    energy_history: List[float] = Field(
        default_factory=list, description="Recent energy levels"
    )
    environmental_context: Dict[str, Any] = Field(
        default_factory=dict, description="Environmental context memory"
    )
    behavioral_preferences: Dict[str, float] = Field(
        default_factory=dict, description="Learned behavioral preferences"
    )
    adaptation_factors: Dict[str, float] = Field(
        default_factory=dict, description="Adaptation factors for decision making"
    )
    ...
    def update_time(self, current_step: int) -> None:
        ...
        if self.current_state in [
            BeeStatus.NECTAR_FORAGING,
            BeeStatus.POLLEN_FORAGING,
            BeeStatus.COLLECT_NECTAR,
            BeeStatus.COLLECT_POLLEN,
        ]:
    ...
    def get_preferred_state_sequence(self) -> List[str]:
        ...
        for i in range(len(self.state_transition_history) - 1):
            seq = (
                self.state_transition_history[i]["to_state"],
    ...
    def adapt_behavior_preferences(
        self, state: BeeStatus, success: bool, environmental_factors: Dict[str, Any]
    ) -> None:
        ...
        self.behavioral_preferences[state_key] = max(
            0.0, min(1.0, self.behavioral_preferences[state_key])
        )
        ...
        self.adaptation_factors[env_key] = max(
            0.5, min(1.5, self.adaptation_factors[env_key])
        )

    def get_contextual_preference(
        self, state: BeeStatus, current_conditions: Dict[str, Any]
    ) -> float:
...
class ActivityStateMachine(BaseModel):
    ...
    # State categories
    state_categories: Dict[BeeStatus, ActivityStateCategory] = Field(
    ...
    def _initialize_default_configurations(self) -> None:
        ...
        self.state_configs[BeeStatus.NEST_CONSTRUCTION] = ActivityStateConfig(
            energy_consumption=3.0, duration_range=(5, 20), energy_cost_multiplier=1.5
        )
        ...
        self.state_configs[BeeStatus.POLLEN_FORAGING] = ActivityStateConfig(
            energy_consumption=3.5, duration_range=(10, 30), energy_cost_multiplier=1.4
        )

        self.state_configs[BeeStatus.COLLECT_NECTAR] = ActivityStateConfig(
            energy_consumption=2.0, duration_range=(1, 3), energy_cost_multiplier=1.1
        )

        self.state_configs[BeeStatus.COLLECT_POLLEN] = ActivityStateConfig(
            energy_consumption=2.5, duration_range=(1, 3), energy_cost_multiplier=1.2
        )

        self.state_configs[BeeStatus.BRINGING_NECTAR] = ActivityStateConfig(
            energy_consumption=2.0, duration_range=(5, 15), energy_cost_multiplier=1.1
        )
        ...
        self.state_configs[BeeStatus.RETURNING_EMPTY] = ActivityStateConfig(
            energy_consumption=1.5, duration_range=(3, 10), energy_cost_multiplier=1.0
        )

        self.state_configs[BeeStatus.RETURNING_UNHAPPY_NECTAR] = ActivityStateConfig(
            energy_consumption=1.8, duration_range=(3, 10), energy_cost_multiplier=1.0
        )
        ...
        self.state_configs[BeeStatus.EXPERIMENTAL_FORAGING_NECTAR] = (
            ActivityStateConfig(
        ...
        self.state_configs[BeeStatus.EXPERIMENTAL_FORAGING_POLLEN] = (
            ActivityStateConfig(
                energy_consumption=4.0,
                duration_range=(20, 60),
                energy_cost_multiplier=1.6,
        ...
        self.state_configs[BeeStatus.EGG_LAYING] = ActivityStateConfig(
            energy_consumption=2.0, duration_range=(1, 5), energy_cost_multiplier=1.2
        )

        # Maintenance states
        self.state_configs[BeeStatus.NURSING] = ActivityStateConfig(
            energy_consumption=1.5, duration_range=(5, 20), energy_cost_multiplier=1.1
        )

        # Communication states
        self.state_configs[BeeStatus.DANCING] = ActivityStateConfig(
            energy_consumption=2.0, duration_range=(2, 8), energy_cost_multiplier=1.2
        )

        # General states
        self.state_configs[BeeStatus.RESTING] = ActivityStateConfig(
            energy_consumption=0.5, duration_range=(1, 10), energy_cost_multiplier=0.5
        )

        self.state_configs[BeeStatus.FORAGING] = ActivityStateConfig(
            energy_consumption=3.0, duration_range=(10, 30), energy_cost_multiplier=1.3
        )
    ...
    def transition_bee_state(
        self, bee_id: int, new_state: BeeStatus, current_step: int
    ) -> None:
    ...
    def should_transition_state(
        self, bee_id: int, current_conditions: Dict[str, Any]
    ) -> Optional[BeeStatus]:
        ...
        forced_transition = self._check_forced_transitions(
            current_state, current_conditions, tracker
        )
        if forced_transition:
            return forced_transition

        # 2. Check for environmental transitions
        environmental_transition = self._check_environmental_transitions(
            current_state, current_conditions, tracker
        )
        ...
        energy_transition = self._check_energy_based_transitions(
            current_state, current_conditions, tracker
        )
        ...
        role_transition = self._check_role_specific_transitions(
            current_state, current_conditions, tracker
        )
        if role_transition:
        ...
        time_transition = self._check_time_based_transitions(
            current_state, current_conditions, tracker
        )
        if time_transition:
        ...
        rule_transition = self._check_rule_based_transitions(
            current_state, current_conditions, tracker
        )
        if rule_transition:
        ...
        adaptive_transition = self._check_adaptive_transitions(
            current_state, current_conditions, tracker
        )
    ...
    def _check_forced_transitions(
        self,
        current_state: BeeStatus,
        ...
        if current_state in self.state_configs:
            ...
            max_duration = (
                duration_range[1] * 2
            )  # Allow 2x normal duration before forced transition
        ...
        if role == "queen" and current_state in [
            BeeStatus.NECTAR_FORAGING,
            BeeStatus.POLLEN_FORAGING,
        ]:
            return BeeStatus.EGG_LAYING
        elif role == "drone" and current_state in [
            BeeStatus.NURSING,
            BeeStatus.NEST_CONSTRUCTION,
        ]:
            return BeeStatus.RESTING

        return None

    def _check_environmental_transitions(
        self,
        current_state: BeeStatus,
        conditions: Dict[str, Any],
        tracker: PersonalTimeTracker,
        ...
        if (
            weather in ["storm", "heavy_rain"]
            and current_state in self.get_foraging_states()
        ):
            return BeeStatus.RETURNING_EMPTY
        ...
        if isinstance(time_of_day, int):
            if (
                time_of_day < 6 or time_of_day > 20
            ) and current_state in self.get_foraging_states():
    ...
    def _check_energy_based_transitions(
        self,
        current_state: BeeStatus,
        conditions: Dict[str, Any],
        tracker: PersonalTimeTracker,
        ...
        if energy_level == "low" or (
            isinstance(energy_value, (int, float)) and energy_value < 30
        ):
        ...
        if energy_level == "critical" or (
            isinstance(energy_value, (int, float)) and energy_value < 10
        ):
            if current_state != BeeStatus.RESTING:
                return BeeStatus.RESTING

        # High energy transitions
        if energy_level == "high" or (
            isinstance(energy_value, (int, float)) and energy_value > 80
        ):
            if current_state == BeeStatus.RESTING:
    ...
    def _check_role_specific_transitions(
        self,
        current_state: BeeStatus,
        conditions: Dict[str, Any],
        tracker: PersonalTimeTracker,
        ...
        if role == "queen":
            if current_state == BeeStatus.RESTING and conditions.get(
                "colony_needs_eggs", True
            ):
                return BeeStatus.EGG_LAYING
        ...
        # Nurse-specific transitions
            ...
            if current_state == BeeStatus.RESTING and conditions.get(
                ...
                if conditions.get("brood_needs_care", False):
                    return BeeStatus.NURSING
            elif current_state == BeeStatus.NURSING and not conditions.get(
                "brood_needs_care", True
            ):
                # No more brood to care for
                if age > 15 and conditions.get("energy", "sufficient") in [
                    "sufficient",
                    "high",
                ]:
        ...
        # Builder-specific transitions
            ...
            if current_state == BeeStatus.RESTING and conditions.get(
                ...
                return BeeStatus.NEST_CONSTRUCTION
            elif current_state == BeeStatus.NEST_CONSTRUCTION and not conditions.get(
    ...
    def _check_time_based_transitions(
        ...
        if current_state in self.state_configs:
            ...
            if tracker.state_duration >= max_duration:
                ...
                if random.random() < transition_probability:
                    return self._get_natural_transition_target(
        ...
        if isinstance(time_of_day, int):
            ...
            if 6 <= time_of_day <= 8 and current_state == BeeStatus.RESTING:
                role = conditions.get("role", "forager")
                if role in ["forager", "worker"] and conditions.get(
                    "energy", "sufficient"
                ) in ["sufficient", "high"]:
                    return BeeStatus.SEARCHING
            ...
            # Evening wind-down
                ...
                18 <= time_of_day <= 20 and current_state in self.get_foraging_states()
            ):
                if current_state not in [
                    BeeStatus.BRINGING_NECTAR,
                    BeeStatus.BRINGING_POLLEN,
                ]:
                    return BeeStatus.RETURNING_EMPTY

        return None

    def _check_rule_based_transitions(
        self,
        current_state: BeeStatus,
        conditions: Dict[str, Any],
        tracker: PersonalTimeTracker,
        ...
        applicable_rules = [
            rule for rule in self.transition_rules if rule.from_state == current_state
        ]
        ...
        for rule in applicable_rules:
            ...
            if (
                rule.max_duration is not None
                and tracker.state_duration > rule.max_duration
            ):
            ...
            if self._check_transition_conditions(rule.conditions, conditions):
                ...
                adjusted_probability = self._adjust_transition_probability(
                    rule.probability, conditions, tracker
                )
    ...
    def _check_adaptive_transitions(
        self,
        current_state: BeeStatus,
        conditions: Dict[str, Any],
        tracker: PersonalTimeTracker,
        ...
        if success_rate < 0.3:
            if current_state == BeeStatus.SEARCHING and tracker.state_duration > 3:
                ...
                current_state in [BeeStatus.COLLECT_NECTAR, BeeStatus.COLLECT_POLLEN]
                and tracker.state_duration > 2
            ):
        ...
        if (
            dance_followers > 3
            and current_state == BeeStatus.DANCING
            and tracker.state_duration > 3
        ):
    ...
    def _get_natural_transition_target(
        self, current_state: BeeStatus, conditions: Dict[str, Any]
    ) -> BeeStatus:
        ...
        if current_state == BeeStatus.SEARCHING:
            ...
            if patch_found:
                ...
                return (
                    ...
                    else BeeStatus.POLLEN_FORAGING
                )
            else:
                return BeeStatus.RETURNING_EMPTY
            ...
            if at_patch:
                return (
                    BeeStatus.COLLECT_NECTAR
                    if current_state == BeeStatus.NECTAR_FORAGING
                    else BeeStatus.COLLECT_POLLEN
                )
            else:
                return BeeStatus.SEARCHING
            ...
            if collection_successful:
                ...
                if resource_quality == "high":
                    return (
                        BeeStatus.BRINGING_NECTAR
                        if current_state == BeeStatus.COLLECT_NECTAR
                        else BeeStatus.BRINGING_POLLEN
                    )
                else:
                    return (
                        BeeStatus.RETURNING_UNHAPPY_NECTAR
                        if current_state == BeeStatus.COLLECT_NECTAR
                        else BeeStatus.RETURNING_UNHAPPY_POLLEN
                    )
            else:
                return BeeStatus.SEARCHING
    ...
    def _adjust_transition_probability(
        self,
        ...
        if energy_level == "high":
            ...
            adjusted_probability *= (
                0.7  # Less likely to transition to energy-intensive states
            )
    ...
    def _check_transition_conditions(
        self, required_conditions: Dict[str, Any], current_conditions: Dict[str, Any]
    ) -> bool:
    ...
    def validate_personal_tracker(self, bee_id: int) -> List[str]:
        ...
        if tracker.state_start_time > tracker.last_activity_change:
            errors.append(
                f"Invalid time sequence: start {tracker.state_start_time} > last change {tracker.last_activity_change}"
            )
    ...
    def get_foraging_states(self) -> List[BeeStatus]:
        ...
        return [
            state
            for state, category in self.state_categories.items()
            if category == ActivityStateCategory.FORAGING
        ]
    ...
    def calculate_state_specific_energy_cost(
        self,
        state: BeeStatus,
        base_energy: float,
    ...
    def get_state_behavioral_parameters(self, state: BeeStatus) -> Dict[str, Any]:
        ...
        parameters = {
            ...
            "energy_efficiency": 1.0 / config.energy_cost_multiplier
            if config.energy_cost_multiplier > 0
            else 1.0,
        }
            ...
            parameters.update(
                {
                    ...
                    "attention_span": config.duration_range[1]
    ...
    def predict_next_state(
        self, bee_id: int, current_conditions: Dict[str, Any], lookahead_steps: int = 5
    ) -> List[Tuple[BeeStatus, float]]:
    ...
    def get_state_transition_matrix(self) -> Dict[BeeStatus, Dict[BeeStatus, float]]:
        ...
        matrix: Dict[BeeStatus, Dict[BeeStatus, float]] = {}
    ...
    def analyze_state_patterns(self, bee_id: int) -> Dict[str, Any]:
        ...
        analysis["behavioral_consistency"] = self._calculate_behavioral_consistency(
            tracker
        )
    ...
    def _calculate_transition_confidence(
        self, from_state: BeeStatus, to_state: BeeStatus, conditions: Dict[str, Any]
    ) -> float:
        ...
        for rule in self.transition_rules:
            if rule.from_state == from_state and rule.to_state == to_state:
                ...
                condition_match = self._calculate_condition_match(
        ...
        for key, required_value in required_conditions.items():
            if key in current_conditions:
                ...
                if isinstance(required_value, bool) and current_value == required_value:
                    ...
                    isinstance(required_value, str) and current_value == required_value
                ):
                    matches += 1
                elif isinstance(required_value, (int, float)) and isinstance(
                    current_value, (int, float)
                ):
                    # For numeric values, use tolerance
                    if (
                        abs(current_value - required_value)
                        / max(abs(required_value), 1)
                        < 0.2
                    ):
                        matches += 1
    ...
    def _update_predicted_conditions(
        self, conditions: Dict[str, Any], new_state: BeeStatus, step: int
    ) -> Dict[str, Any]:
    ...
    def _calculate_behavioral_consistency(self, tracker: PersonalTimeTracker) -> float:
        ...
        max_entropy = (
            math.log2(len(tracker.daily_activities)) if tracker.daily_activities else 1
        )
        consistency = 1.0 - (entropy / max_entropy) if max_entropy > 0 else 0.0

        return max(0.0, min(1.0, consistency))

    def get_state_transition_history(
        self, bee_id: int, steps: int = 10
    ) -> List[Dict[str, Any]]:
