...
class ColonyHealthMonitor(BaseModel):
    ...
    # Health thresholds
    critical_population_threshold: int = Field(
        default=50, ge=0, description="Critical population threshold"
    )
    energy_depletion_threshold: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Energy depletion threshold"
    )
    disease_outbreak_threshold: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Disease outbreak threshold"
    )

    # Monitoring parameters
    health_check_interval: float = Field(
        default=24.0, ge=0.0, description="Health check interval (hours)"
    )
    historical_window: int = Field(
        default=30, ge=1, description="Historical data window (days)"
    )

    # Alert thresholds
    declining_threshold: float = Field(
        default=0.6, ge=0.0, le=1.0, description="Declining health threshold"
    )
    critical_threshold: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Critical health threshold"
    )
    ...
    def assess_colony_health(self, colony_data: Dict[str, Any]) -> ColonyHealthMetrics:
        ...
        age_distribution = self._calculate_age_distribution(
            colony_data.get("bee_ages", [])
        )
        # Calculate weighted average age using age group midpoints
        age_group_midpoints = {
            "young": 25,  # 0-50 days midpoint
            "adult": 125,  # 51-200 days midpoint
            "mature": 300,  # 201-400 days midpoint
            "old": 450,  # 400+ days estimate
        }
        average_age = (
            sum(
                age_group_midpoints.get(age, 0) * count
                for age, count in age_distribution.items()
            )
            / sum(age_distribution.values())
            if age_distribution and sum(age_distribution.values()) > 0
            else 0.0
        )
    ...
    def determine_colony_status(
        self, health_metrics: ColonyHealthMetrics
    ) -> ColonyStatus:
        ...
        if (
            health_metrics.total_energy / max(1.0, health_metrics.total_population)
            < 10.0
        ):
            return ColonyStatus.CRITICAL
    ...
    def identify_warning_signs(
        self,
        health_metrics: ColonyHealthMetrics,
        historical_data: List[ColonyHealthMetrics],
    ) -> List[str]:
        ...
        if len(historical_data) >= 7:
            ...
            if all(
                recent_populations[i] <= recent_populations[i - 1]
                for i in range(1, len(recent_populations))
            ):
        ...
        energy_per_bee = health_metrics.total_energy / max(
            1.0, health_metrics.total_population
        )
        ...
        if (
            health_metrics.forager_count / max(1.0, health_metrics.total_population)
            < 0.2
        ):
        ...
        if (
            health_metrics.age_distribution.get("young", 0)
            / max(1.0, health_metrics.total_population)
            < 0.2
        ):
...
class CollapsePredictor(BaseModel):
    ...
    prediction_window: int = Field(
        default=14, ge=1, description="Prediction window (days)"
    )
    minimum_data_points: int = Field(
        default=7, ge=1, description="Minimum data points for prediction"
    )

    # Model weights
    population_weight: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Population trend weight"
    )
    energy_weight: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Energy trend weight"
    )
    health_weight: float = Field(
        default=0.2, ge=0.0, le=1.0, description="Health trend weight"
    )
    foraging_weight: float = Field(
        default=0.15, ge=0.0, le=1.0, description="Foraging trend weight"
    )
    genetic_weight: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Genetic trend weight"
    )

    def predict_collapse_risk(
        self, historical_data: List[ColonyHealthMetrics]
    ) -> Dict[str, Any]:
        ...
        population_trend = self._calculate_trend(
            [m.total_population for m in historical_data]
        )
        energy_trend = self._calculate_trend([m.total_energy for m in historical_data])
        health_trend = self._calculate_trend(
            [m.calculate_health_score() for m in historical_data]
        )
        foraging_trend = self._calculate_trend(
            [m.foraging_efficiency for m in historical_data]
        )
        genetic_trend = self._calculate_trend(
            [m.genetic_diversity for m in historical_data]
        )
        ...
        # Combined risk
        collapse_risk = (
            pop_risk + energy_risk + health_risk + foraging_risk + genetic_risk
        )
    ...
    def _predict_collapse_timeline(
        self, historical_data: List[ColonyHealthMetrics], collapse_risk: float
    ) -> Optional[int]:
...
class DeadColonyTracker(BaseModel):
    ...
    # Tracking data
    colony_health_history: Dict[int, List[ColonyHealthMetrics]] = Field(
        default_factory=dict
    )
    colony_status_history: Dict[int, List[Tuple[float, ColonyStatus]]] = Field(
        default_factory=dict
    )
    collapse_events: List[CollapseEvent] = Field(default_factory=list)

    # Statistics
    total_colonies_tracked: int = Field(
        default=0, description="Total colonies ever tracked"
    )
    active_colonies: Set[int] = Field(
        default_factory=set, description="Currently active colonies"
    )
    ...
    def update_colony_health(
        self, colony_id: int, colony_data: Dict[str, Any], simulation_time: float = 0.0
    ) -> ColonyStatus:
        ...
        # Check for collapse
        if (
            colony_status == ColonyStatus.COLLAPSED
            and colony_id in self.active_colonies
        ):
            self._record_colony_collapse(colony_id, health_metrics)

        return colony_status

    def _record_colony_collapse(
        self, colony_id: int, final_health: ColonyHealthMetrics
    ) -> None:
        ...
        peak_date = next(
            ...
            if m.total_population == peak_population
        )
        days_since_peak = (time.time() - peak_date) / (24 * 3600)

        # Get warning signs
        warning_signs = self.health_monitor.identify_warning_signs(
            final_health, historical_data
        )
    ...
    def _determine_collapse_reason(
        self,
        final_health: ColonyHealthMetrics,
        historical_data: List[ColonyHealthMetrics],
    ) -> CollapseReason:
        ...
        if (
            final_health.temperature_stress > 0.7
            or final_health.resource_availability < 0.3
        ):
    ...
    def _identify_contributing_factors(
        ...
        population_trend = self.collapse_predictor._calculate_trend(
            [m.total_population for m in recent_data]
        )
        if population_trend < -0.1:
            factors.append(CollapseReason.POPULATION_COLLAPSE)

        energy_trend = self.collapse_predictor._calculate_trend(
            [m.total_energy for m in recent_data]
        )
        if energy_trend < -0.1:
            factors.append(CollapseReason.RESOURCE_DEPLETION)

        foraging_trend = self.collapse_predictor._calculate_trend(
            [m.foraging_efficiency for m in recent_data]
        )
        ...
        avg_temp_stress = sum(m.temperature_stress for m in recent_data) / len(
            recent_data
        )
    ...
    def _calculate_collapse_duration(
        self, status_history: List[Tuple[float, ColonyStatus]]
    ) -> float:
    ...
    def get_mortality_statistics(self) -> Dict[str, Any]:
        ...
        survival_rate = (
            self.total_colonies_tracked - total_dead
        ) / self.total_colonies_tracked
        ...
        # Average metrics
        avg_lifespan = sum(
            event.days_since_peak for event in self.collapse_events
        ) / len(self.collapse_events)
        avg_collapse_duration = sum(
            event.collapse_duration for event in self.collapse_events
        ) / len(self.collapse_events)
        avg_peak_population = sum(
            event.peak_population for event in self.collapse_events
        ) / len(self.collapse_events)
        ...
        return {
            ...
            "most_common_collapse_reason": max(
                ...
                if colony_id in self.colony_health_history:
                    ...
                    if history:
                        ...
                        report_data["colony_details"][colony_id] = {
                            "status": "dead"
                            if colony_id in self.dead_colonies
                            else "active",
                            "latest_population": latest.total_population,
                            "latest_health_score": latest.calculate_health_score(),
                            "data_points": len(history),
                            "tracking_duration": (
                                latest.timestamp - history[0].timestamp
                            )
                            / (24 * 3600)
                            if len(history) > 1
                            else 0,
                ...
                report_data["collapse_events"].append(
                    {
                        ...
                        "collapse_time": event.collapse_timestamp,
                        "primary_reason": event.primary_reason.value,
                        "contributing_factors": [
                            f.value for f in event.contributing_factors
                        ],
