...
class ColonyHealthProfile(BaseModel):
    ...
    metric_history: Dict[HealthIndicator, deque] = Field(default_factory=lambda: defaultdict(lambda: deque(maxlen=100)))
    ...
    def critical_indicators(self) -> List[HealthIndicator]:
        ...
        return [
            indicator for indicator, metric in self.current_metrics.items()
    ...
    def declining_indicators(self) -> List[HealthIndicator]:
        ...
        return [
            indicator for indicator, trend in self.health_trends.items()
...
class HealthMonitoringSystem(BaseModel):
    ...
    alert_cooldown_periods: Dict[AlertLevel, float] = Field(default_factory=lambda: {
    ...
    def register_colony(self, colony_id: int, initial_data: Optional[Dict[str, Any]] = None) -> None:
    ...
    def update_colony_health(self, colony_id: int, colony_data: Dict[str, Any], 
                           simulation_time: float) -> ColonyHealthProfile:
        ...
        profile.health_status = self._determine_health_status(profile.overall_health_score)
    ...
    def _establish_baseline_metrics(self, profile: ColonyHealthProfile, initial_data: Dict[str, Any]) -> None:
        """Establish baseline metrics for comparison"""
        
        # Population baseline
        population = initial_data.get('adult_population', initial_data.get('population', 50))
        ...
        # Foraging baseline
        profile.baseline_metrics[HealthIndicator.FORAGING] = 0.8  # Assume 80% efficiency baseline
        
        # Other baselines
        profile.baseline_metrics[HealthIndicator.MORTALITY] = 0.01  # 1% daily mortality baseline
        ...
        if self.logger:
            self.logger.info(f"Established baseline metrics for colony {profile.colony_id}")
    
    def _calculate_health_metrics(self, colony_data: Dict[str, Any], timestamp: float) -> Dict[HealthIndicator, HealthMetric]:
        """Calculate current health metrics from colony data"""
        
        metrics = {}
        
        # Population metrics
        population = colony_data.get('adult_population', colony_data.get('population', 0))
        ...
        # Mortality metrics
        mortality_rate = colony_data.get('mortality_rate', colony_data.get('daily_mortality', 0.0))
        ...
        # Foraging metrics
        foraging_efficiency = colony_data.get('foraging_efficiency', colony_data.get('avg_foraging_efficiency', 0.5))
        ...
        # Disease metrics
        disease_prevalence = colony_data.get('disease_prevalence', colony_data.get('infection_rate', 0.0))
        ...
        reproduction_rate = colony_data.get('reproduction_rate', colony_data.get('egg_laying_rate', 0.0))
    ...
    def _calculate_stress_level(self, colony_data: Dict[str, Any]) -> float:
        ...
        if foraging_efficiency < self.thresholds.min_foraging_efficiency:
            forage_stress = 1.0 - (foraging_efficiency / self.thresholds.min_foraging_efficiency)
    ...
    def _calculate_trend(self, values: List[float], timestamps: List[float]) -> HealthTrend:
    ...
    def _should_emit_alert(self, alert_key: str, level: AlertLevel, timestamp: float) -> bool:
    ...
    def _create_debounced_alert(self, alert_type: str, indicator: HealthIndicator, 
                               level: AlertLevel, colony_id: int, message: str,
                               timestamp: float, current_value: float, 
                               threshold_value: float, recommendation: str = "") -> Optional[HealthAlert]:
    ...
    def _check_health_alerts(self, profile: ColonyHealthProfile, timestamp: float) -> List[HealthAlert]:
        """Check for health alerts based on current metrics and trends"""
        
        alerts = []
        
        # Check if colony is still in startup grace period
        colony_age_days = timestamp / 24.0  # Convert hours to days (assuming timestamp is in hours)
        ...
        for indicator, metric in profile.current_metrics.items():
            ...
            if metric.status == HealthStatus.CRITICAL:
                ...
                alert_level = AlertLevel.WARNING if in_grace_period else AlertLevel.CRITICAL
                ...
                alert = self._create_debounced_alert(
                    ...
                    recommendation=self._get_recommendation_for_indicator(indicator, metric.status)
                )
                if alert:
                    alerts.append(alert)
    ...
    def _get_recommendation_for_indicator(self, indicator: HealthIndicator, status: HealthStatus) -> str:
        ...
        base_recommendation = recommendations.get(indicator, "Monitor closely and consult management protocols")
    ...
    def _process_health_alert(self, alert: HealthAlert) -> None:
        ...
        if alert.level != AlertLevel.EMERGENCY or not self._alert_debounce_config.get('emergency_bypass', True):
            if alert_key in self._alert_debounce_cache:
                last_time, count = self._alert_debounce_cache[alert_key]
                
                # Check minimum interval
                if current_time - last_time < self._alert_debounce_config['min_interval']:
        ...
        self._alert_debounce_cache[alert_key] = (current_time, 
    ...
    def _cleanup_resolved_alerts(self, profile: ColonyHealthProfile) -> None:
        ...
        for alert in profile.active_alerts:
            ...
            if alert.indicator in profile.current_metrics:
                ...
                if (alert.level == AlertLevel.CRITICAL and 
                    current_metric.status not in [HealthStatus.CRITICAL, HealthStatus.POOR]):
                    alert.resolved = True
                    resolved_alerts.append(alert)
                elif (alert.level == AlertLevel.WARNING and
                      current_metric.status in [HealthStatus.GOOD, HealthStatus.EXCELLENT]):
                    alert.resolved = True
                    resolved_alerts.append(alert)
        ...
        if resolved_alerts:
            if self.logger:
                self.logger.info(f"Resolved {len(resolved_alerts)} alerts for colony {profile.colony_id}")
    ...
    def get_system_health_overview(self) -> Dict[str, Any]:
        ...
        # Critical colonies
        critical_colonies = [
            p.colony_id for p in self.colony_profiles.values()
            if p.health_status in [HealthStatus.CRITICAL, HealthStatus.POOR, HealthStatus.COLLAPSED]
        ]
        
        return {
            "system_health_score": system_health_score,
            "total_colonies": total_colonies,
            "healthy_colonies": status_counts.get("good", 0) + status_counts.get("excellent", 0),
            ...
            "monitoring_statistics": {
                ...
                "last_system_update": max(
                    (p.last_update for p in self.colony_profiles.values()), 
                    default=0.0
                )
            }
        }
