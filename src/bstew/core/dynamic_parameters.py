...
class ParameterConstraint:
    ...
    validation_function: Optional[str] = None  # String representation of validation logic
...
class ModificationRule:
    ...
    rule_id: str
    parameter_path: str  # Dot notation path to parameter (e.g., "colony.foraging.efficiency")
...
class DynamicParameterSystem(BaseModel):
    ...
    def add_modification_rule(self, rule: ModificationRule) -> None:
        ...
        self.logger.info(f"Rule {rule.rule_id} added successfully for parameter {rule.parameter_path}")
    ...
    def update_system_state(self, simulation_time: float, model_state: Dict[str, Any]) -> None:
    ...
    def apply_manual_modification(self, parameter_path: str, new_value: Any, 
                                duration: Optional[float] = None) -> Dict[str, Any]:
        """Apply manual parameter modification"""
        
        self.logger.info(f"Applying manual modification to {parameter_path}: {new_value}")
    ...
    def get_parameter_analytics(self, parameter_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        if parameter_path:
            ...
            parameter_history = [h for h in self.modification_history if h.parameter_path == parameter_path]
            ...
            return {
                ...
                "recent_activity": self.modification_history[-20:] if self.modification_history else []
            }
    
    def optimize_parameters_adaptive(self, target_metrics: Dict[str, float], 
                                   learning_rate: float = 0.1) -> Dict[str, Any]:
        """Adaptive parameter optimization based on target metrics"""
        
        self.logger.info(f"Starting adaptive parameter optimization with targets: {target_metrics}")
        ...
        for metric_name, target_value in target_metrics.items():
            ...
            if abs(current_value - target_value) > 0.05:  # 5% tolerance
                ...
                influential_parameters = self._find_parameters_affecting_metric(metric_name)
                ...
                for param_path, influence_score in influential_parameters.items():
                    if influence_score > 0.1:  # Significant influence
                        ...
                        adjustment = self._calculate_parameter_adjustment(
                            param_path, metric_name, current_value, target_value, learning_rate
                        )
                        ...
                        if adjustment["magnitude"] > 0.01:  # Meaningful adjustment
                            ...
                            optimization_results["parameter_adjustments"][param_path] = adjustment
    ...
    def _validate_modification_rule(self, rule: ModificationRule) -> Dict[str, Any]:
        ...
        if rule.trigger == ModificationTrigger.EVENT_BASED and not rule.trigger_event:
            return {"valid": False, "error": "Event-based trigger requires trigger_event"}
        
        if rule.trigger == ModificationTrigger.CONDITION_BASED and not rule.trigger_condition:
            return {"valid": False, "error": "Condition-based trigger requires trigger_condition"}
        
        # Check modification configuration
        if not rule.target_value and not rule.value_function:
            return {"valid": False, "error": "Either target_value or value_function is required"}
        
        # Check constraints
        if rule.constraint:
            constraint_validation = self._validate_parameter_constraint(rule.constraint, rule.target_value)
            if not constraint_validation["valid"]:
                return constraint_validation
        
        return {"valid": True}
    
    def _validate_parameter_constraint(self, constraint: ParameterConstraint, value: Any) -> Dict[str, Any]:
        """Validate parameter value against constraints"""
        
        if constraint.min_value is not None and isinstance(value, (int, float)):
            if value < constraint.min_value:
                return {"valid": False, "error": f"Value {value} below minimum {constraint.min_value}"}
        
        if constraint.max_value is not None and isinstance(value, (int, float)):
            if value > constraint.max_value:
                return {"valid": False, "error": f"Value {value} above maximum {constraint.max_value}"}
        
        if constraint.allowed_values and value not in constraint.allowed_values:
            return {"valid": False, "error": f"Value {value} not in allowed values {constraint.allowed_values}"}
        ...
        for parameter_path, monitor in self.parameter_monitors.items():
            try:
                ...
                if monitor.current_value != current_value:
                    ...
                    monitor.change_frequency = len([h for h in monitor.change_history 
                                                   if h.timestamp > self.current_simulation_time - 100.0])
                
                monitor.current_value = current_value
                
            except Exception as e:
                self.logger.warning(f"Failed to update monitor for {parameter_path}: {e}")
    ...
    def _check_modification_triggers(self) -> List[str]:
        ...
        for rule_id, rule in self.modification_rules.items():
            ...
            if (rule.last_applied and 
                self.current_simulation_time - rule.last_applied < self.modification_cooldown):
                continue
    ...
    def _evaluate_trigger(self, rule: ModificationRule) -> bool:
        ...
        if rule.trigger == ModificationTrigger.TIME_BASED:
            return rule.trigger_time and self.current_simulation_time >= rule.trigger_time
        
        elif rule.trigger == ModificationTrigger.CONDITION_BASED:
            return rule.trigger_condition and self._evaluate_condition(rule.trigger_condition)
    ...
    def _apply_modification_rule(self, rule_id: str, model_state: Dict[str, Any]) -> None:
        ...
        if rule.value_function:
            new_value = self._calculate_value_from_function(rule.value_function, model_state)
        else:
        ...
        if rule.strategy == ModificationStrategy.IMMEDIATE:
            ...
            result = {"success": False, "error": f"Unsupported modification strategy: {rule.strategy}"}
    ...
    def _execute_parameter_modification(self, rule: ModificationRule, new_value: Any) -> Dict[str, Any]:
        """Execute immediate parameter modification"""
        
        try:
            # Get current value
            current_value = self._get_parameter_value(rule.parameter_path, self.system_state)
            
            # Validate new value against constraints
            if rule.constraint:
                validation = self._validate_parameter_constraint(rule.constraint, new_value)
    ...
    def _start_gradual_modification(self, rule: ModificationRule, target_value: Any) -> Dict[str, Any]:
    ...
    def _get_parameter_value(self, parameter_path: str, model_state: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            self.logger.warning(f"Failed to get parameter value for {parameter_path}: {e}")
            return None
    ...
    def _get_current_performance_metrics(self) -> Dict[str, float]:
        ...
        if "foraging_efficiency" in self.system_state:
            performance_metrics["foraging_efficiency"] = self.system_state["foraging_efficiency"]
        ...
        if "population_growth" in self.system_state:
            performance_metrics["population_growth"] = self.system_state["population_growth"]
        
        # Calculate derived metrics
        if "total_energy_collected" in self.system_state and "total_energy_consumed" in self.system_state:
            total_collected = self.system_state["total_energy_collected"]
            total_consumed = self.system_state["total_energy_consumed"]
            if total_consumed > 0:
                performance_metrics["energy_efficiency"] = total_collected / total_consumed
    ...
    def _calculate_parameter_adjustment(self, param_path: str, metric_name: str, 
        ...
        if "efficiency" in metric_name or "health" in metric_name or "growth" in metric_name:
        ...
        return {
            ...
            "adjustment": adjustment if isinstance(current_param_value, (int, float)) else 0.0,
            "magnitude": adjustment_magnitude,
            "direction": direction,
            "error": error
        }
    
    def _calculate_value_from_function(self, value_function: str, model_state: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            self.logger.warning(f"Failed to calculate value from function '{value_function}': {e}")
            return None
    ...
    def _execute_stepped_modification(self, rule: ModificationRule, new_value: Any) -> Dict[str, Any]:
    ...
    def _update_system_health_metrics(self) -> None:
        ...
        total_modifications = len(self.modification_history)
        recent_modifications = len([h for h in self.modification_history 
                                   if h.timestamp > self.current_simulation_time - 50.0])
        
        # Calculate modification success rate
        successful_modifications = len([h for h in self.modification_history if h.success])
