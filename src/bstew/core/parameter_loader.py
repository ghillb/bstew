...
class RuntimeParameterManager:
    ...
    def initialize_parameters(self, parameters: Dict[str, Any], 
                            validation_rules: Optional[Dict[str, ParameterValidationRule]] = None) -> None:
    ...
    def modify_parameter(self, parameter_name: str, new_value: Any, 
                        change_reason: str = "Manual modification") -> bool:
        """Modify a parameter during simulation runtime"""
        with self.parameter_lock:
            old_value = self.current_parameters.get(parameter_name)
            
            # Validate the change
            validation_result = self._validate_parameter_change(parameter_name, new_value)
            ...
            if validation_result[0]:
                ...
                self.logger.info(f"Parameter {parameter_name} changed from {old_value} to {new_value}")
                return True
            else:
                self.logger.warning(f"Parameter change rejected: {validation_result[1]}")
                return False
                
    def _validate_parameter_change(self, parameter_name: str, new_value: Any) -> Tuple[bool, Optional[str]]:
        ...
        if not isinstance(new_value, rule.data_type):
            return False, f"Invalid type for {parameter_name}: expected {rule.data_type.__name__}, got {type(new_value).__name__}"
            
        # Range validation
        if rule.min_value is not None and new_value < rule.min_value:
            return False, f"Value {new_value} is below minimum {rule.min_value} for {parameter_name}"
            
        if rule.max_value is not None and new_value > rule.max_value:
            return False, f"Value {new_value} is above maximum {rule.max_value} for {parameter_name}"
            
        # Allowed values validation
        if rule.allowed_values is not None and new_value not in rule.allowed_values:
            return False, f"Value {new_value} not in allowed values {rule.allowed_values} for {parameter_name}"
    ...
    def rollback_parameter(self, parameter_name: str) -> bool:
        ...
        for event in reversed(self.parameter_history):
            if event.parameter_name == parameter_name and event.validation_passed:
                return self.modify_parameter(parameter_name, event.old_value, "Rollback")
    ...
    def rollback_all_parameters(self) -> bool:
        ...
        with self.parameter_lock:
            ...
            # Log rollback
            self.parameter_history.append(ParameterChangeEvent(
                timestamp=datetime.now(),
                parameter_name="ALL",
    ...
    def get_parameter_history(self, parameter_name: Optional[str] = None) -> List[ParameterChangeEvent]:
        """Get parameter change history"""
        if parameter_name is None:
            return self.parameter_history.copy()
        else:
            return [event for event in self.parameter_history if event.parameter_name == parameter_name]
...
class BehavioralParameters(BaseModel):
    ...
    foraging_duration_min: int = Field(default=10, ge=1, description="Minimum foraging duration")
    foraging_duration_max: int = Field(default=50, ge=1, description="Maximum foraging duration")
    nursing_duration_min: int = Field(default=5, ge=1, description="Minimum nursing duration")
    nursing_duration_max: int = Field(default=30, ge=1, description="Maximum nursing duration")
    resting_duration_min: int = Field(default=1, ge=1, description="Minimum resting duration")
    resting_duration_max: int = Field(default=10, ge=1, description="Maximum resting duration")
    
    # Energy parameters
    base_energy_consumption: float = Field(default=1.0, ge=0.0, description="Base energy consumption per step")
    foraging_energy_multiplier: float = Field(default=3.0, ge=1.0, description="Foraging energy multiplier")
    nursing_energy_multiplier: float = Field(default=1.5, ge=1.0, description="Nursing energy multiplier")
    
    # Foraging behavior
    exploration_probability: float = Field(default=0.2, ge=0.0, le=1.0, description="Exploration probability")
    dance_following_probability: float = Field(default=0.3, ge=0.0, le=1.0, description="Dance following probability")
    memory_decay_rate: float = Field(default=0.95, ge=0.0, le=1.0, description="Memory decay rate per day")
    
    # Social behavior
    dance_threshold: float = Field(default=0.7, ge=0.0, le=1.0, description="Quality threshold for dancing")
    communication_range: float = Field(default=5.0, ge=0.0, description="Communication range in meters")
    
    # Role transition parameters
    nurse_to_forager_age: int = Field(default=200, ge=0, description="Age for nurse to forager transition")
    forager_experience_threshold: int = Field(default=5, ge=0, description="Experience threshold for foraging")
    
    @field_validator('foraging_duration_max')
    @classmethod
    def validate_foraging_duration(cls, v: float, info) -> float:
        if hasattr(info, 'data') and 'foraging_duration_min' in info.data and v < info.data['foraging_duration_min']:
            raise ValueError('foraging_duration_max must be >= foraging_duration_min')
        return v
    
    @field_validator('nursing_duration_max')
    @classmethod
    def validate_nursing_duration(cls, v: float, info) -> float:
        if hasattr(info, 'data') and 'nursing_duration_min' in info.data and v < info.data['nursing_duration_min']:
            raise ValueError('nursing_duration_max must be >= nursing_duration_min')
        return v
    
    @field_validator('resting_duration_max')
    @classmethod
    def validate_resting_duration(cls, v: float, info) -> float:
        if hasattr(info, 'data') and 'resting_duration_min' in info.data and v < info.data['resting_duration_min']:
...
class EnvironmentalParameters(BaseModel):
    ...
    # Seasonal parameters
    season_length: int = Field(default=365, ge=1, description="Length of season in days")
    flowering_season_start: int = Field(default=60, ge=0, description="Start of flowering season")
    flowering_season_end: int = Field(default=280, ge=0, description="End of flowering season")
    
    # Resource availability
    nectar_base_availability: float = Field(default=1.0, ge=0.0, description="Base nectar availability")
    pollen_base_availability: float = Field(default=1.0, ge=0.0, description="Base pollen availability")
    resource_regeneration_rate: float = Field(default=0.1, ge=0.0, description="Resource regeneration rate")
    
    # Spatial parameters
    foraging_range: float = Field(default=1000.0, ge=0.0, description="Maximum foraging range (m)")
    patch_density: float = Field(default=0.1, ge=0.0, description="Patch density per unit area")
    
    # Environmental stress factors
    weather_stress_threshold: float = Field(default=0.3, ge=0.0, le=1.0, description="Weather stress threshold")
    predator_pressure_base: float = Field(default=0.1, ge=0.0, le=1.0, description="Base predator pressure")
    
    @field_validator('temperature_max')
    @classmethod
    def validate_temperature_range(cls, v: float, info) -> float:
        if hasattr(info, 'data') and 'temperature_min' in info.data and v < info.data['temperature_min']:
            raise ValueError('temperature_max must be >= temperature_min')
        return v
    
    @field_validator('flowering_season_end')
    @classmethod
    def validate_flowering_season(cls, v: int, info) -> int:
        if hasattr(info, 'data') and 'flowering_season_start' in info.data and v < info.data['flowering_season_start']:
...
class ColonyParameters(BaseModel):
    ...
    # Initial population
    initial_population: int = Field(default=1000, ge=1, description="Initial colony population")
    initial_nurses: int = Field(default=600, ge=0, description="Initial number of nurses")
    initial_foragers: int = Field(default=300, ge=0, description="Initial number of foragers")
    initial_builders: int = Field(default=80, ge=0, description="Initial number of builders")
    initial_guards: int = Field(default=20, ge=0, description="Initial number of guards")
    
    # Queen parameters
    queen_egg_laying_rate: float = Field(default=2000.0, ge=0.0, description="Queen egg laying rate per day")
    queen_lifespan: int = Field(default=1825, ge=1, description="Queen lifespan in days")
    
    # Colony energy management
    energy_storage_capacity: float = Field(default=10000.0, ge=0.0, description="Colony energy storage capacity")
    energy_reserve_threshold: float = Field(default=0.3, ge=0.0, le=1.0, description="Energy reserve threshold")
    
    # Genetic parameters
    genetic_diversity_target: float = Field(default=0.8, ge=0.0, le=1.0, description="Target genetic diversity")
    inbreeding_tolerance: float = Field(default=0.1, ge=0.0, le=1.0, description="Inbreeding tolerance")
    
    # Population dynamics
    max_population: int = Field(default=50000, ge=1, description="Maximum colony population")
    carrying_capacity_factor: float = Field(default=0.9, ge=0.0, le=1.0, description="Carrying capacity factor")
    
    @field_validator('initial_population')
    @classmethod
    def validate_initial_population(cls, v: int, info) -> int:
        # Check if sum of role populations equals total (only if all role values are provided)
        role_fields = ['initial_nurses', 'initial_foragers', 'initial_builders', 'initial_guards']
        if hasattr(info, 'data') and all(field in info.data for field in role_fields):
            ...
            if role_sum != v:
                raise ValueError(f'Sum of role populations ({role_sum}) must equal initial_population ({v})')
        return v
...
class ParameterLoader(BaseModel):
    ...
    # Configuration
    parameter_directory: str = Field(default="artifacts/parameters", description="Directory containing parameter files")
    validation_enabled: bool = Field(default=True, description="Enable parameter validation")
    auto_save_enabled: bool = Field(default=True, description="Enable auto-save of validated parameters")
    
    # Parameter storage
    behavioral_params: Optional[BehavioralParameters] = Field(default=None, description="Loaded behavioral parameters")
    environmental_params: Optional[EnvironmentalParameters] = Field(default=None, description="Loaded environmental parameters")
    colony_params: Optional[ColonyParameters] = Field(default=None, description="Loaded colony parameters")
    
    # Validation rules
    validation_rules: Dict[str, ParameterValidationRule] = Field(default_factory=dict, description="Validation rules")
    
    # Load history
    load_history: List[Dict[str, Any]] = Field(default_factory=list, description="Parameter load history")
    
    # Runtime components (excluded from Pydantic validation)
    runtime_manager: RuntimeParameterManager = Field(default=None, exclude=True)
    netlogo_integrator: NetLogoDataIntegrator = Field(default=None, exclude=True)
    ...
    def _initialize_validation_rules(self) -> None:
        ...
        self.validation_rules.update({
            "foraging_duration_min": ParameterValidationRule(min_value=1, max_value=1000, data_type=int),
            "foraging_duration_max": ParameterValidationRule(min_value=1, max_value=1000, data_type=int),
            "exploration_probability": ParameterValidationRule(min_value=0.0, max_value=1.0, data_type=float),
            "dance_following_probability": ParameterValidationRule(min_value=0.0, max_value=1.0, data_type=float),
            "memory_decay_rate": ParameterValidationRule(min_value=0.0, max_value=1.0, data_type=float),
        })
        
        # Environmental parameter rules
        self.validation_rules.update({
            "temperature_min": ParameterValidationRule(min_value=-50.0, max_value=50.0, data_type=float),
            "temperature_max": ParameterValidationRule(min_value=-50.0, max_value=50.0, data_type=float),
            "nectar_base_availability": ParameterValidationRule(min_value=0.0, max_value=10.0, data_type=float),
            "pollen_base_availability": ParameterValidationRule(min_value=0.0, max_value=10.0, data_type=float),
        })
        
        # Colony parameter rules
        self.validation_rules.update({
            "initial_population": ParameterValidationRule(min_value=1, max_value=100000, data_type=int),
            "queen_egg_laying_rate": ParameterValidationRule(min_value=0.0, max_value=10000.0, data_type=float),
            "genetic_diversity_target": ParameterValidationRule(min_value=0.0, max_value=1.0, data_type=float),
        })
    
    def load_parameters_from_csv(self, file_path: str, parameter_type: ParameterType) -> Dict[str, Any]:
    ...
    def load_parameters_from_json(self, file_path: str, parameter_type: ParameterType) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Error loading JSON parameters from {file_path}: {str(e)}")
    ...
    def load_parameters_from_excel(self, file_path: str, parameter_type: ParameterType, 
        ...
        try:
            ...
            raise ValueError(f"Error loading Excel parameters from {file_path}: {str(e)}")
    ...
    def _validate_parameter(self, param_name: str, param_value: Any) -> None:
        ...
        if not isinstance(param_value, rule.data_type):
            raise ValueError(f"Parameter {param_name} must be of type {rule.data_type.__name__}")
        ...
        if rule.allowed_values is not None and param_value not in rule.allowed_values:
            raise ValueError(f"Parameter {param_name} must be one of {rule.allowed_values}")
    
    def load_behavioral_parameters(self, file_path: str, file_format: ParameterFormat = ParameterFormat.CSV) -> BehavioralParameters:
        ...
        if file_format == ParameterFormat.CSV:
            ...
            params = self.load_parameters_from_excel(file_path, ParameterType.BEHAVIORAL)
        else:
        ...
        if self.auto_save_enabled:
            self._save_validated_parameters("behavioral", self.behavioral_params.model_dump())
        
        return self.behavioral_params
    
    def load_environmental_parameters(self, file_path: str, file_format: ParameterFormat = ParameterFormat.CSV) -> EnvironmentalParameters:
        """Load environmental parameters from file"""
        
        if file_format == ParameterFormat.CSV:
            params = self.load_parameters_from_csv(file_path, ParameterType.ENVIRONMENTAL)
        elif file_format == ParameterFormat.JSON:
            params = self.load_parameters_from_json(file_path, ParameterType.ENVIRONMENTAL)
        elif file_format == ParameterFormat.XLSX:
            params = self.load_parameters_from_excel(file_path, ParameterType.ENVIRONMENTAL)
        else:
            raise ValueError(f"Unsupported file format: {file_format}")
        ...
        # Auto-save if enabled
        if self.auto_save_enabled:
            self._save_validated_parameters("environmental", self.environmental_params.model_dump())
        
        return self.environmental_params
    
    def load_colony_parameters(self, file_path: str, file_format: ParameterFormat = ParameterFormat.CSV) -> ColonyParameters:
    ...
    def load_all_parameters(self, parameter_directory: str = None) -> Dict[str, Any]:
        ...
        if not os.path.exists(parameter_directory):
            raise FileNotFoundError(f"Parameter directory not found: {parameter_directory}")
        ...
        file_patterns = {
            ...
            "environmental": ["environmental.csv", "environmental.json", "environmental.xlsx"],
            ...
            for file_name in file_names:
                ...
                if os.path.exists(file_path):
                    ...
                    if param_type == "behavioral":
                        all_params["behavioral"] = self.load_behavioral_parameters(file_path, file_format)
                    elif param_type == "environmental":
                        all_params["environmental"] = self.load_environmental_parameters(file_path, file_format)
                    elif param_type == "colony":
                        all_params["colony"] = self.load_colony_parameters(file_path, file_format)
                    
                    break  # Use first found file for each type
        
        return all_params
    
    def _save_validated_parameters(self, param_type: str, params: Dict[str, Any]) -> None:
    ...
    def get_parameter_summary(self) -> Dict[str, Any]:
        ...
        if self.environmental_params:
            summary["environmental_params"] = {
                "temperature_range": (self.environmental_params.temperature_min, self.environmental_params.temperature_max),
                "foraging_range": self.environmental_params.foraging_range,
                "resource_availability": (self.environmental_params.nectar_base_availability, self.environmental_params.pollen_base_availability)
            }
    ...
    def create_parameter_template(self, param_type: ParameterType, output_path: str) -> None:
    ...
    def validate_all_parameters(self) -> Dict[str, List[str]]:
        ...
        if self.behavioral_params:
            try:
                self.behavioral_params.model_validate(self.behavioral_params.model_dump())
            except Exception as e:
        ...
        if self.environmental_params:
            try:
                self.environmental_params.model_validate(self.environmental_params.model_dump())
            except Exception as e:
    ...
    def enable_runtime_modifications(self, parameters: Dict[str, Any], 
                                   validation_rules: Optional[Dict[str, ParameterValidationRule]] = None) -> None:
    ...
    def modify_parameter_runtime(self, parameter_name: str, new_value: Any, 
                               change_reason: str = "Runtime modification") -> bool:
        """Modify a parameter during runtime"""
        return self.runtime_manager.modify_parameter(parameter_name, new_value, change_reason)
    ...
    def register_parameter_change_callback(self, parameter_name: str, callback: callable) -> None:
        """Register callback for parameter changes"""
        self.runtime_manager.register_change_callback(parameter_name, callback)
        
    def get_parameter_change_history(self, parameter_name: Optional[str] = None) -> List[ParameterChangeEvent]:
        """Get parameter change history"""
    ...
    def setup_default_validation_rules(self) -> Dict[str, ParameterValidationRule]:
        ...
        rules = {
            ...
            "bee_species": ParameterValidationRule(
                allowed_values=["apis_mellifera", "bombus_terrestris", "bombus_lapidarius"],
    ...
    def convert_netlogo_to_bstew(self, netlogo_directory: str, output_directory: str) -> None:
        """Convert NetLogo data files to BSTEW format"""
        self.netlogo_integrator.convert_netlogo_to_bstew(Path(netlogo_directory), Path(output_directory))
    
    def load_parameters_with_validation(self, file_path: str, parameter_type: ParameterType, 
                                      validate_ranges: bool = True) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        try:
            ...
            if validate_ranges and parameter_type in [ParameterType.BEHAVIORAL, ParameterType.ENVIRONMENTAL]:
                range_validation = self._validate_parameter_ranges(params, parameter_type)
                validation_result["range_violations"] = range_validation["violations"]
                if range_validation["violations"]:
                    validation_result["valid"] = False
                    validation_result["errors"].extend([f"Range violation: {v}" for v in range_validation["violations"]])
    ...
    def get_parameter_validation_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "behavioral_parameters": {
                "cropvolume_myl": {"min": 10.0, "max": 150.0, "type": "float", "description": "Crop volume in microliters"},
                "glossaLength_mm": {"min": 1.0, "max": 10.0, "type": "float", "description": "Glossa length in millimeters"},
                "proboscis_length_mm": {"min": 3.0, "max": 15.0, "type": "float", "description": "Proboscis length in millimeters"},
                "mandible_width_mm": {"min": 0.5, "max": 3.0, "type": "float", "description": "Mandible width in millimeters"},
                "dance_threshold": {"min": 0.0, "max": 1.0, "type": "float", "description": "Quality threshold for dancing"},
                "exploration_probability": {"min": 0.0, "max": 1.0, "type": "float", "description": "Exploration probability"},
                "memory_decay_rate": {"min": 0.8, "max": 1.0, "type": "float", "description": "Memory decay rate per day"}
            },
            "environmental_parameters": {
                "temperature_min": {"min": -10.0, "max": 40.0, "type": "float", "description": "Minimum temperature in Celsius"},
                "temperature_max": {"min": 0.0, "max": 50.0, "type": "float", "description": "Maximum temperature in Celsius"},
                "season_length": {"min": 30, "max": 365, "type": "int", "description": "Season length in days"}
            },
            "colony_parameters": {
                "initial_population": {"min": 10, "max": 10000, "type": "int", "description": "Initial colony population"},
                "max_foraging_range": {"min": 100.0, "max": 5000.0, "type": "float", "description": "Maximum foraging range in meters"},
                "energy_threshold": {"min": 1.0, "max": 100.0, "type": "float", "description": "Energy threshold for activities"}
            }
        }
        ...
        if NetLogoFileType.PARAMETERS in files_by_type:
            for file_info in files_by_type[NetLogoFileType.PARAMETERS]:
                try:
                    ...
                    self.logger.error(f"Error loading parameters from {file_info.file_path}: {e}")
        ...
        if NetLogoFileType.SPECIES in files_by_type:
            ...
            for file_info in files_by_type[NetLogoFileType.SPECIES]:
                try:
                    ...
                    self.logger.error(f"Error loading species from {file_info.file_path}: {e}")
            if species_data:
