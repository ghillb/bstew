...
class SystemIntegrator(BaseModel):
    ...
    output_directory: str = Field(
        default="artifacts", description="Base output directory"
    )
    enable_mortality_tracking: bool = Field(
        default=True, description="Enable dead colony tracking"
    )
    enable_excel_reporting: bool = Field(
        default=True, description="Enable Excel report generation"
    )
    enable_spatial_analysis: bool = Field(
        default=True, description="Enable spatial analysis"
    )
    ...
    # Integration status
    systems_initialized: bool = Field(
        default=False, description="Systems initialization status"
    )
    active_systems: List[str] = Field(
        default_factory=list, description="Currently active systems"
    )
    ...
    def initialize_systems(self, model: Any = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"System integration completed. Active systems: {self.active_systems}"
            )
        ...
        try:
            ...
            if self.data_collector and "data_collector" in self.active_systems:
                for colony in colonies:
                    self.logger.debug(
                        f"Updating colony metrics for colony {getattr(colony, 'unique_id', 'unknown')}"
                    )
                ...
                if hasattr(model, "environment"):
                    self.data_collector.collect_environmental_data(
                        model.environment, current_step
                    )

            # Update mortality tracking
            if (
                self.mortality_tracker
                and "mortality_tracker" in self.active_systems
                and self.data_collector
            ):
                for colony in colonies:
                    colony_id = getattr(
                        colony, "colony_id", getattr(colony, "unique_id", 0)
                    )
                    if colony_id in self.data_collector.colony_metrics:
                        metrics = self.data_collector.colony_metrics[colony_id]
                        self.mortality_tracker.update_colony_health(
                            colony_id, metrics.model_dump(), current_step
                        )
            ...
            if self.health_monitor and "health_monitor" in self.active_systems:
                for colony in colonies:
                    self.logger.debug(
                        f"Updating health monitor for colony {getattr(colony, 'unique_id', 'unknown')}"
                    )

                    # Get population count as a number, not a dict
                    population_counts = getattr(colony, "population_counts", {})
                    if isinstance(population_counts, dict):
                        # Sum all population categories or get total_population
                        total_population = getattr(
                            colony,
                            "total_population",
                            sum(population_counts.values()) if population_counts else 0,
                        )
                    else:
                        total_population = population_counts

                    # Get resources properly
                    resources = getattr(colony, "resources", {})
                    if hasattr(resources, "model_dump"):
                        resources_dict = resources.model_dump()
                    elif hasattr(resources, "__dict__"):
                        resources_dict = resources.__dict__
                    else:
                        resources_dict = (
                    ...
                    colony_data = {
                        "population": total_population,  # Pass number, not dict
                        "adult_population": total_population,  # Also provide as adult_population
                        "resources": resources_dict,
                        "location": getattr(colony, "location", (0.0, 0.0)),
                        "total_energy": getattr(colony, "total_energy", 0.0),
                        "mortality_rate": getattr(colony, "mortality_rate", 0.01),
                        "foraging_efficiency": getattr(
                            colony, "foraging_efficiency", 0.5
                        ),
                    }
                    self.logger.debug(
                        f"Colony data types - population: {type(colony_data['population'])}, resources: {type(colony_data['resources'])}, location: {type(colony_data['location'])}"
                    )
                    self.health_monitor.update_colony_health(
                        colony.unique_id, colony_data, float(current_step)
                    )

            # Update spatial analysis (less frequent)
            if (
                self.spatial_analyzer
                and "spatial_analyzer" in self.active_systems
                and current_step % 24 == 0
            ):  # Update daily
    ...
    def _group_agents_by_colony(self, agents: List[Any]) -> List[Any]:
        ...
        colony_agents: Dict[int, List[Any]] = {}
        for agent in agents:
        ...
        for colony_id, agent_list in colony_agents.items():
            ...
            class PseudoColony:
                def __init__(self, colony_id: int, agents: List[Any]) -> None:
    ...
    def _update_spatial_analysis(self, model: Any, current_step: int) -> None:
        ...
        try:
            ...
            if hasattr(model, "space") and hasattr(
                model.space, "get_all_cell_contents"
            ):
                # Mesa space-based model
                all_agents = model.space.get_all_cell_contents()

                if all_agents and self.spatial_analyzer is not None:
                    self.spatial_analyzer.update_agent_positions(all_agents)

        except Exception as e:
            self.logger.error(f"Error updating spatial analysis: {e}")

    def generate_reports(
        self, current_step: int, final_report: bool = False
    ) -> Dict[str, str]:
        ...
        try:
            ...
            if self.mortality_tracker and "mortality_tracker" in self.active_systems:
                if final_report:
                    mortality_summary = (
                        self.mortality_tracker.generate_survival_report()
                    )
    ...
    def _generate_spatial_report(self, current_step: int) -> str:
        ...
        try:
            ...
            if self.spatial_analyzer is not None and hasattr(
                self.spatial_analyzer, "get_analysis_summary"
            ):
        ...
        for system_name in self.active_systems:
            ...
            if system_obj:
                ...
                if hasattr(system_obj, "get_status"):
                    status["system_details"][system_name]["status"] = (
                        system_obj.get_status()
                    )
