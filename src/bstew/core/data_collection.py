...
class BeeMetrics(BaseModel):
    ...
    total_steps: int = Field(
        default=0, ge=0, description="Total simulation steps lived"
    )
    activity_time: Dict[str, int] = Field(
        default_factory=dict, description="Time spent in each activity"
    )
    state_transitions: int = Field(
        default=0, ge=0, description="Number of state transitions"
    )

    # Foraging metrics
    foraging_trips: int = Field(default=0, ge=0, description="Total foraging trips")
    successful_foraging: int = Field(
        default=0, ge=0, description="Successful foraging attempts"
    )
    energy_collected: float = Field(
        default=0.0, ge=0.0, description="Total energy collected"
    )
    patches_discovered: int = Field(
        default=0, ge=0, description="New patches discovered"
    )

    # Energy and physiology
    energy_history: List[float] = Field(
        default_factory=list, description="Energy level history"
    )
    metabolic_rate: float = Field(
        default=1.0, ge=0.0, description="Current metabolic rate"
    )
    stress_level: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Current stress level"
    )

    # Social interactions
    dances_performed: int = Field(
        default=0, ge=0, description="Waggle dances performed"
    )
    dances_followed: int = Field(default=0, ge=0, description="Dances followed")
    communication_events: int = Field(
        default=0, ge=0, description="Total communication events"
    )
    recruitment_attempts: int = Field(
        default=0, ge=0, description="Recruitment attempts initiated"
    )
    recruits_obtained: int = Field(
        default=0, ge=0, description="Successful recruits obtained"
    )
    information_shared: int = Field(
        default=0, ge=0, description="Information sharing events"
    )

    # Enhanced foraging metrics
    foraging_sessions: int = Field(
        default=0, ge=0, description="Total foraging sessions"
    )
    foraging_session_duration_avg: float = Field(
        default=0.0, ge=0.0, description="Average session duration"
    )
    patch_visits: int = Field(default=0, ge=0, description="Total patch visits")
    unique_patches_visited: int = Field(
        default=0, ge=0, description="Number of unique patches visited"
    )
    foraging_efficiency_avg: float = Field(
        default=0.0, ge=0.0, description="Average foraging efficiency"
    )
    travel_distance_total: float = Field(
        default=0.0, ge=0.0, description="Total travel distance"
    )

    # Detailed foraging trip metrics
    foraging_trip_durations: List[float] = Field(
        default_factory=list, description="All foraging trip durations"
    )
    foraging_trip_energies: List[float] = Field(
        default_factory=list, description="Energy gained per trip"
    )
    foraging_trip_distances: List[float] = Field(
        default_factory=list, description="Distance traveled per trip"
    )
    flowers_visited_per_trip: List[int] = Field(
        default_factory=list, description="Flowers visited per trip"
    )
    patches_visited_per_trip: List[int] = Field(
        default_factory=list, description="Patches visited per trip"
    )

    # Success rate metrics
    successful_trips: int = Field(
        default=0, ge=0, description="Number of successful foraging trips"
    )
    failed_trips: int = Field(
        default=0, ge=0, description="Number of failed foraging trips"
    )
    foraging_success_rate: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Overall foraging success rate"
    )

    # Resource collection metrics
    nectar_collected_total: float = Field(
        default=0.0, ge=0.0, description="Total nectar collected (mg)"
    )
    pollen_collected_total: float = Field(
        default=0.0, ge=0.0, description="Total pollen collected (mg)"
    )
    nectar_per_trip_avg: float = Field(
        default=0.0, ge=0.0, description="Average nectar per trip"
    )
    pollen_per_trip_avg: float = Field(
        default=0.0, ge=0.0, description="Average pollen per trip"
    )

    # Behavioral adaptation
    strategy_changes: int = Field(
        default=0, ge=0, description="Number of strategy changes"
    )
    learning_events: int = Field(default=0, ge=0, description="Learning events")
    adaptation_score: float = Field(
        default=0.0, ge=0.0, description="Adaptation capability score"
    )
    ...
    efficiency_score: float = Field(
        default=0.0, ge=0.0, description="Overall efficiency rating"
    )
    contribution_score: float = Field(
        default=0.0, ge=0.0, description="Colony contribution score"
    )
...
class ColonyMetrics(BaseModel):
    ...
    total_population: int = Field(
        default=0, ge=0, description="Current total population"
    )
    population_by_role: Dict[str, int] = Field(
        default_factory=dict, description="Population by role"
    )
    population_by_age: Dict[int, int] = Field(
        default_factory=dict, description="Population by age group"
    )

    # Enhanced population tracking by life stage
    population_by_life_stage: Dict[str, int] = Field(
        default_factory=dict, description="Population by life stage"
    )
    ...
    population_size_day_list: List[int] = Field(
        default_factory=list, description="Daily population history"
    )
    egg_count_day_list: List[int] = Field(
        default_factory=list, description="Daily egg count history"
    )
    larva_count_day_list: List[int] = Field(
        default_factory=list, description="Daily larva count history"
    )
    pupa_count_day_list: List[int] = Field(
        default_factory=list, description="Daily pupa count history"
    )
    adult_count_day_list: List[int] = Field(
        default_factory=list, description="Daily adult count history"
    )
    ...
    total_queens_produced: int = Field(
        default=0, ge=0, description="Total queens produced to date"
    )
    total_drones_produced: int = Field(
        default=0, ge=0, description="Total drones produced to date"
    )
    total_workers_produced: int = Field(
        default=0, ge=0, description="Total workers produced to date"
    )
    queens_produced_today: int = Field(
        default=0, ge=0, description="Queens produced today"
    )
    drones_produced_today: int = Field(
        default=0, ge=0, description="Drones produced today"
    )
    workers_produced_today: int = Field(
        default=0, ge=0, description="Workers produced today"
    )

    # Production history lists
    queens_produced_day_list: List[int] = Field(
        default_factory=list, description="Daily queen production history"
    )
    drones_produced_day_list: List[int] = Field(
        default_factory=list, description="Daily drone production history"
    )
    workers_produced_day_list: List[int] = Field(
        default_factory=list, description="Daily worker production history"
    )
    ...
    forager_count: int = Field(
        default=0, ge=0, description="Current number of foragers"
    )
    ...
    # Daily metrics
    daily_productivity: float = Field(
        default=0.0, ge=0.0, description="Daily colony productivity"
    )
    foraging_efficiency_today: float = Field(
        default=0.0, ge=0.0, description="Today's foraging efficiency"
    )

    # Activity distribution
    activity_distribution: Dict[str, int] = Field(
        default_factory=dict, description="Bees by activity"
    )
    foraging_efficiency: float = Field(
        default=0.0, ge=0.0, description="Colony foraging efficiency"
    )
    ...
    daily_energy_collection: float = Field(
        default=0.0, ge=0.0, description="Energy collected today"
    )
    daily_energy_consumption: float = Field(
        default=0.0, ge=0.0, description="Energy consumed today"
    )

    # Genetic diversity
    genetic_diversity: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Genetic diversity index"
    )
    inbreeding_coefficient: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Inbreeding coefficient"
    )

    # Health metrics
    average_stress: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Average stress level"
    )
    disease_prevalence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Disease prevalence"
    )

    # Performance indicators
    colony_fitness: float = Field(
        default=0.0, ge=0.0, description="Overall colony fitness"
    )
    survival_probability: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Colony survival probability"
    )
...
class EnvironmentalMetrics(BaseModel):
    ...
    humidity: float = Field(
        default=50.0, ge=0.0, le=100.0, description="Humidity percentage"
    )
    ...
    photoperiod: float = Field(
        default=12.0, ge=0.0, le=24.0, description="Hours of daylight"
    )

    # Resource availability
    nectar_availability: float = Field(
        default=1.0, ge=0.0, description="Nectar availability index"
    )
    pollen_availability: float = Field(
        default=1.0, ge=0.0, description="Pollen availability index"
    )
    patch_quality: Dict[int, float] = Field(
        default_factory=dict, description="Quality by patch"
    )

    # Ecological factors
    predator_pressure: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Predator pressure index"
    )
    competition_level: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Inter-colony competition"
    )
    habitat_quality: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Habitat quality index"
    )
...
class DataCollectionRule(BaseModel):
    ...
    condition: Optional[str] = Field(
        default=None, description="Condition for collection"
    )
...
class ComprehensiveDataCollector(BaseModel):
    ...
    max_history_length: int = Field(
        default=10000, ge=100, description="Maximum history length"
    )
    collection_rules: List[DataCollectionRule] = Field(
        default_factory=list, description="Collection rules"
    )
    ...
    aggregated_metrics: Dict[int, Dict[str, Any]] = Field(default_factory=dict)
    ...
    def _setup_event_handlers(self) -> None:
        ...
        self.register_event_handler(
            "foraging_session_started", self._handle_foraging_session_start
        )
        self.register_event_handler(
            "foraging_session_ended", self._handle_foraging_session_end
        )
        self.register_event_handler(
            "foraging_trip_completed", self._handle_foraging_trip
        )

        # Communication event handlers
        self.register_event_handler("dance_performed", self._handle_dance_event)
        self.register_event_handler(
            "recruitment_initiated", self._handle_recruitment_event
        )
        self.register_event_handler(
            "information_shared", self._handle_information_sharing
        )
        ...
        self.register_event_handler(
            "resource_collection", self._handle_resource_collection
        )
    ...
    def integrate_with_foraging_system(
    ...
    def collect_bee_data(self, bee_agent: Any, current_step: int) -> None:
        ...
        metrics.role = (
            bee_agent.role.value
            if hasattr(bee_agent.role, "value")
            else str(bee_agent.role)
        )
        metrics.status = (
            bee_agent.status.value
            if hasattr(bee_agent.status, "value")
            else str(bee_agent.status)
        )
        ...
        if (
            hasattr(bee_agent, "activity_state_machine")
            and bee_agent.activity_state_machine
        ):
        ...
        if (
            hasattr(bee_agent, "foraging_decision_engine")
            and bee_agent.foraging_decision_engine
        ):
        ...
        if (
            ...
            effects = bee_agent.activity_state_machine.get_state_physiological_effects(
                bee_agent.status
            )
            if effects:
    ...
    def collect_colony_data(self, colony: Any, current_step: int) -> None:
        ...
        if hasattr(colony, "agents"):
            ...
            metrics.total_population = len(
                [a for a in agents if a.status != BeeStatus.DEAD]
            )
            ...
            for agent in agents:
                if agent.status != BeeStatus.DEAD:
                    ...
                    if (
                        hasattr(agent, "activity_state_machine")
                        and agent.activity_state_machine
                    ):
                        effects = agent.activity_state_machine.get_state_physiological_effects(
                            agent.status
                        )
                        if effects:
        ...
        self._log_data_point(
            current_step, "colony_update", colony_id, colony_id=colony_id
        )
    ...
    def _update_daily_history_lists(
        self, metrics: "ColonyMetrics", current_step: int
    ) -> None:
    ...
    def track_foraging_trip_details(
        self, bee_id: int, trip_data: Dict[str, Any]
    ) -> None:
        ...
        if metrics.foraging_trips > 0:
            metrics.nectar_per_trip_avg = (
                metrics.nectar_collected_total / metrics.foraging_trips
            )
            metrics.pollen_per_trip_avg = (
                metrics.pollen_collected_total / metrics.foraging_trips
            )
    ...
    def collect_environmental_data(self, environment: Any, current_step: int) -> None:
        ...
        if hasattr(environment, "resource_availability"):
            ...
            self.environmental_metrics.nectar_availability = resource_info.get(
                "nectar", 1.0
            )
            self.environmental_metrics.pollen_availability = resource_info.get(
                "pollen", 1.0
            )
    ...
    def _calculate_bee_performance(self, metrics: BeeMetrics, bee_agent: Any) -> None:
        ...
        if metrics.total_steps > 0:
            foraging_efficiency = metrics.successful_foraging / max(
                1, metrics.foraging_trips
            )
            activity_efficiency = (
                sum(metrics.activity_time.values()) / metrics.total_steps
            )
        ...
        if metrics.role == "forager":
            metrics.contribution_score = metrics.energy_collected / max(
                1, metrics.total_steps
            )
        elif metrics.role == "nurse":
            metrics.contribution_score = metrics.communication_events / max(
                1, metrics.total_steps
            )
        else:
    ...
    def _calculate_colony_fitness(self, metrics: ColonyMetrics) -> None:
        ...
        population_stability = min(
            1.0, metrics.total_population / 100
        )  # Assume 100 is optimal

        # Energy efficiency
        energy_efficiency = 1.0
        if metrics.daily_energy_consumption > 0:
            energy_efficiency = min(
                1.0, metrics.daily_energy_collection / metrics.daily_energy_consumption
            )
    ...
    def get_time_series(
        self, metric_name: str, time_range: Optional[Tuple[int, int]] = None
    ) -> List[Tuple[int, Any]]:
    ...
    def get_aggregated_data(
        self, metric_name: str, aggregation_method: str, time_window: int = 100
    ) -> Dict[str, Any]:
    ...
    def _export_to_json(self, filename: str) -> None:
        ...
        export_data = {
            ...
            "colony_metrics": {
                k: v.model_dump() for k, v in self.colony_metrics.items()
            },
    ...
    def _handle_foraging_session_end(self, event: SimulationEvent) -> None:
        ...
        if event.source_agent_id and event.colony_id:
            ...
            if session_result and event.source_agent_id in self.bee_metrics:
                ...
                metrics.travel_distance_total += session_result.get(
                ...
                new_duration = (
                    session_result.get("session_duration", 0.0) / 60.0
                )  # Convert to minutes

                if session_count > 0:
                    metrics.foraging_session_duration_avg = (
                        current_avg * (session_count - 1) + new_duration
                    ) / session_count
                else:
                ...
                if session_count > 0:
                    metrics.foraging_efficiency_avg = (
                        metrics.foraging_efficiency_avg * (session_count - 1)
                        + session_efficiency
                    ) / session_count
                else:
            ...
            data_point = DataPoint(
                ...
                value=event.event_data.get("session_result", {}).get(
    ...
    def _handle_foraging_trip(self, event: SimulationEvent) -> None:
        ...
        if event.source_agent_id and event.source_agent_id in self.bee_metrics:
            ...
            if event.event_data:
                self.track_foraging_trip_details(
                    event.source_agent_id, event.event_data
                )
    ...
    def _handle_bee_emergence(self, event: SimulationEvent) -> None:
        ...
        if event.source_agent_id:
            ...
            if event.source_agent_id not in self.bee_metrics:
                self.bee_metrics[event.source_agent_id] = BeeMetrics(
                    bee_id=event.source_agent_id
                )
    ...
    def aggregate_data(self, current_step: int) -> Dict[str, Any]:
        ...
        for rule in self.aggregation_rules:
            try:
                ...
                if self.logger:
                    self.logger.error(
                        f"Error applying aggregation rule {rule.metric_name}: {e}"
                    )
    ...
    def _apply_aggregation_rule(self, rule: AggregationRule, current_step: int) -> Any:
        ...
        for group_key, points in grouped_data.items():
            values = [
                point.value for point in points if isinstance(point.value, (int, float))
            ]

            if values:
                result: Union[float, List[Union[int, float]]]
    ...
    def _evaluate_condition(self, point: DataPoint, condition: str) -> bool:
        ...
        try:
            ...
            if ">" in condition:
                field_str, threshold_str = condition.split(">")
    ...
    def get_aggregated_metric_data(
        self, metric_name: str, time_range: Optional[Tuple[int, int]] = None
    ) -> Dict[str, Any]:
        ...
        if time_range:
            start_time, end_time = time_range
            # Filter data based on time range - handle both int and string keys
            filtered_data = {}
            for k, v in data.items():
                try:
                    k_num = int(k) if isinstance(k, str) else k
                    if start_time <= k_num <= end_time:
                        filtered_data[k] = v
    ...
    def get_comprehensive_analytics(self) -> Dict[str, Any]:
        ...
        total_foraging_sessions = sum(
            m.foraging_sessions for m in self.bee_metrics.values()
        )
        total_energy_collected = sum(
            m.energy_collected for m in self.bee_metrics.values()
        )
        avg_foraging_efficiency = (
            statistics.mean(
                [
                    m.foraging_efficiency_avg
                    for m in self.bee_metrics.values()
                    if m.foraging_efficiency_avg > 0
                ]
            )
            if any(m.foraging_efficiency_avg > 0 for m in self.bee_metrics.values())
            else 0.0
        )
        ...
        analytics["foraging_analytics"] = {
            ...
            "total_patches_discovered": sum(
                m.patches_discovered for m in self.bee_metrics.values()
            ),
        }

        # Aggregate communication analytics
        total_dances = sum(m.dances_performed for m in self.bee_metrics.values())
        total_communication_events = sum(
            m.communication_events for m in self.bee_metrics.values()
        )
        ...
        analytics["communication_analytics"] = {
            ...
            "total_recruitment_attempts": sum(
                m.recruitment_attempts for m in self.bee_metrics.values()
            ),
            "total_information_shared": sum(
                m.information_shared for m in self.bee_metrics.values()
            ),
        }

        # Behavioral analytics
        total_state_transitions = sum(
            m.state_transitions for m in self.bee_metrics.values()
        )
        total_learning_events = sum(
            m.learning_events for m in self.bee_metrics.values()
        )

        analytics["behavioral_analytics"] = {
            "total_state_transitions": total_state_transitions,
            "total_learning_events": total_learning_events,
            "total_strategy_changes": sum(
                m.strategy_changes for m in self.bee_metrics.values()
            ),
            "average_adaptation_score": statistics.mean(
                [
                    m.adaptation_score
                    for m in self.bee_metrics.values()
                    if m.adaptation_score > 0
                ]
            )
            if any(m.adaptation_score > 0 for m in self.bee_metrics.values())
            else 0.0,
        }
        ...
        try:
            ...
            if simulation_metadata is None:
                simulation_metadata = {
                    ...
                    "duration_days": max(self.aggregated_metrics.keys())
                    if self.aggregated_metrics
                    else 100,
                    "start_date": min(m.timestamp for m in self.simulation_events)
                    if self.simulation_events
                    else time.time(),
                    "end_date": max(m.timestamp for m in self.simulation_events)
                    if self.simulation_events
                    else time.time(),
                }
    ...
    def get_population_data_for_excel(self) -> List[Dict[str, Any]]:
        ...
        for day, metrics in self.aggregated_metrics.items():
            if isinstance(metrics, dict) and "population" in metrics:
                ...
                total_pop = sum(
                    getattr(colony, "population_size", 0) for colony in metrics.values()
                )
                total_foragers = sum(
                    getattr(colony, "active_foragers", 0) for colony in metrics.values()
                )
                total_nurses = sum(
                    getattr(colony, "nurse_bees", 0) for colony in metrics.values()
                )
                ...
                population_data.append(
                    {
                        ...
                        "avg_fitness": statistics.mean(
                            [
                                getattr(colony, "colony_fitness", 0)
                                for colony in metrics.values()
                            ]
                        )
                        if metrics
                        else 0.0,
    ...
    def get_health_data_for_excel(self) -> List[Dict[str, Any]]:
        ...
        health_by_day: Dict[int, Dict[int, List[float]]] = defaultdict(
    ...
    def get_foraging_data_for_excel(self) -> List[Dict[str, Any]]:
        ...
        for day, metrics in self.aggregated_metrics.items():
            if isinstance(metrics, dict) and "foraging" in metrics:
                ...
                # New colony metrics format
                total_energy = sum(
                    getattr(colony, "daily_energy_collection", 0)
                    for colony in metrics.values()
                )
                avg_efficiency = (
                    statistics.mean(
                        [
                            getattr(colony, "foraging_efficiency", 0)
                            for colony in metrics.values()
                        ]
                    )
                    if metrics
            ...
            events.append(
                {
                    ...
                    "data_summary": str(event.event_data)[:100]
                    if event.event_data
                    else "",
                }
            )
    ...
    def get_summary_statistics_for_excel(self) -> Dict[str, Any]:
        ...
        if self.aggregated_metrics:
            population_data = [
                metrics.get("population", {}).get("total", 0)
            ...
            stats.update(
                {
                    ...
                    "Health Score Std Dev": statistics.stdev(all_health_scores)
                    if len(all_health_scores) > 1
                    else 0,
                }
            )
        ...
        if all_efficiency_scores:
            stats.update(
                {
                    "Average Foraging Efficiency": statistics.mean(
                        all_efficiency_scores
                    ),
    ...
    def get_key_findings_for_excel(self) -> List[str]:
        ...
        if self.aggregated_metrics:
            ...
            if len(population_data) > 1:
                growth_rate = (
                    (population_data[-1] - population_data[0]) / population_data[0]
                    if population_data[0] > 0
                    else 0
                )
                if growth_rate > 0.1:
                    findings.append(
                        f"Population grew by {growth_rate:.1%} over simulation period"
                    )
                elif growth_rate < -0.1:
                    findings.append(
                        f"Population declined by {abs(growth_rate):.1%} over simulation period"
                    )
                else:
        ...
        if event_types:
            most_common = max(event_types, key=lambda k: event_types[k])
            findings.append(
                f"Most frequent event type: {most_common} ({event_types[most_common]} occurrences)"
            )
        ...
        if total_collections > 1000:
            findings.append(
                f"High-frequency data collection: {total_collections:,} data points"
            )
