...
class NetLogoFileDetector:
            ...
            if headers:
                ...
                for file_type, keywords in self.column_patterns.items():
                    matches = sum(
                        ...
                        if any(keyword in h for h in headers_lower)
                    )
    ...
    def scan_directory(self, directory: Path) -> List[NetLogoFileInfo]:
        ...
        netlogo_files: List[NetLogoFileInfo] = []
    ...
    def _validate_file_structure(
        self, df: pd.DataFrame, file_type: NetLogoFileType
    ) -> List[str]:
...
class NetLogoParameterMapper:
    ...
    def map_netlogo_to_bstew(self, netlogo_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for netlogo_param, value in netlogo_params.items():
            if netlogo_param in self.parameter_mapping:
                ...
                # Try fuzzy matching
                bstew_param_match = self._fuzzy_match_parameter(netlogo_param)
                if bstew_param_match:
                    bstew_params[bstew_param_match] = value
                else:
                    unmapped_params[netlogo_param] = value

        if unmapped_params:
            self.logger.warning(
                f"Unmapped NetLogo parameters: {list(unmapped_params.keys())}"
            )
    ...
    def map_bstew_to_netlogo(self, bstew_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if unmapped_params:
            self.logger.warning(
                f"Unmapped BSTEW parameters: {list(unmapped_params.keys())}"
            )
...
class NetLogoDataIntegrator:
    ...
    def discover_netlogo_files(
        self, directory: Path
    ) -> Dict[NetLogoFileType, List[NetLogoFileInfo]]:
        """Discover and classify NetLogo files"""
        all_files = self.file_detector.scan_directory(directory)

        # Group by file type
        files_by_type: Dict[NetLogoFileType, List[NetLogoFileInfo]] = {}
    ...
    def load_netlogo_species(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for species in species_data:
                ...
                species_dict = {str(k): v for k, v in species.items()}
                mapped_species.append(self._map_species_data(species_dict))
    ...
    def validate_netlogo_compatibility(self, directory: Path) -> Dict[str, Any]:
        ...
        for required_type in required_files:
            if required_type not in files_by_type or not files_by_type[required_type]:
                validation_report["errors"].append(
                    f"Missing required file type: {required_type.value}"
                )
                validation_report["compatible"] = False

        # Enhanced NetLogo BEE-STEWARD v2 parameter validation
        if NetLogoFileType.PARAMETERS in files_by_type:
            param_validation = self._validate_netlogo_parameters(
                files_by_type[NetLogoFileType.PARAMETERS]
            )
        ...
        if NetLogoFileType.SPECIES in files_by_type:
            species_validation = self._validate_netlogo_species(
                files_by_type[NetLogoFileType.SPECIES]
            )
        ...
        if NetLogoFileType.FLOWERSPECIES in files_by_type:
            flower_validation = self._validate_netlogo_flower_species(
                files_by_type[NetLogoFileType.FLOWERSPECIES]
            )
        ...
        if NetLogoFileType.SPECIES not in files_by_type:
            validation_report["recommendations"].append(
                "Consider adding species files for better compatibility"
            )

        if NetLogoFileType.FLOWERSPECIES not in files_by_type:
            validation_report["recommendations"].append(
                "Consider adding flower species files for resource modeling"
            )
    ...
    def _validate_netlogo_parameters(
        self, parameter_files: List[NetLogoFileInfo]
    ) -> Dict[str, Any]:
        ...
        for file_info in parameter_files:
            try:
                ...
                validation["errors"].append(
                    f"Error reading parameter file {file_info.file_path}: {e}"
                )
            ...
            if param_name in found_parameters:
                ...
                try:
                    ...
                    if callable(param_type):
                        value = param_type(found_parameters[param_name])
                    else:
                    ...
                    validation["errors"].append(
                        f"Parameter {param_name} has invalid type: {e}"
                    )
                    validation["valid"] = False
            else:
                validation["required_parameters_found"][param_name] = False
                validation["warnings"].append(
                    f"Required parameter {param_name} not found - using default value"
                )

        return validation

    def _validate_netlogo_species(
        self, species_files: List[NetLogoFileInfo]
    ) -> Dict[str, Any]:
        ...
        for file_info in species_files:
            try:
                ...
                current_count = validation.get("species_count", 0)
                validation["species_count"] = (
                ...
                for col in required_columns:
                    if col in df.columns:
                        ...
                        validation["warnings"].append(
                            f"Species file missing column: {col}"
                        )

            except Exception as e:
                validation["warnings"].append(
                    f"Error reading species file {file_info.file_path}: {e}"
                )

        return validation

    def _validate_netlogo_flower_species(
        self, flower_files: List[NetLogoFileInfo]
    ) -> Dict[str, Any]:
        ...
        for file_info in flower_files:
            try:
                df = pd.read_csv(file_info.file_path)
                current_count = validation.get("flower_species_count", 0)
                validation["flower_species_count"] = (
                    current_count if isinstance(current_count, int) else 0
                ) + len(df)
                ...
                for col in required_columns:
                    if col in df.columns:
                        ...
                        validation["required_columns_found"][col] = False
                        validation["warnings"].append(
                            f"Flower species file missing column: {col}"
                        )

            except Exception as e:
                validation["warnings"].append(
                    f"Error reading flower species file {file_info.file_path}: {e}"
                )

        return validation

    def _validate_bee_steward_v2_requirements(
        ...
        if not any(
            "BEE-STEWARD" in str(files) for files in validation_report["files_found"]
        ):
            validation_report["recommendations"].append(
    ...
    def convert_netlogo_to_bstew(
        self, netlogo_directory: Path, output_directory: Path
    ) -> None:
            ...
            for file_info in files:
                try:
                    if file_type == NetLogoFileType.PARAMETERS:
                        ...
                        output_file = (
                            output_dir
                            / f"bstew_parameters_{file_info.file_path.stem}.json"
                        )
                        ...
                        output_file = (
                            output_dir
                            / f"bstew_species_{file_info.file_path.stem}.json"
                        )
                        with open(output_file, "w") as f:
