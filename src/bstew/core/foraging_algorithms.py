...
class ForagingContext:
    ...
    flower_density: Dict[int, float] = Field(
        default_factory=dict
    )  # Flower density per patch
    patch_distances: Dict[int, float] = Field(
        default_factory=dict
    )  # Distance to each patch
...
class ForagingDecisionEngine(BaseModel):
    ...
    memory_capacity: int = Field(
        default=20, ge=1, le=100, description="Maximum patches remembered"
    )
    memory_decay_rate: float = Field(
        default=0.95, ge=0.0, le=1.0, description="Daily memory decay"
    )

    # Decision weights
    quality_weight: float = Field(
        default=0.4, ge=0.0, le=1.0, description="Quality importance"
    )
    distance_weight: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Distance importance"
    )
    memory_weight: float = Field(
        default=0.2, ge=0.0, le=1.0, description="Memory strength importance"
    )
    novelty_weight: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Novelty preference"
    )

    # Behavioral parameters
    exploration_probability: float = Field(
        default=0.2, ge=0.0, le=1.0, description="Base exploration rate"
    )
    dance_following_probability: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Dance following rate"
    )
    energy_conservation_threshold: float = Field(
        default=30.0, ge=0.0, le=100.0, description="Energy conservation threshold"
    )

    # Internal state
    foraging_memories: Dict[int, ForagingMemory] = Field(
        default_factory=dict, description="Foraging memories by patch ID"
    )
    current_strategy: ForagingStrategy = Field(
        default=ForagingStrategy.BALANCED, description="Current foraging strategy"
    )
    decision_history: List[ForagingDecisionType] = Field(
        default_factory=list, description="Decision history"
    )
    success_rates: Dict[ForagingDecisionType, float] = Field(
        default_factory=dict, description="Success rates by decision type"
    )
    patch_selector: AdvancedPatchSelector = Field(
        default_factory=AdvancedPatchSelector, description="Advanced patch selector"
    )
    ...
    def make_foraging_decision(
        ...
        available_patches: List[Any],
    ) -> Tuple[ForagingDecisionType, Optional[int]]:
    ...
    def update_foraging_strategy(
        self, context: ForagingContext, current_energy: float
    ) -> None:
    ...
    def calculate_decision_probabilities(
        self, context: ForagingContext, current_energy: float
    ) -> Dict[ForagingDecisionType, float]:
    ...
    def weighted_random_choice(
        self, probabilities: Dict[ForagingDecisionType, float]
    ) -> ForagingDecisionType:
    ...
    def execute_decision(
        self,
        decision_type: ForagingDecisionType,
        context: ForagingContext,
        available_patches: List[Any],
    ) -> Optional[int]:
    ...
    def calculate_patch_utility(
        ...
        # Distance penalty
        distance_penalty = (
            memory.distance / 1000.0
        ) * self.distance_weight  # Normalize by 1km
        ...
        # Energy yield bonus
        energy_bonus = (
            min(1.0, memory.energy_yield / 50.0) * 0.1
        )  # Normalize by 50 units
        utility += energy_bonus

        return max(0.0, utility)

    def select_exploration_patch(
        self, available_patches: List[Any], context: ForagingContext
    ) -> Optional[int]:
    ...
    def select_least_visited_patch(self, available_patches: List[Any]) -> Optional[int]:
        ...
        for patch in available_patches:
            ...
            if patch_id in self.foraging_memories:
                visits = self.foraging_memories[patch_id].visits
                if visits < min_visits:
                    min_visits = visits
                    least_visited = patch_id
    ...
    def follow_dance_information(self, context: ForagingContext) -> Optional[int]:
        ...
        for dance_info in context.dance_information:
            ...
            patch_id_raw = getattr(dance_info, "patch_id", None)
            quality = float(getattr(dance_info, "quality", 0.5))
            urgency = float(getattr(dance_info, "urgency", 1.0))

            if patch_id_raw is None:
                continue

            patch_id = int(patch_id_raw)
    ...
    def select_alternative_resource_patch(
        self, context: ForagingContext
    ) -> Optional[int]:
    ...
    def follow_scent_trail(self, context: ForagingContext) -> Optional[int]:
        ...
        strongest_trail = max(
            context.scent_trails, key=lambda x: float(getattr(x, "strength", 0.0))
        )
        patch_id_raw = getattr(strongest_trail, "patch_id", None)
        return int(patch_id_raw) if patch_id_raw is not None else None
    ...
    def update_contextual_quality(
        self, memory: ForagingMemory, context: ForagingContext, success: bool
    ) -> None:
        ...
        memory.seasonal_quality[context.current_season] = max(
            0.1, min(2.0, memory.seasonal_quality[context.current_season])
        )
        ...
        memory.time_of_day_quality[context.current_hour] = max(
            0.1, min(2.0, memory.time_of_day_quality[context.current_hour])
        )
        ...
        memory.weather_quality[context.current_weather] = max(
            0.1, min(2.0, memory.weather_quality[context.current_weather])
        )
    ...
    def prune_weak_memories(self) -> None:
        ...
        sorted_memories = sorted(
            self.foraging_memories.items(), key=lambda x: x[1].memory_strength
        )
    ...
    def update_decision_success_rate(
        self, decision_type: ForagingDecisionType, success: bool
    ) -> None:
        ...
        self.success_rates[decision_type] = (1 - alpha) * self.success_rates[
            decision_type
        ] + alpha * new_value
    ...
    def get_foraging_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_success_rate": sum(self.success_rates.values())
    ...
    def _convert_to_patch_info(self, patches: List[Any]) -> List[PatchInfo]:
        ...
        for patch in patches:
            ...
            distance = getattr(
                patch, "distance", getattr(patch, "distance_from_hive", 100.0)
            )

            # Create quality metrics
            quality_metrics = {
                PatchQualityMetric.RESOURCE_DENSITY: getattr(
                    patch, "resource_density", 0.5
                ),
                PatchQualityMetric.SUGAR_CONCENTRATION: getattr(
                    patch, "sugar_concentration", 0.3
                ),
                PatchQualityMetric.ACCESSIBILITY: getattr(patch, "accessibility", 0.8),
                PatchQualityMetric.COMPETITION_LEVEL: getattr(
                    patch, "competition_level", 0.2
                ),
...
class ForagingTripManager(BaseModel):
    ...
    # Flight parameters
    base_flight_speed: float = Field(
        default=6.5, ge=0.1, le=20.0, description="Base flight speed m/s"
    )
    wind_effect_factor: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Wind effect on flight speed"
    )
    energy_per_meter_flight: float = Field(
        default=0.001, ge=0.0, le=0.01, description="Energy per meter of flight"
    )

    # Foraging parameters
    base_handling_time: float = Field(
        default=8.0, ge=1.0, le=30.0, description="Base flower handling time (seconds)"
    )
    inter_flower_time: float = Field(
        default=2.0, ge=0.1, le=10.0, description="Time between flowers (seconds)"
    )
    energy_per_second_foraging: float = Field(
        default=0.05, ge=0.0, le=0.2, description="Energy per second foraging"
    )

    # Resource parameters
    base_nectar_per_flower: float = Field(
        default=0.5, ge=0.1, le=2.0, description="Base nectar per flower (mg)"
    )
    base_pollen_per_flower: float = Field(
        default=0.3, ge=0.1, le=1.0, description="Base pollen per flower (mg)"
    )
    crop_capacity: float = Field(
        default=70.0, ge=10.0, le=150.0, description="Bee crop capacity (mg)"
    )
    pollen_capacity: float = Field(
        default=20.0, ge=5.0, le=50.0, description="Pollen basket capacity (mg)"
    )

    # Trip history
    completed_trips: List[ForagingTripLifecycle] = Field(
        default_factory=list, description="Completed trip history"
    )

    def simulate_complete_foraging_trip(
        self,
        patch_id: int,
        context: ForagingContext,
        bee_energy: float,
        bee_physiology: Any = None,
    ) -> ForagingTripLifecycle:
        """Simulate complete foraging trip with detailed lifecycle"""

        # Get patch information
        patch_distance = (
            context.patch_distances.get(patch_id, 100.0)
            if context.patch_distances
            else 100.0
        )
        flower_density = (
            context.flower_density.get(patch_id, 1.0) if context.flower_density else 1.0
        )

        # Calculate travel time to patch
        travel_time_to_patch = self.calculate_travel_time(
            patch_distance, context.wind_speed, "to_patch"
        )

        # Calculate foraging parameters
        flowers_available = max(
            1, int(flower_density * 50)
        )  # Density affects available flowers
        ...
        energy_consumed_travel = self.calculate_travel_energy(
            patch_distance, context.wind_speed, trip_result["load_factor"]
        )
        energy_consumed_foraging = self.calculate_foraging_energy(
            trip_result["flowers_visited"], handling_time, inter_flower_time
        )

        # Create complete trip lifecycle
        total_duration = (
            travel_time_to_patch + trip_result["foraging_time"] + travel_time_to_hive
        )
        net_energy_gain = (
            trip_result["energy_gained"]
            - energy_consumed_travel
            - energy_consumed_foraging
        )
    ...
    def calculate_handling_time(
        self, context: ForagingContext, bee_physiology: Any = None
    ) -> float:
        ...
        if bee_physiology:
            ...
            corolla_depth = (
                context.metadata.get("average_corolla_depth", 6.0)
                if hasattr(context, "metadata")
                else 6.0
            )

            # Calculate proboscis-corolla match efficiency
            match_efficiency = self.calculate_proboscis_corolla_match(
                proboscis_length, corolla_depth
            )
    ...
    def calculate_proboscis_corolla_match(
        self, proboscis_length: float, corolla_depth: float
    ) -> float:
        ...
        if actual_ratio < 1.0:
            ...
            efficiency = max(
                0.6, 1.0 - excess * 0.1
            )  # Decrease efficiency with excess length

        return min(1.0, max(0.1, efficiency))

    def calculate_resource_extraction_efficiency(
        self, bee_physiology: Any, context: ForagingContext
    ) -> float:
    ...
    def can_access_flower(
        self, bee_physiology: Any, flower_morphology: Dict[str, float]
    ) -> bool:
    ...
    def calculate_inter_flower_time(
        self, context: ForagingContext, flower_density: float
    ) -> float:
    ...
    def simulate_flower_visitation(
        self,
        flowers_available: int,
        handling_time: float,
        inter_flower_time: float,
        context: ForagingContext,
        bee_energy: float,
        patch_id: int,
        bee_physiology: Any = None,
        ...
        if bee_physiology:
            extraction_efficiency = self.calculate_resource_extraction_efficiency(
                bee_physiology, context
            )
        ...
        for flower_num in range(flowers_available):
            ...
            if (
                nectar_collected >= crop_capacity
                and pollen_collected >= pollen_capacity
            ):
                break

            # Check energy levels
            energy_cost = self.energy_per_second_foraging * (
            ...
            if (
                nectar_collected < crop_capacity and random.random() < 0.8
            ):  # 80% nectar success
                nectar_this_flower = min(
                    nectar_per_flower, crop_capacity - nectar_collected
                )
                nectar_collected += nectar_this_flower
                energy_gained += nectar_this_flower * 0.5  # Energy content

            if (
                pollen_collected < pollen_capacity and random.random() < 0.6
            ):  # 60% pollen success
                pollen_this_flower = min(
                    pollen_per_flower, pollen_capacity - pollen_collected
                )
    ...
    def calculate_travel_energy(
        self, distance: float, wind_speed: float, load_factor: float
    ) -> float:
    ...
    def calculate_foraging_energy(
        self, flowers_visited: int, handling_time: float, inter_flower_time: float
    ) -> float:
        """Calculate energy consumed during foraging"""

        # Energy for flower handling
        handling_energy = (
            flowers_visited * handling_time * self.energy_per_second_foraging
        )

        # Energy for inter-flower movement
        movement_energy = (
            (flowers_visited - 1) * inter_flower_time * self.energy_per_second_foraging
        )
    ...
    def get_trip_efficiency_metrics(self) -> Dict[str, float]:
        ...
        # Calculate averages
        avg_trip_duration = sum(
            trip.total_trip_duration for trip in self.completed_trips
        ) / len(self.completed_trips)
        avg_flowers_visited = sum(
            trip.flowers_visited for trip in self.completed_trips
        ) / len(self.completed_trips)
        avg_net_energy_gain = sum(
            trip.net_energy_gain for trip in self.completed_trips
        ) / len(self.completed_trips)
        avg_energy_efficiency = sum(
            trip.energy_gained
            / max(1, trip.energy_consumed_travel + trip.energy_consumed_foraging)
            for trip in self.completed_trips
        ) / len(self.completed_trips)

        # Calculate success rate (positive energy gain)
        successful_trips = sum(
            1 for trip in self.completed_trips if trip.net_energy_gain > 0
        )
    ...
    def optimize_foraging_parameters(self) -> None:
        ...
        successful_trips = [
            trip for trip in self.completed_trips if trip.net_energy_gain > 0
        ]
        unsuccessful_trips = [
            trip for trip in self.completed_trips if trip.net_energy_gain <= 0
        ]
        ...
        avg_successful_flowers = sum(
            trip.flowers_visited for trip in successful_trips
        ) / len(successful_trips)
        avg_unsuccessful_flowers = (
            sum(trip.flowers_visited for trip in unsuccessful_trips)
            / len(unsuccessful_trips)
            if unsuccessful_trips
            else 0
        )
    ...
    def predict_trip_outcome(
        self, patch_id: int, context: ForagingContext, bee_energy: float
    ) -> Dict[str, float]:
        ...
        # Quick estimates
        travel_time = (
            self.calculate_travel_time(patch_distance, context.wind_speed, "to_patch")
            * 2
        )
        estimated_flowers = min(20, int(flower_density * 10))  # Conservative estimate
        foraging_time = estimated_flowers * (
            self.base_handling_time + self.inter_flower_time
        )

        # Energy estimates
        travel_energy = self.calculate_travel_energy(
            patch_distance, context.wind_speed, 0.5
        )
        foraging_energy = self.calculate_foraging_energy(
            estimated_flowers, self.base_handling_time, self.inter_flower_time
        )
        ...
        return {
            ...
            "predicted_energy_gain": estimated_energy_gain
            - travel_energy
            - foraging_energy,
            "predicted_efficiency": estimated_energy_gain
            / max(1, travel_energy + foraging_energy),
            "risk_level": self.calculate_trip_risk(patch_distance, context, bee_energy),
        }

    def calculate_trip_risk(
        self, distance: float, context: ForagingContext, bee_energy: float
    ) -> float:
