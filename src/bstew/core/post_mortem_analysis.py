...
class PostMortemAnalyzer(BaseModel):
    ...
    def analyze_simulation(
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    f"Post-mortem analysis completed in {self.analysis_duration:.2f} seconds"
                )
    ...
    def _extract_simulation_metrics(
        ...
        duration_days = model_data.get(
            "current_day", model_data.get("duration_days", 0)
        )
        ...
        if population_series:
            ...
            population_std_deviation = (
                statistics.stdev(population_series)
            ...
            if initial_population > 0 and duration_days > 0:
                population_growth_rate = (final_population - initial_population) / (
                    initial_population * duration_days
                )
            else:
        ...
        colonies_survived = len(
            [c for c in colonies_data if c.get("status") != "collapsed"]
        )
        ...
        if weather_data:
            ...
            weather_stress_days = len(
                [
                    w
                    for w in weather_data
                    if w.get("temperature", 20.0) < 10
                    or w.get("temperature", 20.0) > 30
                ]
            )
        else:
        ...
        seasonal_effects = model_data.get(
            "seasonal_effects",
        ...
        steps_per_second = (
            duration_days / max(1, simulation_time_seconds)
    ...
    def _analyze_colony_outcomes(
        ...
        for colony_data in colonies_data:
            ...
            # Timeline analysis
            lifespan_days = colony_data.get(
                "lifespan", model_data.get("current_day", 0)
            )
            ...
            if health_history:
                ...
                if len(health_history) > 1:
                    health_decline_rate = (
                        final_health_score - initial_health_score
                    ) / len(health_history)
                else:
    ...
    def _identify_primary_cause(
        self, colony_data: Dict[str, Any], failure_mode: Optional[FailureMode]
    ) -> Optional[str]:
    ...
    def _identify_intervention_opportunities(
        ...
        if collapse_day and first_alert_day:
            ...
            if intervention_window > 5:
                opportunities.append(
                    f"Early intervention possible {intervention_window} days before collapse"
                )
        ...
        if min_energy < 50.0:
            opportunities.append(
                "Resource supplementation could have prevented starvation"
            )
    ...
    def _create_trend_segments(
        self, values: List[float], change_points: List[int]
    ) -> List[Dict[str, Any]]:
        ...
        for start, end in zip(start_points, end_points):
            ...
            if len(segment_values) > 1:
                segment_slope = np.polyfit(
                    range(len(segment_values)), segment_values, 1
                )[0]
                segments.append(
                    {
                        "start_index": start,
    ...
    def _generate_performance_insights(self) -> List[PerformanceInsight]:
        ...
        population_cv = self.simulation_metrics.population_std_deviation / max(
            1.0, self.simulation_metrics.avg_population
        )
        ...
        if (
            self.simulation_metrics.weather_stress_days
            > self.simulation_metrics.duration_days * 0.2
        ):
            insights.append(
                PerformanceInsight(
    ...
    def _perform_failure_analysis(self) -> Dict[str, Any]:
        ...
        failure_mode_counts = Counter(
            [c.failure_mode.value for c in failed_colonies if c.failure_mode]
        )
        ...
        failure_analysis["common_patterns"] = self._identify_failure_patterns(
            failed_colonies
        )

        # Generate prevention strategies
        failure_analysis["prevention_strategies"] = (
            self._generate_prevention_strategies(failed_colonies)
        )
    ...
    def _create_failure_distribution(self, failure_days: List[int]) -> Dict[str, int]:
        ...
        duration = (
            self.simulation_metrics.duration_days if self.simulation_metrics else 365
        )
    ...
    def _identify_failure_patterns(
        self, failed_colonies: List[ColonyOutcome]
    ) -> List[str]:
        ...
        if species_failures:
            ...
            if most_vulnerable[1] > len(failed_colonies) * 0.5:
                patterns.append(
                    f"Species {most_vulnerable[0]} shows higher failure rate"
                )
    ...
    def _generate_prevention_strategies(
        ...
        primary_causes = Counter(
            [c.primary_cause for c in failed_colonies if c.primary_cause]
        )

        for cause, count in primary_causes.most_common(3):
            if "starvation" in cause.lower():
                strategies.append(
                    "Implement resource monitoring and supplemental feeding protocols"
                )
            elif "disease" in cause.lower():
                strategies.append(
                    "Establish disease surveillance and treatment programs"
                )
            elif "population" in cause.lower():
                strategies.append(
                    "Monitor population dynamics and implement breeding support"
                )
        ...
        if any("rapid" in p for p in self._identify_failure_patterns(failed_colonies)):
            strategies.append(
                "Implement early intervention protocols for rapid decline detection"
            )

        if any(
            "health decline" in p
            for p in self._identify_failure_patterns(failed_colonies)
        ):
            strategies.append(
                "Enhance health monitoring frequency during critical periods"
            )
    ...
    def _perform_comparative_analysis(self) -> Dict[str, Any]:
        ...
        successful_colonies = [
            c for c in self.colony_outcomes if c.outcome == OutcomeCategory.SUCCESS
        ]
        failed_colonies = [
            c
            for c in self.colony_outcomes
        ...
        return {
            "success_factors": self._identify_success_factors(
                successful_colonies, failed_colonies
            ),
            "performance_differences": self._analyze_performance_differences(
                successful_colonies, failed_colonies
            ),
    ...
    def _identify_success_factors(
        self, successful: List[ColonyOutcome], failed: List[ColonyOutcome]
    ) -> List[str]:
        ...
        success_avg_health = statistics.mean(
            [c.initial_health_score for c in successful]
        )
        ...
        success_avg_foraging = statistics.mean(
            [c.avg_foraging_efficiency for c in successful]
        )
        failed_avg_foraging = statistics.mean(
            [c.avg_foraging_efficiency for c in failed]
        )

        if success_avg_foraging > failed_avg_foraging * 1.3:
            factors.append("Superior foraging efficiency is key to survival")

        success_avg_population = statistics.mean(
            [c.peak_population for c in successful]
        )
    ...
    def _analyze_performance_differences(
        ...
        return {
            "health_scores": {
                "successful_avg": statistics.mean(
                    [c.initial_health_score for c in successful]
                ),
            ...
            "population_metrics": {
                "successful_peak": statistics.mean(
                    [c.peak_population for c in successful]
                ),
                "failed_peak": statistics.mean([c.peak_population for c in failed]),
                "successful_final": statistics.mean(
                    [c.final_population for c in successful]
                ),
    ...
    def _identify_key_differentiators(
        ...
        if success_energy > failed_energy * 2:
            differentiators.append(
                "Successful colonies collect significantly more energy"
            )
    ...
    def _generate_executive_summary(self) -> Dict[str, Any]:
        ...
        key_findings.append(
            f"Colony survival rate: {self.simulation_metrics.survival_rate:.1%}"
        )
        key_findings.append(
            f"Average health score: {self.simulation_metrics.avg_health_score:.2f}"
        )
        key_findings.append(
            f"Foraging efficiency: {self.simulation_metrics.avg_foraging_efficiency:.2f}"
        )

        if self.simulation_metrics.colonies_collapsed > 0:
            key_findings.append(
                f"{self.simulation_metrics.colonies_collapsed} colonies collapsed"
            )
        ...
        return {
            ...
            "critical_issues": len(
                [i for i in self.performance_insights if i.priority == "high"]
            ),
            "analysis_confidence": "High"
            if len(self.colony_outcomes) >= 3
            else "Medium",
        }


def generate_comprehensive_report(
    analysis_results: Dict[str, Any], output_format: str = "json"
...
def _generate_markdown_report(analysis_results: Dict[str, Any]) -> str:
    ...
    report.append(
        f"**Performance Grade:** {exec_summary.get('performance_grade', 'N/A')}\n"
    )
    report.append(
        f"**Simulation Duration:** {exec_summary.get('simulation_duration', 'N/A')}\n"
    )
    ...
    if sim_metrics:
        ...
        report.append(
            f"- **Population Growth Rate:** {sim_metrics.population_growth_rate:.3f}\n"
        )
        report.append(f"- **Survival Rate:** {sim_metrics.survival_rate:.1%}\n")
        report.append(
            f"- **Average Health Score:** {sim_metrics.avg_health_score:.2f}\n"
        )
        report.append(
            f"- **Foraging Efficiency:** {sim_metrics.avg_foraging_efficiency:.2f}\n"
        )
    ...
    if insights:
        ...
        for insight in insights:
            report.append(
                f"### {insight.title} ({insight.priority.upper()} Priority)\n"
            )
...
def _generate_html_report(analysis_results: Dict[str, Any]) -> str:
    ...
    if sim_metrics:
        ...
        """
        html += (
            f"<p><strong>Survival Rate:</strong> {sim_metrics.survival_rate:.1%}</p>"
        )
