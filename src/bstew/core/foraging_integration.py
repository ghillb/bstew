...
from .patch_selection import (
    AdvancedPatchSelector,
    PatchInfo,
    ResourceType as PatchResourceType,
)

# REMOVED: ForagingCommunicationIntegrator (honey bee behavior)
...
class IntegratedForagingSystem(BaseModel):
    ...
    decision_engine: ForagingDecisionEngine = Field(
        default_factory=ForagingDecisionEngine
    )
    ...
    # REMOVED: communication_integrator (honey bee behavior)
    bumblebee_communication_system: BumblebeeCommunicationSystem = Field(
        default_factory=BumblebeeCommunicationSystem
    )
    proboscis_system: ProboscisCorollaSystem = Field(
        default_factory=ProboscisCorollaSystem
    )
    ...
    system_performance_metrics: Dict[int, Any] = Field(default_factory=dict)
    colony_foraging_strategies: Dict[int, ForagingMode] = Field(default_factory=dict)

    # Configuration
    max_session_duration: float = Field(
        default=480.0, description="Maximum foraging session duration (minutes)"
    )
    performance_update_interval: int = Field(
        default=100, description="Steps between performance updates"
    )
    ...
    def initialize_colony_foraging(
        self,
        colony_id: int,
        colony_state: Dict[str, Any],
        landscape_patches: List[PatchInfo],
    ) -> None:
    ...
    def execute_foraging_step(
        self,
        bee_id: int,
        colony_id: int,
        bee_state: Dict[str, Any],
        colony_state: Dict[str, Any],
    ...
    def _evaluate_foraging_decision(
        self,
        bee_id: int,
        colony_id: int,
        bee_state: Dict[str, Any],
        ...
        if isinstance(decision_result, dict):
            ...
            if selected_patch_id is not None:
                selected_patch = next(
                    (p for p in available_patches if p.patch_id == selected_patch_id),
                    None,
                )
                if selected_patch:
            ...
            foraging_decision = {
                ...
                "confidence": 0.7
                if selected_patches
                else 0.3,  # Default confidence based on whether patch was found
            }

        # Apply colony strategy influence
        colony_strategy = self.colony_foraging_strategies.get(
            colony_id, ForagingMode.MIXED_STRATEGY
        )
        decision_probability = self._apply_strategy_influence(
            foraging_decision, colony_strategy
        )
    ...
    def _continue_foraging_session(
        self,
        bee_id: int,
        colony_id: int,
        bee_state: Dict[str, Any],
        ...
        if (
            ...
            or bee_state.get("energy", 100.0) < 20.0
            or session_data["session_status"] != "active"
        ):
            return self._end_foraging_session(
                bee_id, colony_id, bee_state, colony_state
            )
    ...
    def _execute_foraging_trip(
        ...
        available_patches = [
            p
            for p in self.patch_database.values()
        ...
        selection_result = self.patch_selector.select_optimal_patches(
            available_patches,
            bee_species,
            bee_state.get("energy", 100.0),
            bee_state.get("memory", {}),
            current_conditions,
        )
        ...
        accessibility_results = self._evaluate_flower_accessibility(
            bee_species, target_patch
        )

        # Calculate patch quality (currently unused but may be needed for future enhancements)
        # patch_quality = patch_qualities.get(
        #     target_patch.patch_id, target_patch.success_rate
        # )

        # Simulate complete foraging trip
        from ..core.foraging_algorithms import ForagingContext
        ...
        context = ForagingContext(
            ...
            current_hour=environmental_context.get("hour", 12),
            current_weather=environmental_context.get("weather", "clear"),
            colony_energy_level=colony_state.get("energy_level", 100.0),
            patch_competition=environmental_context.get("patch_competition", {}),
            dance_information=colony_state.get("dance_information", []),
            scent_trails=colony_state.get("scent_trails", []),
            wind_speed=environmental_context.get("wind_speed", 0.0),
            temperature=environmental_context.get("temperature", 20.0),
            flower_density=environmental_context.get("flower_density", {}),
            patch_distances=environmental_context.get("patch_distances", {}),
        )

        # Get patch ID
        patch_id = getattr(target_patch, "patch_id", 0)
        ...
        session_data["total_energy_consumed"] += (
            trip_lifecycle.energy_consumed_travel
            + trip_lifecycle.energy_consumed_foraging
        )
        session_data["total_energy_gained"] += trip_lifecycle.energy_gained
        session_data["total_distance"] += (
            trip_lifecycle.travel_time_to_patch + trip_lifecycle.travel_time_to_hive
        )
        ...
        trip_result = {
            ...
            "selection_analytics": (
                selection_result.get("selection_summary", {})
                if isinstance(selection_result, dict)
                else {}
            ),
            "trip_efficiency": trip_lifecycle.net_energy_gain
            / max(1.0, trip_lifecycle.total_trip_duration),
            "success": trip_lifecycle.net_energy_gain > 0,
        }

        return trip_result

    def _evaluate_flower_accessibility(
        self, bee_species: str, patch: PatchInfo
    ) -> Dict[str, Any]:
        """Evaluate flower accessibility for bee species"""

        # Create mock flower species from patch info
        from .patch_selection import PatchQualityMetric

        mock_flower = FlowerSpecies(
            name=f"Patch_{patch.patch_id}_flower",
            bloom_start=120,  # May (day 120)
            bloom_end=240,  # August (day 240)
            corolla_depth_mm=patch.quality_metrics.get(
                PatchQualityMetric.COROLLA_DEPTH, 5.0
            ),
            corolla_width_mm=patch.quality_metrics.get(
                PatchQualityMetric.COROLLA_WIDTH, 3.0
            ),
        ...
        accessibility_result = self.proboscis_system.calculate_accessibility(
            proboscis, mock_flower
        )
        efficiency_modifiers = self.proboscis_system.get_foraging_efficiency_modifier(
            bee_species, mock_flower
        )
    ...
    def _process_trip_communication(
        self,
        bee_id: int,
        colony_id: int,
        patch: PatchInfo,
        ...
        foraging_trip_result = {
            ...
            "trip_efficiency": trip_lifecycle.net_energy_gain
            / max(1.0, trip_lifecycle.total_trip_duration),
            "patch_quality": trip_lifecycle.net_energy_gain
            / max(1.0, trip_lifecycle.energy_gained),
            "resource_abundance": trip_lifecycle.flowers_visited * 10.0,  # Simplified
            "energy_cost": trip_lifecycle.energy_consumed_travel
            + trip_lifecycle.energy_consumed_foraging,
            "patch_id": patch.patch_id,
        }

        # Use bumblebee communication system to update individual memory
        if trip_lifecycle.net_energy_gain > 0:
            # Convert patch resource type to communication system resource type
            from .resource_collection import ResourceType as CommResourceType

            resource_type_mapping = {
                PatchResourceType.NECTAR: CommResourceType.NECTAR,
                PatchResourceType.POLLEN: CommResourceType.POLLEN,
                PatchResourceType.MIXED: CommResourceType.NECTAR,  # Default to nectar for mixed
                PatchResourceType.WATER: CommResourceType.NECTAR,  # Water mapped to nectar type
                PatchResourceType.PROPOLIS: CommResourceType.POLLEN,  # Propolis mapped to pollen type
            }

            comm_resource_type = resource_type_mapping.get(
                patch.resource_type, CommResourceType.NECTAR
            )

            self.bumblebee_communication_system.update_bee_memory(
                bee_id=bee_id,
                patch_id=str(patch.patch_id),
                location=patch.location,  # Already a tuple
                resource_type=comm_resource_type,
                reward=foraging_trip_result.get("energy_gained", 0.0),
                energy_cost=foraging_trip_result.get("energy_cost", 0.0),
                current_time=foraging_trip_result.get("timestamp", 0.0),
            )

        return {
            "communication_type": "individual_memory_based",
            "memory_updated": trip_lifecycle.net_energy_gain > 0,
            "patch_id": patch.patch_id,
            "communication_triggered": trip_lifecycle.net_energy_gain > 0,
        }

    def _end_foraging_session(
        self,
        bee_id: int,
        colony_id: int,
        bee_state: Dict[str, Any],
        ...
        if total_trips > 0:
            trip_efficiency = (
                sum(t.net_energy_gain for t in session_data["completed_trips"])
                / total_trips
            )
            energy_efficiency = session_data["total_energy_gained"] / max(
                1.0, session_data["total_energy_consumed"]
            )
            time_efficiency = session_data["total_energy_gained"] / max(
        ...
        success_score = (
            trip_efficiency * 0.4 + energy_efficiency * 0.3 + time_efficiency * 0.3
        )
        ...
        session_result = ForagingSessionResult(
            ...
            net_energy_result=session_data["total_energy_gained"]
            - session_data["total_energy_consumed"],
            ...
            dances_performed=len(
                [
                    e
                    for e in session_data["communication_events"]
                    if e.get("dance_performed", False)
                ]
            ),
            recruits_obtained=0,  # Would be calculated from communication system
            information_shared=len(
                [
                    e
                    for e in session_data["communication_events"]
                    if e.get("information_shared", False)
                ]
            ),
    ...
    def _apply_strategy_influence(
        self, foraging_decision: Dict[str, Any], strategy: ForagingMode
    ) -> float:
        """Apply colony foraging strategy influence on decision probability"""

        base_probability = float(foraging_decision.get("confidence", 0.5))
        decision_type = foraging_decision.get(
    ...
    def _update_performance_metrics(
        self, colony_id: int, session_result: ForagingSessionResult
    ) -> None:
        """Update colony foraging performance metrics"""

        if colony_id not in self.system_performance_metrics:
            self.system_performance_metrics[colony_id] = {
                "total_foraging_sessions": 0,
                "successful_sessions": 0,
                "average_efficiency": 0.0,
        ...
        metrics = self.system_performance_metrics[colony_id]
        ...
        metrics["average_efficiency"] = (
            (current_avg * (session_count - 1)) + session_result.overall_success_score
        ) / session_count

        # Update communication metrics
        metrics["communication_events"] += (
            session_result.dances_performed + session_result.information_shared
        )
    ...
    def get_colony_foraging_analytics(self, colony_id: int) -> Dict[str, Any]:
        ...
        successful_sessions = sum(
            1 for s in colony_sessions if s.overall_success_score > 0.5
        )

        avg_efficiency = (
            sum(s.overall_success_score for s in colony_sessions) / total_sessions
        )
        avg_energy_gain = (
            sum(s.net_energy_result for s in colony_sessions) / total_sessions
        )
        total_resources = sum(
            s.nectar_collected + s.pollen_collected for s in colony_sessions
        )

        # Recent performance (last 20 sessions)
        recent_sessions = colony_sessions[-20:]
        recent_avg_efficiency = (
            sum(s.overall_success_score for s in recent_sessions) / len(recent_sessions)
            if recent_sessions
            else 0.0
        )
        ...
        return {
            "colony_id": colony_id,
            "foraging_strategy": self.colony_foraging_strategies.get(
                colony_id, ForagingMode.MIXED_STRATEGY
            ).value,
            "session_statistics": {
            ...
            "resource_collection": {
                ...
                "nectar_vs_pollen_ratio": self._calculate_resource_ratio(
                    colony_sessions
                ),
            },
            "patch_utilization": {
                "unique_patches_visited": len(
                    set().union(*[s.patches_visited for s in colony_sessions])
                ),
                "avg_patches_per_session": sum(
                    len(s.patches_visited) for s in colony_sessions
                )
                / total_sessions,
            },
            "communication_activity": {
                "total_dances": sum(s.dances_performed for s in colony_sessions),
                "total_information_sharing": sum(
                    s.information_shared for s in colony_sessions
                ),
                "communication_rate": sum(
                    s.dances_performed + s.information_shared for s in colony_sessions
                )
                / total_sessions,
            },
            ...
            "active_sessions": len(
                [
                    s
                    for s in self.active_foraging_sessions.values()
                    if s["colony_id"] == colony_id
                ]
            ),
        }

    def _calculate_resource_ratio(
        self, sessions: List[ForagingSessionResult]
    ) -> Dict[str, float]:
        ...
        if total_resources > 0:
            ...
            return {
                "nectar_percentage": 0.0,
                "pollen_percentage": 0.0,
                "nectar_pollen_ratio": 0.0,
            }
    ...
    def get_system_wide_analytics(self) -> Dict[str, Any]:
        ...
        for colony_id in all_colonies:
            system_analytics["colony_analytics"][colony_id] = (
                self.get_colony_foraging_analytics(colony_id)
            )
        ...
        if self.foraging_history:
            system_analytics["system_performance"] = {
                "overall_success_rate": sum(
                    1 for s in self.foraging_history if s.overall_success_score > 0.5
                )
                / len(self.foraging_history),
                "average_efficiency": sum(
                    s.overall_success_score for s in self.foraging_history
                )
                / len(self.foraging_history),
                "total_energy_collected": sum(
                    s.net_energy_result for s in self.foraging_history
                ),
                "total_resources_collected": sum(
                    s.nectar_collected + s.pollen_collected
                    for s in self.foraging_history
                ),
                "communication_effectiveness": self.bumblebee_communication_system.get_communication_summary(),
    ...
    def _calculate_patch_utilization_stats(self) -> Dict[str, Any]:
        ...
        patch_visit_counts: defaultdict[int, int] = defaultdict(int)
        ...
        if patch_visit_counts:
            ...
            most_successful_patch = max(
                avg_success_by_patch.items(), key=lambda x: x[1]
            )

            return {
                "total_unique_patches_visited": len(patch_visit_counts),
                "most_visited_patch": {
                    "patch_id": most_visited_patch[0],
                    "visit_count": most_visited_patch[1],
                },
                "most_successful_patch": {
                    "patch_id": most_successful_patch[0],
                    "success_rate": most_successful_patch[1],
                },
                "average_visits_per_patch": sum(patch_visit_counts.values())
                / len(patch_visit_counts),
                "patch_utilization_rate": len(patch_visit_counts)
                / len(self.patch_database),
            }
        else:
