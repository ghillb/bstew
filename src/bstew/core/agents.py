...
class ForagingMemory(BaseModel):
    ...
    seasonal_quality: Dict[str, float] = Field(default_factory=dict, description="Quality by season")
    time_of_day_quality: Dict[int, float] = Field(default_factory=dict, description="Quality by hour")
    weather_quality: Dict[str, float] = Field(default_factory=dict, description="Quality by weather")
    energy_yield: float = Field(default=0.0, ge=0.0, description="Average energy yield per visit")
    travel_time: float = Field(default=0.0, ge=0.0, description="Travel time to patch")
    competition_level: float = Field(default=0.0, ge=0.0, le=1.0, description="Competition intensity")
    memory_strength: float = Field(default=1.0, ge=0.0, le=1.0, description="Memory decay factor")
    ...
    def update_memory_strength(self, days_since_visit: int) -> None:
        ...
        self.memory_strength = max(0.1, self.memory_strength * (decay_rate ** days_since_visit))
...
class BeeAgent(mesa.Agent):
    ...
    def _initialize_physiological_parameters(self) -> None:
        ...
        def safe_gauss(mean: float, std: float) -> float:
            try:
                ...
                if hasattr(result, '_mock_name') or not isinstance(result, (int, float)):
        ...
        if self.role == BeeRole.QUEEN:
            ...
            self.proboscis_length_mm = self.glossaLength_mm + 2.0  # Include other mouthparts
            ...
            self.glossaLength_mm = safe_gauss(13.5, 1.2)  # 11-16mm (longer for better access)
            ...
            self.glossaLength_mm = safe_gauss(8.0, 0.8)  # 6.4-9.6mm (shorter, less foraging)
            self.cropvolume_myl = safe_gauss(25.0, 5.0)  # 15-35μl (smaller, less storage needed)
        ...
        self.proboscis_diameter_mm = max(0.1, self.proboscis_diameter_mm)  # Minimum 0.1mm
    ...
    def _apply_genetic_physiological_effects(self) -> None:
        ...
        self.cropvolume_myl *= (size_factor * 0.7 + crop_factor * 0.3)  # Weighted combination
    ...
    def calculate_state_specific_energy_consumption(self) -> float:
        ...
        base_metabolic_rate = self.weight_mg * 0.01  # Base rate proportional to weight
        ...
        multiplier = state_multipliers.get(current_state, 0.6)  # Default moderate activity
        
        # Additional factors based on physiology
        flight_cost_factor = (self.weight_mg / 160.0) ** 0.75  # Allometric scaling for flight
        foraging_efficiency_factor = 1.0 / max(0.1, self.foraging_efficiency)  # Less efficient = more energy
        
        # Calculate total energy consumption
        energy_consumption = base_metabolic_rate * multiplier * flight_cost_factor * foraging_efficiency_factor
    ...
    def calculate_foraging_capacity(self) -> Dict[str, float]:
        ...
        return {
            'max_nectar_load_myl': self.cropvolume_myl * 0.8,  # 80% of crop volume usable
            'max_pollen_load_mg': self.weight_mg * 0.15,  # 15% of body weight in pollen
            'flight_range_m': (self.weight_mg / 200.0) ** 0.5 * 1000,  # Flight range based on size
            'handling_time_factor': 1.0 / (self.glossaLength_mm / 12.0),  # Longer tongue = faster handling
        }

    def can_access_flower(self, corolla_depth_mm: float, corolla_width_mm: float) -> bool:
        """Check if bee can access a flower based on proboscis-corolla matching"""
        # Check if proboscis is long enough to reach nectar
        can_reach = self.proboscis_length_mm >= corolla_depth_mm * 0.9  # 90% reach required
        
        # Check if proboscis fits in corolla opening
        can_fit = self.proboscis_diameter_mm <= corolla_width_mm * 0.8  # 80% fit tolerance
        
        # Check if mandibles can handle flower (for pollen collection)
        can_handle = self.mandible_width_mm <= corolla_width_mm + 2.0  # Some overhang allowed
        
        return can_reach and can_fit and can_handle

    def calculate_resource_extraction_efficiency(self, flower_type: Dict[str, float]) -> float:
        ...
        concentration_factor = 0.5 + (nectar_concentration * 1.5)  # Easier to extract concentrated nectar
    ...
    def update_activity_state(self) -> None:
        ...
        if self.activity_state_machine and hasattr(self.model, 'schedule'):
            ...
            self.activity_state_machine.update_bee_activity(self.unique_id, current_step)
    ...
    def get_current_conditions(self) -> Dict[str, Any]:
        ...
        if self.role == BeeRole.FORAGER:
            conditions.update({
                "patch_found": self.target_patch is not None if hasattr(self, 'target_patch') else False,
                "resources_available": True,  # Simplified
                "collection_successful": True,  # Simplified
                "resource_quality": "high" if self.model.random.random() > 0.3 else "low"
            })
        elif self.role == BeeRole.NURSE:
            conditions.update({
                "brood_present": self.colony.get_brood_count() > 0 if self.colony else False
            })
    ...
    def transition_to_state(self, new_state: BeeStatus, current_step: int) -> None:
        ...
        if self.activity_state_machine:
            self.activity_state_machine.transition_bee_state(self.unique_id, new_state, current_step)
    ...
    def get_enhanced_current_conditions(self, tracker) -> Dict[str, Any]:
        ...
        if self.colony:
            ...
            conditions['brood_needs_care'] = getattr(self.colony, 'brood_needs_care', False)
            conditions['construction_needed'] = getattr(self.colony, 'needs_construction', False)
        ...
        conditions['collection_successful'] = True  # Will be updated by specific behaviors
    ...
    def evaluate_activity_success(self, completed_state: BeeStatus) -> bool:
        ...
        if completed_state in [BeeStatus.COLLECT_NECTAR, BeeStatus.COLLECT_POLLEN]:
            ...
            return self.is_carrying_capacity_full() if hasattr(self, 'is_carrying_capacity_full') else True
            ...
            return self.colony.get_brood_count() > 0 if self.colony and hasattr(self.colony, 'get_brood_count') else True
            ...
            return getattr(self.colony, 'needs_construction', True) if self.colony else True
        
        # Default to success for other activities
        return True
    
    def handle_role_specific_transition(self, old_state: BeeStatus, new_state: BeeStatus) -> None:
        ...
        # Forager-specific behaviors
            ...
            if new_state == BeeStatus.DANCING:
                ...
                if hasattr(self, 'current_foraging_target') and self.current_foraging_target:
                    patch_id = self.current_foraging_target
                    resources = getattr(self, 'unload_resources', lambda: {'nectar': 0, 'pollen': 0})()
    ...
    def get_activity_factor(self) -> float:
        ...
        if self.activity_state_machine and self.status in self.activity_state_machine.state_configs:
    ...
    def make_foraging_decision(self, available_patches: List[Any]) -> Tuple[str, Optional[int]]:
        ...
        decision_type, target_patch = self.foraging_decision_engine.make_foraging_decision(
    ...
    def update_foraging_memory(self, patch_id: int, success: bool, energy_gained: float) -> None:
    ...
    def get_foraging_summary(self) -> Dict[str, Any]:
        ...
        if self.foraging_decision_engine:
            return self.foraging_decision_engine.get_foraging_summary()
    ...
    def record_foraging_success(self, patch_id: int, success: bool, energy_gained: float) -> None:
    ...
    def record_dance_performance(self, dance_type: str, duration: int, followers: int) -> None:
    ...
    def get_comprehensive_metrics(self) -> Dict[str, Any]:
        ...
        if self.activity_state_machine:
            activity_summary = self.activity_state_machine.get_activity_summary(self.unique_id)
        ...
        if self.genotype:
            base_metrics.update({
                "sex": self.genotype.sex.value if hasattr(self.genotype.sex, 'value') else str(self.genotype.sex),
                "ploidy": self.genotype.ploidy.value if hasattr(self.genotype.ploidy, 'value') else str(self.genotype.ploidy),
    ...
    def attempt_resource_collection(self, patch: Any, resource_type: str = "nectar") -> Tuple[str, float, Dict[str, Any]]:
        ...
        self.resource_collector.update_bee_physiology(
            self.bee_physiology, result, amount, res_type, details.get("handling_time", 3.0)
        )
    ...
    def _get_environmental_conditions(self) -> Dict[str, float]:
        ...
        if hasattr(self.model, 'get_environmental_conditions'):
            return self.model.get_environmental_conditions()
    ...
    def get_crop_load_percentage(self) -> float:
        ...
        return (self.bee_physiology.current_crop_volume / self.bee_physiology.crop_capacity) * 100.0
    ...
    def get_pollen_load_percentage(self) -> float:
        ...
        return (self.bee_physiology.current_pollen_load / self.bee_physiology.pollen_basket_capacity) * 100.0
    ...
    def get_resource_collection_stats(self) -> Dict[str, Any]:
        ...
        stats = self.resource_collector.get_collection_efficiency_stats()
                ...
                if self.foraging_decision_engine:
                    ...
                    if target_patch:
                        ...
                        if self.model.random.random() < 0.6:  # Prefer nectar
                            self.transition_to_state(BeeStatus.NECTAR_FORAGING, self.model.schedule.steps)
                        else:
                            self.transition_to_state(BeeStatus.POLLEN_FORAGING, self.model.schedule.steps)
                    else:
                        # No suitable patch found, continue searching or return
                        if self.energy < 30:  # Low energy, return empty
                            self.transition_to_state(BeeStatus.RETURNING_EMPTY, self.model.schedule.steps)
                else:
                    # Basic patch selection without advanced decision engine
                    selected_patch = self.model.random.choice(available_patches)
                    self.current_foraging_target = selected_patch.patch_id if hasattr(selected_patch, 'patch_id') else 0
                    self.transition_to_state(BeeStatus.NECTAR_FORAGING, self.model.schedule.steps)
            else:
    ...
    def execute_returning_empty_behavior(self) -> None:
        ...
        if self.colony and hasattr(self.colony, 'location'):
            hive_distance = self.get_distance_to(self.colony.location) if self.location else 0
    ...
    def execute_returning_unhappy_nectar_behavior(self) -> None:
        ...
        if self.colony and hasattr(self.colony, 'location'):
            ...
            if hive_distance < 10:  # At hive
                ...
                self.record_foraging_success(self.current_foraging_target or 0, True, resources.get('nectar', 0))
    ...
    def execute_returning_unhappy_pollen_behavior(self) -> None:
        ...
        if self.colony and hasattr(self.colony, 'location'):
            ...
            if hive_distance < 10:  # At hive
                ...
                # Record moderate success
                self.record_foraging_success(self.current_foraging_target or 0, True, resources.get('pollen', 0))
        ...
        if self.current_foraging_target and hasattr(self.model, 'get_patch_by_id'):
            ...
            if target_patch:
                ...
                if patch_distance < 5:  # At patch
                    ...
                    self.transition_to_state(BeeStatus.COLLECT_NECTAR, self.model.schedule.steps)
                else:
    ...
    def execute_collect_nectar_behavior(self) -> None:
        ...
        if self.current_foraging_target and hasattr(self.model, 'get_patch_by_id'):
            ...
            if target_patch:
                ...
                collection_result, energy_gained, details = self.attempt_resource_collection(target_patch, "nectar")
                ...
                if collection_result == "success":
                    ...
                    if self.is_carrying_capacity_full():
                        ...
                        if resource_quality > 0.7:
                            self.transition_to_state(BeeStatus.BRINGING_NECTAR, self.model.schedule.steps)
                        else:
                            self.transition_to_state(BeeStatus.RETURNING_UNHAPPY_NECTAR, self.model.schedule.steps)
    ...
    def execute_experimental_foraging_nectar_behavior(self) -> None:
        ...
        if hasattr(self.model, 'explore_new_patches'):
            new_patches = self.model.explore_new_patches(self.location if self.location else (0, 0))
    ...
    def execute_pollen_foraging_behavior(self) -> None:
        ...
        if self.current_foraging_target and hasattr(self.model, 'get_patch_by_id'):
            ...
            if target_patch:
                ...
                if patch_distance < 5:  # At patch
                    self.transition_to_state(BeeStatus.COLLECT_POLLEN, self.model.schedule.steps)
                else:
                    self.move_towards_patch(target_patch)
            else:
                self.transition_to_state(BeeStatus.SEARCHING, self.model.schedule.steps)
    ...
    def execute_collect_pollen_behavior(self) -> None:
        ...
        if self.current_foraging_target and hasattr(self.model, 'get_patch_by_id'):
            ...
            if target_patch:
                collection_result, energy_gained, details = self.attempt_resource_collection(target_patch, "pollen")
                ...
                if collection_result == "success":
                    ...
                    if self.is_carrying_capacity_full():
                        resource_quality = details.get('quality', 0.5)
                        if resource_quality > 0.7:
                            self.transition_to_state(BeeStatus.BRINGING_POLLEN, self.model.schedule.steps)
                        else:
                            self.transition_to_state(BeeStatus.RETURNING_UNHAPPY_POLLEN, self.model.schedule.steps)
                elif collection_result == "depleted":
                    self.transition_to_state(BeeStatus.SEARCHING, self.model.schedule.steps)
                elif collection_result == "incompatible":
                    self.transition_to_state(BeeStatus.SEARCHING, self.model.schedule.steps)
    ...
    def execute_experimental_foraging_pollen_behavior(self) -> None:
        ...
        if hasattr(self.model, 'explore_new_patches'):
            ...
            if new_patches:
                ...
                best_patch = max(new_patches, key=lambda p: getattr(p, 'pollen_quality', 0.5))
                self.current_foraging_target = getattr(best_patch, 'patch_id', 0)
    ...
    def execute_egg_laying_behavior(self) -> None:
        ...
        if self.colony and hasattr(self.colony, 'can_lay_eggs') and self.colony.can_lay_eggs():
    ...
    def execute_nursing_behavior(self) -> None:
        ...
        if self.colony and hasattr(self.colony, 'get_brood_needing_care'):
            ...
            if brood_needing_care:
                ...
                care_provided = min(3, len(brood_needing_care))  # Care for up to 3 brood per step
                ...
                if self.data_collector and hasattr(self.data_collector, 'bee_metrics'):
                    if self.unique_id in self.data_collector.bee_metrics:
                        metrics = self.data_collector.bee_metrics[self.unique_id]
                        metrics.communication_events += care_provided  # Track care events
            else:
    ...
    def execute_dancing_behavior(self) -> None:
        ...
        if self.current_foraging_target and self.current_foraging_target in self.foraging_memories:
    ...
    def choose_next_activity(self) -> Optional[BeeStatus]:
        ...
        if self.role == BeeRole.QUEEN:
            ...
            if self.colony and hasattr(self.colony, 'get_brood_count') and self.colony.get_brood_count() > 0:
            ...
            if self.colony and hasattr(self.colony, 'needs_construction') and self.colony.needs_construction():
    ...
    def communicate_patch_info(self, dance_info: DanceInfo) -> int:
        ...
        if self.colony and hasattr(self.colony, 'get_nearby_bees'):
            nearby_bees = self.colony.get_nearby_bees(self.location if self.location else (0, 0), radius=10)
            
            for bee in nearby_bees:
                if (bee != self and 
                    hasattr(bee, 'role') and 
                    bee.role in [BeeRole.FORAGER, BeeRole.NURSE] and
                    bee.energy > 40):
                    ...
                    if self.model.random.random() < follow_probability:
                        ...
                        if hasattr(bee, 'foraging_memories'):
                            bee.foraging_memories[dance_info.patch_id] = ForagingMemory(
                                ...
                                quality=dance_info.quality * 0.9,  # Slightly reduced quality for followers
    ...
    def get_personal_activity_summary(self) -> Dict[str, Any]:
        ...
        summary.update({
            ...
            "colony_id": getattr(self.colony, 'unique_id', None) if self.colony else None
        })
    ...
    def set_initial_behavioral_preferences(self, tracker) -> None:
        ...
        if self.role == BeeRole.QUEEN:
            ...
            tracker.behavioral_preferences.update({
                BeeStatus.NEST_CONSTRUCTION.value: 0.8,
                BeeStatus.RESTING.value: 0.5
            })
...
class Forager(Worker):
    ...
    def forage_at_patch(self) -> None:
        ...
        # Update foraging memory
        self.update_foraging_memory(patch)
    ...
    def update_foraging_memory(self, patch_id: int, success: bool, energy_gained: float) -> None:
        ...
        if success:
            memory.quality = 0.9 * memory.quality + 0.1 * (energy_gained / 10.0)  # Normalize energy to quality
