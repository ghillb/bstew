...
from .patch_selection import PatchInfo, ResourceType as PatchResourceType
from .data_collection import ComprehensiveDataCollector, SimulationEvent
from .dynamic_parameters import DynamicParameterSystem, ModificationRule, ParameterType, ModificationTrigger, ModificationStrategy
from .health_monitoring import HealthMonitoringSystem, HealthThresholds, HealthAlert
from .spatial_integration import SpatialEnvironmentManager, SpatialBeeManager, create_spatial_integration_system
from .dance_communication_integration import DanceCommunicationIntegrator, create_dance_communication_integration
from ..visualization.live_visualization import LiveVisualizationManager, create_live_visualization_system
...
class BeeModel(Model):
    ...
    dance_communication_integrator: Optional[DanceCommunicationIntegrator]
    ...
    def run_simulation(self, days: Optional[int] = None) -> None:
        ...
        while self.current_day < target_day and self.running:  # type: ignore[has-type]
            self.step()
    ...
    def _initialize_dynamic_parameters(self) -> None:
        ...
        if dynamic_params_config.get("enabled", True):
            self.dynamic_parameter_system = DynamicParameterSystem(
                enable_adaptive_modifications=dynamic_params_config.get("adaptive_enabled", True),
                max_simultaneous_modifications=dynamic_params_config.get("max_simultaneous", 10),
                modification_cooldown=dynamic_params_config.get("cooldown_seconds", 5.0)
            )
    ...
    def _initialize_health_monitoring(self) -> None:
        ...
        if health_config.get("enabled", True):
            ...
            thresholds = HealthThresholds(**threshold_config) if threshold_config else HealthThresholds()
            ...
            def health_alert_callback(alert: HealthAlert):
                self.logger.warning(f"Health Alert: {alert.message} (Colony {alert.colony_id})")
    ...
    def _add_colony_components(self, colony: Colony) -> None:
        ...
        if disease_config.get("enabled", True):
            colony.disease_manager = DiseaseManager(colony)  # type: ignore[attr-defined]

        # Reproduction management
        reproduction_config = self.config_dict.get("reproduction", {})
        if reproduction_config.get("enabled", True):
            colony.reproduction_manager = ReproductionManager(colony)  # type: ignore[attr-defined]
    ...
    def _initialize_colony_foraging(self, colony: Colony) -> None:
        ...
        colony_state = {
            ...
            "available_foragers": len([b for b in colony.bees if hasattr(b, 'activity_state') and b.activity_state == 'ready_to_forage'])
        }
        ...
        self.logger.info(f"Foraging system initialized for colony {colony.unique_id} with {len(landscape_patches)} patches")
    ...
    def _create_patches_from_landscape(self, colony: Colony) -> List[PatchInfo]:
        ...
        for i in range(20):  # Create 20 patches around colony
            ...
            resource_type = self.random.choice([PatchResourceType.NECTAR, PatchResourceType.POLLEN, PatchResourceType.MIXED])
            ...
            patch = PatchInfo(
                ...
                quality_metrics={
                    "resource_density": self.random.uniform(0.3, 1.0),
                    "sugar_concentration": self.random.uniform(0.1, 0.8),
                    "flower_count": self.random.randint(10, 100),
                    "accessibility": self.random.uniform(0.5, 1.0),
                    "competition_level": self.random.uniform(0.0, 0.6),
                    "handling_time": self.random.uniform(0.1, 0.5),
                    "corolla_depth": self.random.uniform(2.0, 15.0),
                    "corolla_width": self.random.uniform(1.0, 8.0)
                },
            ...
            for bee in getattr(colony, 'bees', []):
                if hasattr(bee, 'status') and hasattr(bee, 'unique_id'):
                    ...
                    if bee.unique_id not in self.spatial_bee_manager.bee_states:
                        ...
                        self.spatial_bee_manager.register_bee(bee.unique_id, initial_position)
                    
                    # Update spatial behavior
                    behavior_result = self.spatial_bee_manager.update_bee_spatial_behavior(
                    ...
                    if behavior_result.get('foraging_success'):
                        if hasattr(bee, 'add_energy'):
                            bee.add_energy(behavior_result.get('resources_collected', 0))

    def _process_dance_communication(self) -> None:
        """Process dance communication for all colonies"""
        if not self.dance_communication_integrator:
            return
            
        current_time = self.current_day * 24.0  # Convert to hours
        
        # Clean up finished dances
        self.dance_communication_integrator.cleanup_finished_dances(current_time)
        
        # Process returning foragers for dance decisions
        for colony in self.get_colonies():
            colony_id = colony.unique_id
            colony_state = self._get_colony_state_for_communication(colony)
            bee_states = self._get_bee_states_for_communication(colony)
            
            # Check for returning foragers
            for bee in getattr(colony, 'bees', []):
                if (hasattr(bee, 'status') and 
                    hasattr(bee, 'unique_id') and
                    str(bee.status).endswith('RETURNING')):
                    
                    # Create foraging result from bee state
                    foraging_result = self._extract_foraging_result(bee)
                    
                    if foraging_result:
                        # Process potential dance
                        dance_performance = self.dance_communication_integrator.process_returning_forager(
                            bee.unique_id, foraging_result, colony_id, colony_state, bee_states
                        )
                        
                        if dance_performance:
                            self.logger.debug(f"Bee {bee.unique_id} performed dance {dance_performance.performance_id}")
            
            # Update recruitment outcomes
            recruited_bees = self.dance_communication_integrator.get_recruited_bees(colony_id)
            
            # Apply recruitment to bee behaviors
            for recruit_info in recruited_bees:
                bee_id = recruit_info['bee_id']
                target_patch = recruit_info['target_patch_id']
                
                # Find the bee and update its target
                for bee in getattr(colony, 'bees', []):
                    if hasattr(bee, 'unique_id') and bee.unique_id == bee_id:
                        if hasattr(bee, 'set_target_patch'):
                            bee.set_target_patch(target_patch)
                        elif hasattr(bee, 'target_patch'):
                            bee.target_patch = target_patch
    ...
    def _get_colony_state_for_communication(self, colony: Colony) -> Dict[str, Any]:
        ...
        return {
            ...
            'energy_level': getattr(getattr(colony, 'resources', None), 'total_food', 0.5) / 1000.0,
            'available_foragers': len([b for b in getattr(colony, 'bees', []) 
                                     if hasattr(b, 'status') and 'IDLE' in str(b.status)]),
            'recent_dance_count': 0  # Would track recent dances
        }

    def _get_bee_states_for_communication(self, colony: Colony) -> Dict[int, Dict[str, Any]]:
        ...
        for bee in getattr(colony, 'bees', []):
            if hasattr(bee, 'unique_id'):
                bee_states[bee.unique_id] = {
                    ...
                    'dance_following_experience': getattr(bee, 'dance_following_experience', 0.5),
    ...
    def _extract_foraging_result(self, bee) -> Optional[Dict[str, Any]]:
    ...
    def _update_recruitment_networks(self) -> None:
        """Update recruitment mechanism networks"""
        if (not self.dance_communication_integrator or 
            not hasattr(self.dance_communication_integrator, 'recruitment_manager') or
            not self.dance_communication_integrator.recruitment_manager):
            return
            
        recruitment_manager = self.dance_communication_integrator.recruitment_manager
        
        # Initialize colony networks if not already done
        for colony in self.get_colonies():
            colony_id = colony.unique_id
            
            if colony_id not in recruitment_manager.colony_networks:
                bee_ids = [bee.unique_id for bee in getattr(colony, 'bees', []) 
                          if hasattr(bee, 'unique_id')]
                
                if bee_ids:
                    recruitment_manager.initialize_colony_network(colony_id, bee_ids)
            
            # Propagate information through network
            recruitment_manager.propagate_information_through_network(colony_id, self.current_day)
    ...
    def _initialize_spatial_systems(self) -> None:
        ...
        if spatial_config.get("enabled", False):
            ...
            try:
                ...
                self.spatial_environment, self.spatial_bee_manager = create_spatial_integration_system(landscape_config)
    ...
    def _initialize_dance_communication(self) -> None:
        ...
        if comm_config.get("dance_enabled", True):
            try:
                # Initialize dance communication system
                self.dance_communication_integrator = create_dance_communication_integration()
                self.logger.info("Dance communication system initialized")
                
            except Exception as e:
                self.logger.error(f"Failed to initialize dance communication: {e}")
                self.dance_communication_integrator = None
        else:
            self.dance_communication_integrator = None
    ...
    def _initialize_live_visualization(self) -> None:
        ...
        if viz_config.get("enabled", False):
            try:
                ...
                self.live_visualization_manager = create_live_visualization_system(output_dir)
                
                # Register model data collectors
                self.live_visualization_manager.register_model_data_collector(self, "bstew")
    ...
    def _initialize_system_integrator(self) -> None:
        ...
        try:
            self.system_integrator = SystemIntegrator(
                ...
                enable_mortality_tracking=integrator_config.get("enable_mortality_tracking", True),
                enable_excel_reporting=integrator_config.get("enable_excel_reporting", True),
                enable_spatial_analysis=integrator_config.get("enable_spatial_analysis", True),
                enable_health_monitoring=integrator_config.get("enable_health_monitoring", True)
            )
            ...
            if "error" in initialization_status:
                self.logger.error(f"System integration error: {initialization_status['error']}")
            else:
                self.logger.info(f"System integrator initialized with systems: {active_systems}")
    ...
    def _update_foraging_environment(self) -> None:
        ...
        for colony in self.get_colonies():
            ...
            if self.current_day % 10 == 0:  # Log every 10 days
                self.logger.debug(f"Updated foraging environment for colony {colony_id}: {environmental_context}")
    ...
    def _process_foraging_outcomes(self) -> None:
        ...
        for colony in self.get_colonies():
            ...
            if "session_statistics" in analytics:
                ...
                recent_efficiency = analytics["session_statistics"].get("recent_efficiency", 0.0)
                estimated_energy = sessions_today * recent_efficiency * 50.0  # Simplified calculation
    ...
    def emit_simulation_event(self, event_type: str, source_agent_id: Optional[int] = None,
                            target_agent_id: Optional[int] = None, colony_id: Optional[int] = None,
                            event_data: Optional[Dict[str, Any]] = None, success: bool = True,
                            energy_impact: Optional[float] = None) -> None:
        """Emit simulation event for data collection"""
    ...
    def _initialize_data_collection(self) -> None:
        ...
        if self.foraging_system:
            self.comprehensive_data_collector.integrate_with_foraging_system(self.foraging_system)
    ...
    def _configure_performance_logging(self) -> None:
        ...
        self.logger.info(f"Performance logging configured: level={logging.getLevelName(target_level)}")
    ...
    def step(self) -> None:
        ...
        if self.dance_communication_integrator:
            self._process_dance_communication()
            
            # Update recruitment mechanism networks
            self._update_recruitment_networks()

        # Update health monitoring
        if self.health_monitoring_system and self.current_day % self.health_monitoring_system.update_frequency == 0:
        ...
        if hasattr(self, 'comprehensive_data_collector'):
            ...
            for colony in self.get_colonies():
                self.comprehensive_data_collector.collect_colony_data(colony, self.current_day)
                
                # Collect bee-level data
                for bee in getattr(colony, 'bees', []):
                    self.comprehensive_data_collector.collect_bee_data(bee, self.current_day)
            
            # Perform data aggregation
            aggregation_results = self.comprehensive_data_collector.aggregate_data(self.current_day)
            
            # Log aggregation results periodically
            if self.current_day % 100 == 0 and aggregation_results:
                self.logger.info(f"Day {self.current_day} aggregation completed: {len(aggregation_results)} metrics")
    ...
    def get_simulation_summary(self) -> Dict[str, Any]:
        ...
        # Add dance communication summary
        if self.dance_communication_integrator:
            dance_metrics = {}
            for colony in colonies:
                colony_id = colony.unique_id
                colony_dance_metrics = self.dance_communication_integrator.get_colony_communication_metrics(colony_id)
                dance_metrics[f"colony_{colony_id}"] = colony_dance_metrics
            summary["dance_communication"] = dance_metrics
    ...
    def export_results(self, output_dir: str = "results") -> None:
        ...
        exporter.export_to_csv(model_data, output_path / "model_data.csv")  # type: ignore[attr-defined]

        # Export agent data
        agent_data = self.datacollector.get_agent_vars_dataframe()
        if len(agent_data) > 0:
            exporter.export_to_csv(agent_data, output_path / "agent_data.csv")  # type: ignore[attr-defined]

        # Export simulation summary
        summary = self.get_simulation_summary()
        exporter.export_to_json(summary, output_path / "simulation_summary.json")  # type: ignore[attr-defined]
    ...
    def cleanup(self) -> None:
        ...
        if self.system_integrator:
            try:
                final_reports = self.system_integrator.generate_reports(self.current_day, final_report=True)
                if final_reports:
    ...
    def update_configuration(self, config_updates: dict) -> None:
        ...
        if "invalid_key" in config_updates:
    ...
    def _update_dynamic_parameters(self) -> None:
        ...
        self.dynamic_parameter_system.update_system_state(
            simulation_time=float(self.current_day),
    ...
    def _get_current_model_state(self) -> Dict[str, Any]:
        ...
        total_energy = sum(colony.resources.total_food for colony in colonies if hasattr(colony, 'resources'))
        active_colonies = len([c for c in colonies if getattr(c, 'health', 'healthy') != 'collapsed'])
        ...
        if self.foraging_system:
            ...
            foraging_efficiency = system_analytics.get("system_performance", {}).get("average_efficiency", foraging_efficiency)
        ...
        model_state = {
            ...
            "foraging_success_rate": foraging_analytics.get("system_performance", {}).get("overall_success_rate", 0.5),
            ...
            "average_colony_health": (
                np.mean([colony.get_health_score() for colony in colonies])
                if colonies else 0.0
            ),
            
            # Behavioral metrics
            "population_stress": max(0.0, 1.0 - (total_energy / max(1, total_population * 100))),
            "environmental_stress": max(0.0, abs(temperature - 20.0) / 20.0),
            
            # Performance indicators
            "system_efficiency": foraging_efficiency * seasonal_factor,
            "sustainability_score": min(1.0, total_energy / max(1, total_population * 50)),
        }
        
        # Add foraging system specific state if available
        if self.foraging_system:
            model_state.update({
                "active_foraging_sessions": len(self.foraging_system.active_foraging_sessions),
                "total_patches_available": len(self.foraging_system.patch_database),
                "foraging_system_health": foraging_analytics.get("system_performance", {})
            })
    ...
    def _calculate_resource_scarcity(self) -> float:
        ...
        total_resources = sum(
            getattr(colony, 'resources', type('obj', (), {'total_food': 0})).total_food 
            for colony in colonies
        )
        total_population = sum(colony.get_adult_population() for colony in colonies)
    ...
    def apply_manual_parameter_change(self, parameter_path: str, new_value: Any, 
                                    duration: Optional[float] = None) -> Dict[str, Any]:
        """Apply manual parameter modification"""
        
        if not self.dynamic_parameter_system:
            return {"success": False, "error": "Dynamic parameter system not initialized"}
        
        return self.dynamic_parameter_system.apply_manual_modification(
            parameter_path, new_value, duration
        )

    def get_dynamic_parameter_analytics(self, parameter_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    def optimize_parameters_for_target(self, target_metrics: Dict[str, float], 
    ...
    def _register_colony_health_monitoring(self, colony: Colony) -> None:
        ...
        initial_data = {
            ...
            'total_energy': getattr(colony, 'resources', type('obj', (), {'total_food': 1000})).total_food,
            'species': colony.species,
            'location': colony.location,
    ...
    def _collect_colony_health_data(self, colony: Colony) -> Dict[str, Any]:
        ...
        if self.foraging_system:
            analytics = self.foraging_system.get_colony_foraging_analytics(colony.unique_id)
            if analytics and "session_statistics" in analytics:
                foraging_efficiency = analytics["session_statistics"].get("average_efficiency", 0.8)
        ...
        if disease_manager and hasattr(disease_manager, 'get_disease_summary'):
            ...
            disease_prevalence = summary.get('total_infections', 0) / max(adult_population, 1)
        else:
        ...
        colony_health_data = {
            ...
            'population_density': adult_population / max(1, brood_count + adult_population),
    ...
    def _calculate_resource_scarcity_for_colony(self, colony: Colony) -> float:
        ...
        if resources:
            ...
            return min(1.0, max(base_scarcity, colony_scarcity))
    ...
    def get_health_alerts(self, colony_id: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    def get_health_predictions(self, colony_id: int, horizon: int = 10) -> Dict[str, Any]:
    ...
    def _initialize_spatial_systems(self) -> None:
        ...
        try:
            ...
            landscape_config = self.config_dict.get('landscape_config', {
        ...
        for colony in self.colonies:
            for bee in colony.agents:
                if hasattr(bee, 'status') and hasattr(bee, 'unique_id'):
                    ...
                    # Update spatial behavior
                    spatial_result = self.spatial_bee_manager.update_bee_spatial_behavior(
                        bee.unique_id, 
                        bee_status, 
                        self.current_day
                    )
    ...
    def _apply_spatial_results_to_bee(self, bee: Any, spatial_result: Dict[str, Any]) -> None:
        ...
        if 'foraging_success' in spatial_result:
            if hasattr(bee, 'energy') and spatial_result['foraging_success']:
                ...
                bee.energy = min(bee.energy + energy_gain, getattr(bee, 'max_energy', 100))
    ...
    def get_spatial_analysis_results(self) -> Dict[str, Any]:
        ...
        # Landscape analysis
        landscape_quality = self.spatial_environment.landscape_analyzer.assess_landscape_quality()
        fragmentation = self.spatial_environment.landscape_analyzer.calculate_landscape_fragmentation()
        isolation = self.spatial_environment.landscape_analyzer.analyze_patch_isolation()
        ...
        connectivity_graph = self.spatial_environment.connectivity_analyzer.build_connectivity_graph(
            ConnectivityType.FUNCTIONAL
        )
        connectivity_metrics = self.spatial_environment.connectivity_analyzer.analyze_connectivity_metrics(
            connectivity_graph
        )
    ...
    def export_spatial_data_to_gis(self, output_path: str, output_format: str = "geojson") -> Optional[str]:
        ...
        try:
            result_path = self.spatial_environment.export_landscape_to_gis(output_path, output_format)
        ...
        try:
            ...
            combined_results = {
                ...
                'integration_status': {
                    ...
                    'coordinate_transformations': self.spatial_environment.gis_manager is not None,
                    'data_sources': len(self.spatial_environment.spatial_index._patches)
                }
            }
