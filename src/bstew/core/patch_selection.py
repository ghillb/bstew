...
class SpeciesPreferences(BaseModel):
    ...
    nectar_preference: float = Field(default=0.7, ge=0.0, le=1.0, description="Nectar preference weight")
    pollen_preference: float = Field(default=0.3, ge=0.0, le=1.0, description="Pollen preference weight")
    
    # Quality thresholds
    minimum_sugar_concentration: float = Field(default=0.15, ge=0.0, le=1.0, description="Minimum sugar concentration")
    quality_threshold: float = Field(default=0.5, ge=0.0, le=1.0, description="Minimum patch quality threshold")
    
    # Distance preferences
    max_foraging_distance: float = Field(default=1000.0, ge=0.0, description="Maximum foraging distance (m)")
    distance_penalty_factor: float = Field(default=0.1, ge=0.0, description="Distance penalty factor")
    
    # Competition tolerance
    competition_tolerance: float = Field(default=0.3, ge=0.0, le=1.0, description="Competition tolerance level")
    crowding_threshold: int = Field(default=5, ge=0, description="Crowding threshold (foragers per patch)")
    
    # Temporal preferences
    preferred_foraging_hours: List[int] = Field(default_factory=lambda: list(range(8, 18)), description="Preferred foraging hours")
    weather_sensitivity: float = Field(default=0.5, ge=0.0, le=1.0, description="Weather sensitivity factor")
    
    # Flower morphology preferences
    preferred_corolla_depth: Tuple[float, float] = Field(default=(2.0, 8.0), description="Preferred corolla depth range (mm)")
    proboscis_length: float = Field(default=6.0, ge=0.0, description="Proboscis length (mm)")
    
    @field_validator('pollen_preference')
    @classmethod
    def validate_resource_preferences(cls, v: float, info) -> float:
        ...
        if hasattr(info, 'data'):
            ...
            if abs(total - 1.0) > 0.1:  # Allow reasonable floating point errors
                raise ValueError(f"Nectar and pollen preferences must sum to approximately 1.0, got {total}")
        return v
...
class PatchQualityAssessment(BaseModel):
    ...
    # Quality weights
    resource_density_weight: float = Field(default=0.3, ge=0.0, le=1.0, description="Resource density weight")
    sugar_concentration_weight: float = Field(default=0.25, ge=0.0, le=1.0, description="Sugar concentration weight")
    accessibility_weight: float = Field(default=0.2, ge=0.0, le=1.0, description="Accessibility weight")
    competition_weight: float = Field(default=0.15, ge=0.0, le=1.0, description="Competition weight")
    handling_time_weight: float = Field(default=0.1, ge=0.0, le=1.0, description="Handling time weight")
    
    # Environmental factors
    weather_impact_factor: float = Field(default=0.8, ge=0.0, le=1.0, description="Weather impact factor")
    time_of_day_factor: float = Field(default=0.9, ge=0.0, le=1.0, description="Time of day factor")
    seasonal_factor: float = Field(default=1.0, ge=0.0, le=2.0, description="Seasonal availability factor")
    
    def calculate_overall_quality(self, patch: PatchInfo, current_conditions: Dict[str, Any]) -> float:
        """Calculate overall patch quality score"""
        
        base_quality = 0.0
        
        # Resource density component
        resource_density = patch.quality_metrics.get(PatchQualityMetric.RESOURCE_DENSITY, 0.5)
        base_quality += resource_density * self.resource_density_weight
        
        # Sugar concentration component
        sugar_concentration = patch.quality_metrics.get(PatchQualityMetric.SUGAR_CONCENTRATION, 0.5)
        ...
        # Competition penalty
        competition_level = patch.quality_metrics.get(PatchQualityMetric.COMPETITION_LEVEL, 0.0)
...
class DistanceQualityTradeoff(BaseModel):
    ...
    # Tradeoff parameters
    distance_decay_rate: float = Field(default=0.001, ge=0.0, description="Distance decay rate")
    energy_cost_per_meter: float = Field(default=0.01, ge=0.0, description="Energy cost per meter")
    quality_sensitivity: float = Field(default=2.0, ge=0.0, description="Quality sensitivity factor")
    
    # Optimization parameters
    min_acceptable_quality: float = Field(default=0.3, ge=0.0, le=1.0, description="Minimum acceptable quality")
    max_travel_distance: float = Field(default=2000.0, ge=0.0, description="Maximum travel distance (m)")
    
    def calculate_patch_utility(self, patch: PatchInfo, quality: float, bee_energy: float) -> float:
        ...
        # Distance penalty
        distance_penalty = math.exp(-self.distance_decay_rate * patch.distance_from_hive)
...
class ThresholdBasedSelection(BaseModel):
    ...
    distance_threshold: float = Field(default=500.0, ge=0.0, description="Distance threshold (m)")
    competition_threshold: float = Field(default=0.7, ge=0.0, le=1.0, description="Competition threshold")
    
    # Adaptive thresholds
    adaptive_thresholds: bool = Field(default=True, description="Use adaptive thresholds")
    success_rate_influence: float = Field(default=0.3, ge=0.0, le=1.0, description="Success rate influence on thresholds")
    
    def select_patches(self, available_patches: List[PatchInfo], 
                      patch_qualities: Dict[int, float],
                      bee_memory: Dict[int, Any]) -> List[PatchInfo]:
...
class AdvancedPatchSelector(BaseModel):
    ...
    # Component systems
    quality_assessment: PatchQualityAssessment = Field(default_factory=PatchQualityAssessment)
    distance_tradeoff: DistanceQualityTradeoff = Field(default_factory=DistanceQualityTradeoff)
    threshold_selection: ThresholdBasedSelection = Field(default_factory=ThresholdBasedSelection)
    
    # Selection parameters
    default_strategy: PatchSelectionStrategy = Field(default=PatchSelectionStrategy.MULTI_CRITERIA)
    max_patches_to_consider: int = Field(default=20, ge=1, description="Maximum patches to consider")
    
    # Internal state
    species_preferences: Dict[str, SpeciesPreferences] = Field(default_factory=dict, description="Species preferences")
    selection_history: List[Dict[str, Any]] = Field(default_factory=list, description="Selection history")
    selection_performance: Dict[str, float] = Field(default_factory=dict, description="Selection performance metrics")
    ...
    def select_optimal_patches(self, available_patches: List[PatchInfo],
                              bee_species: str,
                              bee_energy: float,
                              bee_memory: Dict[int, Any],
                              current_conditions: Dict[str, Any],
                              strategy: Optional[PatchSelectionStrategy] = None) -> List[PatchInfo]:
        ...
        species_prefs = self.species_preferences.get(bee_species, self.species_preferences["apis_mellifera"])
        
        # Use specified strategy or default
        selection_strategy = strategy or self.default_strategy
        
        # Filter patches by basic criteria
        feasible_patches = self._filter_feasible_patches(available_patches, species_prefs, current_conditions)
        ...
        for patch in feasible_patches:
            quality = self.quality_assessment.calculate_overall_quality(patch, current_conditions)
            patch_qualities[patch.patch_id] = quality
        
        # Apply selection strategy
        selected_patches = self._apply_selection_strategy(
            feasible_patches, patch_qualities, species_prefs, bee_energy, bee_memory, selection_strategy
        )
    ...
    def _filter_feasible_patches(self, patches: List[PatchInfo], 
                               species_prefs: SpeciesPreferences,
                               current_conditions: Dict[str, Any]) -> List[PatchInfo]:
        ...
        for patch in patches:
            ...
            if patch.resource_type == ResourceType.NECTAR and species_prefs.nectar_preference < 0.1:
                continue
            if patch.resource_type == ResourceType.POLLEN and species_prefs.pollen_preference < 0.1:
                continue
            
            # Sugar concentration filter
            sugar_conc = patch.quality_metrics.get(PatchQualityMetric.SUGAR_CONCENTRATION, 0.0)
    ...
    def _check_flower_compatibility(self, patch: PatchInfo, species_prefs: SpeciesPreferences) -> bool:
        """Check if bee can physically access flowers in patch"""
        
        # Get species compatibility score
        species_compatibility = patch.species_compatibility.get(species_prefs.species_name, 0.5)
    ...
    def _apply_selection_strategy(self, patches: List[PatchInfo],
                                patch_qualities: Dict[int, float],
                                species_prefs: SpeciesPreferences,
                                bee_energy: float,
                                bee_memory: Dict[int, Any],
                                strategy: PatchSelectionStrategy) -> List[PatchInfo]:
        """Apply specific selection strategy"""
        
        if strategy == PatchSelectionStrategy.THRESHOLD_BASED:
            return self.threshold_selection.select_patches(patches, patch_qualities, bee_memory)
        
        elif strategy == PatchSelectionStrategy.DISTANCE_WEIGHTED:
            return self._distance_weighted_selection(patches, patch_qualities, bee_energy)
            ...
            return self._energy_optimizing_selection(patches, patch_qualities, bee_energy)
        
        elif strategy == PatchSelectionStrategy.PROBABILISTIC:
            return self._probabilistic_selection(patches, patch_qualities, species_prefs)
        
        elif strategy == PatchSelectionStrategy.MULTI_CRITERIA:
            return self._multi_criteria_selection(patches, patch_qualities, species_prefs, bee_energy)
        
        else:
            # Default to quality maximizing
            return self._quality_maximizing_selection(patches, patch_qualities)
    
    def _distance_weighted_selection(self, patches: List[PatchInfo],
                                   patch_qualities: Dict[int, float],
                                   bee_energy: float) -> List[PatchInfo]:
        ...
        for patch in patches:
            quality = patch_qualities.get(patch.patch_id, 0.0)
            utility = self.distance_tradeoff.calculate_patch_utility(patch, quality, bee_energy)
    ...
    def _quality_maximizing_selection(self, patches: List[PatchInfo],
                                    patch_qualities: Dict[int, float]) -> List[PatchInfo]:
        """Select highest quality patches"""
        
        # Sort by quality
        sorted_patches = sorted(patches, key=lambda p: patch_qualities.get(p.patch_id, 0.0), reverse=True)
        
        # Return top 3 patches
        return sorted_patches[:3]
    
    def _energy_optimizing_selection(self, patches: List[PatchInfo],
                                   patch_qualities: Dict[int, float],
                                   bee_energy: float) -> List[PatchInfo]:
        ...
        for patch in patches:
            ...
            travel_cost = 2 * patch.distance_from_hive * self.distance_tradeoff.energy_cost_per_meter
    ...
    def _probabilistic_selection(self, patches: List[PatchInfo],
                               patch_qualities: Dict[int, float],
                               species_prefs: SpeciesPreferences) -> List[PatchInfo]:
        ...
        for _ in range(min(3, len(patches))):
            if patches:
                ...
                patch_index = patches.index(selected_patch)
                patches.pop(patch_index)
                probabilities.pop(patch_index)
    ...
    def _multi_criteria_selection(self, patches: List[PatchInfo],
                                patch_qualities: Dict[int, float],
                                species_prefs: SpeciesPreferences,
                                bee_energy: float) -> List[PatchInfo]:
        ...
        for patch in patches:
            ...
            distance_score = max(0, 1.0 - (patch.distance_from_hive / species_prefs.max_foraging_distance)) * 0.3
            ...
            # Combined score
            total_score = quality_score + distance_score + energy_score + competition_score
    ...
    def _record_selection(self, selected_patches: List[PatchInfo],
                         patch_qualities: Dict[int, float],
                         strategy: PatchSelectionStrategy) -> None:
        """Record selection for performance tracking"""
        
        selection_record = {
            "strategy": strategy.value,
            "patch_count": len(selected_patches),
            "average_quality": sum(patch_qualities.get(p.patch_id, 0.0) for p in selected_patches) / len(selected_patches) if selected_patches else 0.0,
            "average_distance": sum(p.distance_from_hive for p in selected_patches) / len(selected_patches) if selected_patches else 0.0,
    ...
    def get_selection_performance(self) -> Dict[str, Any]:
        ...
        for strategy, records in strategies.items():
            if records:
                ...
                avg_distance = sum(r["average_distance"] for r in records) / len(records)
    ...
    def add_species_preferences(self, species_name: str, preferences: SpeciesPreferences) -> None:
    ...
    def get_patch_recommendations(self, bee_species: str, current_conditions: Dict[str, Any]) -> List[Dict[str, Any]]:
