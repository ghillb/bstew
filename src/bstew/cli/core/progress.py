...
class StatusDisplay:
    ...
    def show_simulation_info(self, config: Dict[str, Any]) -> None:
        ...
        info_table.add_row(
            "Duration",
            f"{simulation.get('duration_days', 'N/A')} days"
        )
        info_table.add_row(
            "Random Seed",
            str(simulation.get("random_seed", "None"))
        )
        info_table.add_row(
            "Colony Species",
            str(colony.get("species", "N/A"))
        )
    ...
    def show_results_summary(self, results: Dict[str, Any]) -> None:
        ...
        for key, value in results.items():
            if isinstance(value, (int, float)):
                ...
                results_table.add_row(key.replace("_", " ").title(), formatted_value, unit)
