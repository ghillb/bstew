...
def show_display_status(
    ...
    try:
        ...
        console.print(
            Panel(
                f"[bold green]Current Color Scheme:[/bold green] {status['current_color_scheme']}",
        ...
        toggle_states = status.get("toggle_states", {})
        if isinstance(toggle_states, dict):
            for toggle_name, enabled in toggle_states.items():
        ...
        custom_colors = status.get("custom_colors", {})
        if custom_colors and verbose and isinstance(custom_colors, dict):
            ...
            for category, color in custom_colors.items():
                colors_table.add_row(category, str(color))

            console.print(colors_table)

        # Display visibility filters if any
        visibility_filters = status.get("visibility_filters", {})
        if visibility_filters and verbose and isinstance(visibility_filters, dict):
            ...
            for filter_name, filter_value in visibility_filters.items():
                filters_table.add_row(filter_name, str(filter_value))

            console.print(filters_table)

        console.print(
            "[bold green]✓ Display status retrieved successfully[/bold green]"
        )
...
def toggle_display_option(
    ...
    try:
        ...
        if success:
            ...
            console.print(
                f"[bold {status_color}]✓ {option.replace('_', ' ').title()} {status_text}[/bold {status_color}]"
            )
        else:
            console.print(f"[bold red]✗ Invalid display option: {option}[/bold red]")
            console.print(
                "[dim]Available options: show_cohorts, show_dead_cols, show_paths, show_resources, show_territories, show_activity, show_species, show_health, show_connectivity, show_grid, show_labels, show_legend, show_statistics, show_stewardship, show_management[/dim]"
            )
            raise typer.Exit(1)
...
def set_display_configuration(
    ...
    try:
        ...
        if success:
            console.print(
                f"[bold green]✓ Applied configuration: {config_name}[/bold green]"
            )
            ...
            if verbose:
                ...
                enabled_toggles = [
                    toggle for toggle, enabled in status["toggles"].items() if enabled
                ]
                console.print(
                    f"[dim]Enabled toggles: {', '.join(enabled_toggles)}[/dim]"
                )
        else:
            console.print(
                f"[bold red]✗ Configuration not found: {config_name}[/bold red]"
            )

            # Show available configurations
            available_configs = display_manager.get_available_configurations()
            console.print(
                f"[dim]Available configurations: {', '.join(available_configs)}[/dim]"
            )
            raise typer.Exit(1)

    except Exception as e:
        console.print(
            f"[bold red]✗ Error setting display configuration: {e}[/bold red]"
        )
        raise typer.Exit(1)
...
def set_color_scheme(
    ...
    try:
        ...
        try:
            ...
            console.print(
                "[dim]Available schemes: species_colors, activity_colors, health_colors, resource_colors, connectivity_colors, management_colors, custom[/dim]"
            )
            raise typer.Exit(1)
...
def set_visibility_filter(
    ...
    filter_value: str = typer.Argument(
        ..., help="Filter value (JSON format for complex filters)"
    ),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output"),
) -> None:
    ...
    try:
        ...
        console.print(
            f"[bold green]✓ Visibility filter set: {filter_name} = {parsed_value}[/bold green]"
        )
...
def create_custom_configuration(
    ...
    try:
        ...
        console.print(
            f"[bold green]✓ Custom configuration created: {name}[/bold green]"
        )
        console.print(f"[dim]Description: {description}[/dim]")

    except Exception as e:
        console.print(
            f"[bold red]✗ Error creating custom configuration: {e}[/bold red]"
        )
        raise typer.Exit(1)
...
def export_display_configuration(
    output_file: str = typer.Option(
        "artifacts/display_config_export.json", help="Output file"
    ),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output"),
) -> None:
    ...
    try:
        ...
        console.print(
            f"[bold red]✗ Error exporting display configuration: {e}[/bold red]"
        )
        raise typer.Exit(1)
...
def import_display_configuration(
    ...
    apply_immediately: bool = typer.Option(
        False, help="Apply configuration immediately"
    ),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output"),
) -> None:
    ...
    try:
        ...
        if not input_path.exists():
            console.print(
                f"[bold red]✗ Configuration file not found: {input_file}[/bold red]"
            )
            raise typer.Exit(1)
        ...
        if apply_immediately:
            ...
            if "current_color_scheme" in config_data:
                display_manager.set_color_scheme(
                    ColorScheme(config_data["current_color_scheme"])
                )
            ...
            if "toggle_states" in config_data:
                for toggle_name, state in config_data["toggle_states"].items():
                    try:
                        ...
                        console.print(
                            f"[yellow]Warning: Unknown toggle '{toggle_name}' skipped[/yellow]"
                        )
            ...
            if "visibility_filters" in config_data:
                for filter_name, filter_value in config_data[
                    "visibility_filters"
                ].items():
            ...
            if "display_parameters" in config_data:
                for param_name, param_value in config_data[
                    "display_parameters"
                ].items():
                    display_manager.set_display_parameter(param_name, param_value)

            console.print(
                "[bold green]✓ Display configuration imported and applied[/bold green]"
            )
        else:
            console.print("[bold green]✓ Display configuration imported[/bold green]")
            console.print(
                "[dim]Use --apply-immediately to apply the configuration[/dim]"
            )
        ...
        console.print(
            f"[bold red]✗ Error importing display configuration: {e}[/bold red]"
        )
        raise typer.Exit(1)
...
def reset_display_configuration(
    ...
    try:
        if not confirm:
            console.print(
                "[yellow]This will reset all display settings to defaults.[/yellow]"
            )
        ...
        console.print(
            "[bold green]✓ Display configuration reset to defaults[/bold green]"
        )

    except Exception as e:
        console.print(
            f"[bold red]✗ Error resetting display configuration: {e}[/bold red]"
        )
        raise typer.Exit(1)
