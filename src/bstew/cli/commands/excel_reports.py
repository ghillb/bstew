...
def generate_simulation_report(
    ...
    output_file: str = typer.Option("artifacts/excel_reports/simulation_report.xlsx", help="Output Excel file"),
    ...
    try:
        ...
        with console.status("[bold green]Creating Excel report..."):
            report_path = report_generator.generate_simulation_summary_report(simulation_data)
        ...
        console.print("[bold green]✓ Simulation report generated successfully[/bold green]")
...
def generate_colony_report(
    ...
    output_file: str = typer.Option("artifacts/excel_reports/colony_report.xlsx", help="Output Excel file"),
    colony_ids: Optional[str] = typer.Option(None, help="Specific colony IDs (comma-separated)"),
    ...
    try:
        ...
        with console.status("[bold green]Creating Excel report..."):
            report_path = report_generator.generate_colony_performance_report(colony_data)
...
def generate_mortality_report(
    ...
    output_file: str = typer.Option("artifacts/excel_reports/mortality_report.xlsx", help="Output Excel file"),
    ...
    try:
        console.print(f"[bold blue]Loading mortality data from {input_file}[/bold blue]")
        ...
        console.print("[bold green]✓ Mortality report generated successfully[/bold green]")
        console.print(f"[dim]Report saved to: {report_path}[/dim]")
...
def generate_comparative_report(
    ...
    output_file: str = typer.Option("artifacts/excel_reports/comparative_report.xlsx", help="Output Excel file"),
    comparison_type: str = typer.Option("performance", help="Comparison type (performance/mortality/population)"),
    labels: Optional[str] = typer.Option(None, help="Labels for datasets (comma-separated)"),
    ...
    try:
        ...
        console.print(f"[bold blue]Loading data from {len(file_list)} files[/bold blue]")
        ...
        for i, file_path in enumerate(file_list):
            ...
            if comparison_type == "performance":
                ...
                data = load_simulation_data(file_path)  # Population data is part of simulation
            else:
                raise ValueError(f"Unknown comparison type: {comparison_type}")
            
            datasets.append(data)
            dataset_labels.append(provided_labels[i] if i < len(provided_labels) else f"Dataset {i+1}")
        
        console.print(f"[green]Loaded {len(datasets)} datasets[/green]")
        
        # Prepare comparative data
        comparative_data = prepare_comparative_data(datasets, dataset_labels, comparison_type)
        
        # Generate comparative report
        console.print(f"[yellow]Generating {comparison_type} comparative report...[/yellow]")
        ...
        console.print("[bold green]✓ Comparative report generated successfully[/bold green]")
        console.print(f"[dim]Report saved to: {report_path}[/dim]")
        ...
        console.print(f"[bold red]✗ Error generating comparative report: {e}[/bold red]")
        raise typer.Exit(1)
...
def generate_custom_report(
    ...
    output_file: str = typer.Option("artifacts/excel_reports/custom_report.xlsx", help="Output Excel file"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output")
) -> None:
...
def load_colony_data(file_path: str) -> Dict[str, Any]:
    ...
    for i in range(1, 11):  # 10 colonies
        colony_data[f"colony_{i}"] = {
            ...
            "performance_data": [
                {"day": j, "population": 2500 + i*100 + j*2, "energy": 95.5 + j*0.1}
                for j in range(365)
            ]
        }
...
def load_mortality_data(file_path: str) -> Dict[str, Any]:
    ...
    mortality_data = {
        ...
        "mortality_timeline": [
            {"day": i, "deaths": 15 + i//10, "cause": "natural" if i%4==0 else "predation"}
            for i in range(365)
        ]
    }
...
def filter_mortality_by_period(mortality_data: Dict[str, Any], period: str) -> Dict[str, Any]:
...
def prepare_comparative_data(datasets: List[Dict[str, Any]], labels: List[str], comparison_type: str) -> Dict[str, Any]:
    ...
    if comparison_type == "performance":
        ...
        comparative_data["comparison_metrics"] = {
            ...
            "primary_causes": ["natural", "predation", "environmental", "disease"][:len(datasets)]
        }
    
    return comparative_data


def create_comparative_excel_report(data: Dict[str, Any], output_file: str, comparison_type: str, include_stats: bool) -> str:
...
def create_custom_excel_report(data: Dict[str, Any], config: Dict[str, Any], output_file: str) -> str:
...
def display_comparative_summary(data: Dict[str, Any], comparison_type: str) -> None:
    ...
    for i, label in enumerate(data["labels"]):
        if comparison_type == "performance" and "average_population" in data["comparison_metrics"]:
            metric_value = data["comparison_metrics"]["average_population"][i]
        elif comparison_type == "mortality" and "mortality_rates" in data["comparison_metrics"]:
