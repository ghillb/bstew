...
from ...core.spatial_analysis import PatchConnectivityAnalyzer, SpatialPatch, PatchType
...
def analyze_spatial_data(
    ...
    try:
        ...
        clusters_count = cluster_results.get("n_clusters", 0)
        ...
        console.print(
            "[bold green]✓ Spatial analysis completed successfully[/bold green]"
        )
...
def query_spatial_patches(
    ...
    try:
        ...
        query_engine = PatchConnectivityAnalyzer()

        # Add patches to the analyzer
        for patch in patch_dict.values():
            query_engine.add_patch(patch)
        ...
        # Execute query
        console.print(
            f"[yellow]Querying patches near ({center_x}, {center_y}) within radius {radius}...[/yellow]"
        )

        # Combine all criteria into single dict
        criteria = {
        ...
        results = query_engine.multi_criteria_search(criteria)

        # Display results
        if results:
            # Extract patch IDs from results
            patch_ids = [result["patch_id"] for result in results]
            display_query_results(patch_ids, patch_dict)

            # Export results if requested
            if output_file:
                export_query_results(patch_ids, patch_dict, output_file)
        ...
        console.print(
            f"[bold green]✓ Query completed: {len(results)} patches found[/bold green]"
        )
...
def calculate_landscape_metrics(
    ...
    output_file: str = typer.Option(
        "artifacts/spatial_analysis/landscape_metrics.json", help="Output file"
    ),
    ...
    try:
        ...
        console.print(
            "[bold green]✓ Landscape metrics calculated successfully[/bold green]"
        )
        console.print(f"[dim]Results saved to: {output_path}[/dim]")

    except Exception as e:
        console.print(
            f"[bold red]✗ Error calculating landscape metrics: {e}[/bold red]"
        )
        raise typer.Exit(1)
...
def export_spatial_network(
    input_file: str = typer.Argument(..., help="Input file with patch data"),
    output_dir: str = typer.Option(
        "artifacts/spatial_analysis/network", help="Output directory"
    ),
    connectivity_threshold: float = typer.Option(0.1, help="Connectivity threshold"),
    format_type: str = typer.Option(
        "graphml", help="Export format (graphml/gexf/json)"
    ),
    ...
    try:
        ...
        console.print(
            f"[dim]Nodes: {network_stats['nodes']}, Edges: {network_stats['edges']}[/dim]"
        )
...
def load_patch_data(file_path: str) -> List[SpatialPatch]:
    ...
    if path.suffix.lower() == ".json":
        ...
        for idx, row in df.iterrows():
            i_int = (
                int(idx) if isinstance(idx, (int, float, str)) else 0
            )  # Convert index to int safely
            patch = SpatialPatch(
                patch_id=row.get("patch_id", i_int),
                x=row.get("x", 0),
                y=row.get("y", 0),
...
def display_query_results(
    results: List[int], patch_dict: Dict[int, SpatialPatch]
) -> None:
...
def export_query_results(
    results: List[int], patch_dict: Dict[int, SpatialPatch], output_file: str
) -> None:
