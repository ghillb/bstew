...
class ParameterManagerCLI:
    ...
    def load_parameters(
        ...
        try:
            ...
            self.context.print_info(
                f"Loading {parameter_type.value} parameters from {file_path}"
            )

            # Load parameters
            params: Any = None
            if parameter_type == ParameterType.BEHAVIORAL:
                params = self.parameter_loader.load_behavioral_parameters(
                    file_path, parameter_format
                )
            elif parameter_type == ParameterType.ENVIRONMENTAL:
                params = self.parameter_loader.load_environmental_parameters(
                    file_path, parameter_format
                )
            elif parameter_type == ParameterType.COLONY:
                params = self.parameter_loader.load_colony_parameters(
                    file_path, parameter_format
                )
            else:
            ...
            if self.parameter_loader.auto_save_enabled:
                self.parameter_loader.save_validated_parameters(
                    parameter_type.value, params
                )
    ...
    def inspect_parameters(
        self, file_path: str, show_details: bool = False, export_schema: bool = False
    ) -> None:
        ...
        try:
            ...
            if file_path_obj.suffix.lower() == ".csv":
                ...
                info_table.add_row("File Size", f"{file_path_obj.stat().st_size} bytes")
                ...
                for col in df.columns:
                    ...
                    sample_values = ", ".join(
                        map(str, df[col].dropna().head(3).tolist())
                    )
            ...
            if export_schema:
                schema_path = file_path_obj.with_suffix(".schema.json")
    ...
    def validate_parameters(
        self, file_path: str, param_type: str = "behavioral", strict: bool = False
    ) -> None:
        ...
        try:
            ...
            self.context.print_info(
                f"Validating {parameter_type.value} parameters from {file_path}"
            )

            # Load and validate parameters
            if parameter_type == ParameterType.BEHAVIORAL:
                self.parameter_loader.load_behavioral_parameters(
                    file_path, ParameterFormat.CSV
                )
            elif parameter_type == ParameterType.ENVIRONMENTAL:
                self.parameter_loader.load_environmental_parameters(
                    file_path, ParameterFormat.CSV
                )
            elif parameter_type == ParameterType.COLONY:
                self.parameter_loader.load_colony_parameters(
                    file_path, ParameterFormat.CSV
                )
    ...
    def convert_parameters(
        ...
        try:
            ...
            self.context.print_info(
                f"Converting {input_file} from {input_format} to {output_format}"
            )
            ...
            self.context.print_success(
                f"Parameters converted successfully: {output_file}"
            )
    ...
    def discover_netlogo_files(
        ...
        try:
            ...
            summary_table.add_row(
                "TOTAL", str(total_files), str(total_rows), style="bold"
            )
            ...
            for file_type, files in files_by_type.items():
                if files:
                    ...
                    for file_info in files:
                        status = (
                            "✅ Valid"
                            if not file_info.validation_errors
                            else "❌ Issues"
                        )
            ...
            if export_report:
                ...
                self.context.print_success(
                    f"Discovery report exported to: {report_file}"
                )
    ...
    def validate_netlogo_compatibility(
        self, directory: str, fix_issues: bool = False
    ) -> None:
        ...
        try:
            ...
            self.context.print_info(
                f"Validating NetLogo compatibility for: {directory}"
            )

            # Run compatibility validation
            validation_report = self.netlogo_integrator.validate_netlogo_compatibility(
                dir_path
            )
    ...
    def convert_netlogo_data(
        ...
        try:
            ...
            if not netlogo_dir.exists():
                self.context.print_error(
                    f"NetLogo directory not found: {netlogo_directory}"
                )
                return
            ...
            if validate_first:
                ...
                validation_report = (
                    self.netlogo_integrator.validate_netlogo_compatibility(netlogo_dir)
                )

                if not validation_report["compatible"]:
                    self.context.print_error(
                        "NetLogo data is not compatible. Please fix issues first."
                    )
                    return

            self.context.print_info(
                f"Converting NetLogo data from {netlogo_directory} to {output_directory}"
            )
            ...
            self.context.print_success(
                f"NetLogo data converted successfully to: {output_directory}"
            )
    ...
    def create_parameter_template(
        ...
        try:
            ...
            self.context.print_info(
                f"Creating {parameter_type.value} parameter template"
            )
