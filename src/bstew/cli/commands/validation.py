...
class ValidateCommand(BaseCLICommand):
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            results = self._run_validation_analysis(
                model_data, field_df, metrics
            )
    ...
    def validate_inputs(self, **kwargs: Any) -> List[str]:
        ...
        if metrics != "all":
            valid_metrics = [
                "rmse", "mae", "r_squared", "correlation", "bias", 
                "nash_sutcliffe", "kling_gupta", "percent_bias"
            ]
    ...
    def _run_validation_analysis(
        ...
        with self.progress_manager.progress_context() as progress:
            # Data alignment
            align_task = progress.start_task("Aligning model and field data...", total=3)
            ...
            metrics_task = progress.start_task("Calculating validation metrics...", total=5)
    ...
    def _calculate_validation_metrics(
        ...
        for variable, data in aligned_data.items():
            ...
            var_metrics["nash_sutcliffe"] = 1 - (ss_res / ss_tot) if ss_tot != 0 else -np.inf
            
            # Kling-Gupta Efficiency
            r = var_metrics["correlation"]
            alpha = np.std(model_clean) / np.std(obs_clean)
            beta = np.mean(model_clean) / np.mean(obs_clean)
            var_metrics["kling_gupta"] = 1 - np.sqrt((r - 1)**2 + (alpha - 1)**2 + (beta - 1)**2)
            
            # Percent Bias
            var_metrics["percent_bias"] = (np.sum(model_clean - obs_clean) / np.sum(obs_clean)) * 100
            
            validation_metrics[variable] = var_metrics
        
        return validation_metrics
    
    def _run_statistical_tests(self, aligned_data: Dict[str, pd.DataFrame]) -> Dict[str, Dict[str, Any]]:
        ...
        for variable, data in aligned_data.items():
            ...
            if len(model_clean) == len(obs_clean):
                try:
                    wilcoxon_stat, wilcoxon_pvalue = stats.wilcoxon(model_clean, obs_clean)
                    ...
                    var_tests["wilcoxon_test"] = {"error": "Unable to compute"}
    ...
    def _analyze_residuals(self, aligned_data: Dict[str, pd.DataFrame]) -> Dict[str, Dict[str, Any]]:
        ...
        for variable, data in aligned_data.items():
            ...
            analysis = {
                ...
                "heteroscedasticity": self._test_heteroscedasticity(residuals, np.array(model_vals)),
            }
    ...
    def _test_heteroscedasticity(self, residuals: np.ndarray, predicted: np.ndarray) -> Dict[str, Any]:
    ...
    def _generate_validation_recommendations(
        ...
        if not recommendations:
            recommendations.append("Model performance is generally good across all metrics")
    ...
    def _generate_validation_summary(self, metrics: Dict[str, Dict[str, float]]) -> Dict[str, Any]:
        """Generate overall validation summary"""
        
        if not metrics:
            return {"status": "no_data", "message": "No validation metrics calculated"}
        
        # Calculate average metrics across variables
        avg_metrics = {}
        for metric_name in ["r_squared", "rmse", "mae", "nash_sutcliffe"]:
            values = [var_metrics.get(metric_name, 0) for var_metrics in metrics.values()]
            avg_metrics[f"avg_{metric_name}"] = np.mean(values)
        
        # Determine overall performance
        avg_r2 = avg_metrics["avg_r_squared"]
        avg_nse = avg_metrics["avg_nash_sutcliffe"]
        ...
        return {
            ...
            "best_variable": max(metrics.keys(), key=lambda k: metrics[k]["r_squared"]),
            "worst_variable": min(metrics.keys(), key=lambda k: metrics[k]["r_squared"]),
        }
    ...
    def _display_validation_summary(self, results: Dict[str, Any]) -> None:
        ...
        summary_table.add_row("Overall Performance", summary["overall_performance"].title())
        summary_table.add_row("Variables Validated", str(summary["variables_validated"]))
        summary_table.add_row("Average R²", f"{summary['average_metrics']['avg_r_squared']:.3f}")
        summary_table.add_row("Average RMSE", f"{summary['average_metrics']['avg_rmse']:.3f}")
    ...
    def _save_validation_results(self, results: Dict[str, Any], output_path: Path) -> None:
