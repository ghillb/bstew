...
class PerformanceRegressionSuite:
    ...
    def run_regression_check(self) -> Dict[str, Any]:
        ...
        if not self.baseline_data:
            ...
            return {
                ...
                "baseline_file": str(
                    self.output_directory / "performance_baseline.json"
                ),
            }
    ...
    def _compare_with_baseline(
        ...
        if (
            "simulation_speed" in baseline_data
            and "simulation_speed" in current_results
        ):
            ...
            for size_name in baseline_speed:
                if size_name in current_speed:
                    ...
                    if (
                        isinstance(baseline_result, dict)
                        and "steps_per_second" in baseline_result
                        and hasattr(current_result, "steps_per_second")
                    ):
        ...
        if (
            "memory_efficiency" in baseline_data
            and "memory_efficiency" in current_results
        ):
            ...
            for scenario_name in baseline_memory:
                if scenario_name in current_memory:
                    ...
                    if (
                        isinstance(baseline_result, dict)
                        and "memory_peak" in baseline_result
                        and hasattr(current_result, "memory_peak")
                    ):
                        ...
                        regression = RegressionResult(
                            ...
                            severity=self._determine_severity(
                                change_percent, higher_is_worse=True
                            ),
                        )
                        regressions.append(regression)
    ...
    def _determine_severity(
        self, change_percent: float, higher_is_worse: bool = False
    ) -> str:
    ...
    def _generate_regression_report(
        ...
        improvements = [
            r for r in regressions if not r.is_regression and r.change_percent > 0.05
        ]
    ...
    def _generate_recommendations(
        ...
        if critical_regressions:
            ...
            speed_issues = [
                r for r in critical_regressions if "simulation_speed" in r.metric_name
            ]
            memory_issues = [
                r for r in critical_regressions if "memory" in r.metric_name
            ]

            if speed_issues:
                recommendations.append(
                    "Critical simulation speed regressions found. Check recent algorithm changes."
                )
            if memory_issues:
                recommendations.append(
                    "Critical memory usage regressions found. Check for memory leaks."
                )

        elif major_regressions:
            recommendations.append(
                "Major performance regressions detected. Investigation recommended."
            )
        ...
        speed_regressions = [
            ...
            if r.is_regression and "simulation_speed" in r.metric_name
        ]
        if speed_regressions:
            avg_speed_loss = sum(
                abs(r.change_percent) for r in speed_regressions
            ) / len(speed_regressions)
            if avg_speed_loss > 0.2:
                recommendations.append(
                    f"Average simulation speed loss: {avg_speed_loss:.1%}. Consider performance optimization."
                )

        memory_regressions = [
            r for r in regressions if r.is_regression and "memory" in r.metric_name
        ]
        if memory_regressions:
            recommendations.append(
                "Memory usage has increased. Review recent changes for potential memory leaks."
            )
        ...
        if not any(r.is_regression for r in regressions):
            ...
            if improvements:
                recommendations.append(
                    "Performance improvements detected! Good work on optimization."
                )
            else:
                recommendations.append(
                    "Performance is stable. No significant changes detected."
                )
    ...
    def _save_regression_results(
        self, regressions: List[RegressionResult], current_results: Dict[str, Any]
    ) -> None:
        ...
        regression_file = (
            self.output_directory / f"regression_analysis_{timestamp}.json"
        )
        regression_data = {
            "timestamp": datetime.now().isoformat(),
    ...
    def set_custom_thresholds(
        self, minor: float, major: float, critical: float
    ) -> None:
        """Set custom regression thresholds"""

        self.thresholds = {"minor": minor, "major": major, "critical": critical}

        self.logger.info(
            f"Custom thresholds set: minor={minor:.1%}, major={major:.1%}, critical={critical:.1%}"
        )
