...
class NetLogoParityBenchmarks:
    ...
    def _benchmark_simulation_speed(self) -> Dict[str, BenchmarkResult]:
        ...
        for size_name, bee_count in colony_sizes:
            self.logger.info(
                f"Benchmarking simulation speed for {size_name} ({bee_count} bees)"
            )
        ...
        for bee_count in colony_sizes:
            ...
            try:
                ...
                results[size_name] = BenchmarkResult(
                    ...
                    success=True,
                    metadata={"bee_count": bee_count},
                )

            except Exception as e:
                self.logger.error(
                    f"Initialization benchmark failed for {bee_count} bees: {e}"
                )
                results[size_name] = BenchmarkResult(
                    benchmark_name=f"initialization_{size_name}",
                    execution_time=0.0,
                ...
                scalability_results.append(
                    {
                        ...
                        / bee_count,  # Steps per second per bee
                    }
                )

            except Exception as e:
                self.logger.error(f"Scalability test failed for {bee_count} bees: {e}")
                # Skip failed test results to maintain type consistency
                self.logger.warning(
                    f"Skipping failed scalability test for {bee_count} bees"
                )
    ...
    def _benchmark_behavioral_accuracy(self) -> Dict[str, Any]:
        ...
        for run in range(5):  # 5 replicate runs
            ...
            try:
                ...
                if (
                    model.system_integrator is None
                    or model.system_integrator.data_collector is None
                ):
                    continue
                ...
                for colony_metrics in data_collector.colony_metrics.values():
                    if (
                        hasattr(colony_metrics, "population_size_day_list")
                        and colony_metrics.population_size_day_list
                    ):
                ...
                # Log error separately to maintain type consistency
                self.logger.warning(f"Skipping failed consistency test run {run}")
        ...
        if len(successful_runs) > 1:
            ...
            return {
                ...
                "consistency_analysis": {
                    "population_cv": statistics.stdev(populations)
                    / statistics.mean(populations)
                    if populations
                    else 0,
                    "foraging_cv": statistics.stdev(foraging_trips)
                    / statistics.mean(foraging_trips)
                    if foraging_trips
                    else 0,
    ...
    def _compare_with_netlogo_references(self) -> List[NetLogoComparisonResult]:
        ...
        for size_name, result in speed_results.items():
            if (
                result.success
                and size_name in self.netlogo_references["simulation_speed"]
            ):
                ...
                comparison = NetLogoComparisonResult(
                    ...
                    relative_difference=abs(result.steps_per_second - netlogo_ref)
                    / netlogo_ref,
                    passes_benchmark=result.steps_per_second
                    >= netlogo_ref * 0.8,  # 80% of NetLogo performance
                )
        ...
        try:
            ...
            for size_name, current_result in current_speed_results.items():
                if (
                    ...
                    and "simulation_speed" in previous_results
                    and size_name in previous_results["simulation_speed"]
                ):
                        ...
                        regression_analysis.append(
                            {
                                ...
                                "is_regression": performance_change
        ...
        with open(summary_file, "w") as f:
            ...
            if "simulation_speed" in results:
                ...
                for size_name, result in results["simulation_speed"].items():
                    if hasattr(result, "success") and result.success:
                        f.write(
                            f"- **{size_name}**: {result.steps_per_second:.1f} steps/second\n"
                        )
            ...
            if "netlogo_comparison" in results:
                ...
                for comparison in results["netlogo_comparison"]:
                    ...
                    f.write(
                        f"  - NetLogo Reference: {comparison.netlogo_reference:.1f}\n"
                    )
                    f.write(
                        f"  - Performance Ratio: {comparison.performance_ratio:.2f}x\n"
                    )
                f.write("\n")
